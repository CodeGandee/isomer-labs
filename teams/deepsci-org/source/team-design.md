...
| --- | --- | --- |
| `deepsci-org-master` | `isomer-rsch-shared`, `isomer-rsch-decision`, `isomer-rsch-finalize` | `isomer-rsch-review` when the master must inspect a review handoff before opening a Gate. |
...
- Use `isomer-rsch-shared` and `isomer-rsch-decision` to normalize non-blank Topic Workspace state, then hand off trust-ranking work to `isomer-rsch-scout` or `isomer-rsch-review` when the prior evidence needs a separate pass.

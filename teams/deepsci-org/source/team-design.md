...
This document designs `deepsci-org` as a Domain Agent Team Template for Isomer Labs research work. It adapts the DeepScientist research approach into Isomer terms, installs the local `isomer-deepsci-*` skill bundles, and keeps runtime concerns such as launch tooling, credentials, scheduler internals, mailbox routes, and concrete workspace paths outside the role design.
...
| --- | --- | --- |
| `deepsci-org-master` | `isomer-deepsci-shared`, `isomer-deepsci-decision`, `isomer-deepsci-finalize` | `isomer-deepsci-review` when the master must inspect a review handoff before opening a Gate. |
| `deepsci-org-framer` | `isomer-deepsci-shared`, `isomer-deepsci-scout`, `isomer-deepsci-baseline` | `isomer-deepsci-science` for domain package checks, `isomer-deepsci-paper-outline` when the framer must state paper evidence needs early. |
| `deepsci-org-designer` | `isomer-deepsci-shared`, `isomer-deepsci-idea`, `isomer-deepsci-optimize` | `isomer-deepsci-scout` for narrow literature checks that change idea selection. |
| `deepsci-org-experimenter` | `isomer-deepsci-shared`, `isomer-deepsci-experiment`, `isomer-deepsci-science` | `isomer-deepsci-analysis` for tiny post-run checks that do not justify a separate analysis handoff. |
| `deepsci-org-analyzer` | `isomer-deepsci-shared`, `isomer-deepsci-analysis`, `isomer-deepsci-science` | `isomer-deepsci-paper-plot`, `isomer-deepsci-figure-polish` for analysis-facing result displays. |
| `deepsci-org-publisher` | `isomer-deepsci-shared`, `isomer-deepsci-paper-outline`, `isomer-deepsci-write`, `isomer-deepsci-paper-plot`, `isomer-deepsci-figure-polish` | Source DeepScientist companions `nature-data`, `nature-figure`, `nature-paper2ppt`, and `nature-polishing`, or future Isomer equivalents, when those bundles are available in the active Skill Binding projection. |
| `deepsci-org-reviewer` | `isomer-deepsci-shared`, `isomer-deepsci-review`, `isomer-deepsci-rebuttal`, `isomer-deepsci-analysis` | `isomer-deepsci-scout` for novelty benchmarking and comparator-paper checks. |
...
- Use `isomer-deepsci-shared` and `isomer-deepsci-decision` to normalize non-blank Topic Workspace state, then hand off trust-ranking work to `isomer-deepsci-scout` or `isomer-deepsci-review` when the prior evidence needs a separate pass.
...
- Use `isomer-deepsci-decision` for go, stop, branch, write, finalize, reset, baseline waiver, and preference-sensitive choices.
- Use `isomer-deepsci-finalize` to consolidate claims, limitations, recommendations, resume packets, and closure state.
...
- Use `isomer-deepsci-paper-plot` for first-pass standard figures and `isomer-deepsci-figure-polish` for final render-inspect-revise work.
...
4. Routine route choices, such as moving from baseline to idea after a confirmed comparator, can proceed through `isomer-deepsci-decision` without user interruption.
...
| Required shared skill | `isomer-deepsci-shared` for all roles |

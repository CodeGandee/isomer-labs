...
- [x] Stage 6 skillset validation is implemented, tested, and runnable with `pixi run validate-research-skills`.
...
- [x] Add a validation check for stale terms such as Research Goal, Research Thread, Research Branch, and Isomer Workspace.
- [x] Add a validation check that ordinary workspace path TBDs are not reintroduced.
- [x] Add a validation check that every `[[tbd-surface:*]]` id is registered.
- [x] Add a validation check for hard-coded absolute paths and source-analysis paths.
- [x] Add a validation check for broken local reference links from each `SKILL.md`.
- [x] Add a validation check that `agents/openai.yaml` display names match skill names.
- [x] Add CI or repository commands for running the skillset validation harness.
...
- [x] Decide whether venue LaTeX templates should be imported as sanitized optional assets, project-provided templates, or external Capability Bindings.
- [ ] Sanitize any accepted venue templates by removing sample content, source-local notes, TODO macros, and unclear licensing.
- [x] Decide whether paper-plot scripts should be imported as reusable parameterized generators under `scripts/`.
...
- [x] Decide whether the science package-card catalog should be imported as local references, generated assets, or a Capability Binding.
- [ ] Define package-card freshness, source URL, license, and Provenance Record behavior before importing catalog material.
- [x] Consider a separate publication-extension change for Nature-specific helpers.
...
- [x] Run skillset validation once the validation harness exists.

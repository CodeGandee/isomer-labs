...
- Local DeepScientist MCP and artifact implementation: [`../../../extern/orphan/DeepScientist/src/deepscientist/mcp/server.py`](../../../extern/orphan/DeepScientist/src/deepscientist/mcp/server.py), [`../../../extern/orphan/DeepScientist/src/deepscientist/artifact/service.py`](../../../extern/orphan/DeepScientist/src/deepscientist/artifact/service.py), and [`../../../extern/orphan/DeepScientist/src/deepscientist/artifact/schemas.py`](../../../extern/orphan/DeepScientist/src/deepscientist/artifact/schemas.py).
...
- Runtime and semantic-surface implementation anchors: [`../../../src/isomer_labs/workspace/semantic_surfaces.py`](../../../src/isomer_labs/workspace/semantic_surfaces.py), [`../../../src/isomer_labs/runtime/models.py`](../../../src/isomer_labs/runtime/models.py), [`../../../src/isomer_labs/runtime/schema.py`](../../../src/isomer_labs/runtime/schema.py), [`../../../src/isomer_labs/runtime/store.py`](../../../src/isomer_labs/runtime/store.py), and [`../../../src/isomer_labs/runtime/validation_checks.py`](../../../src/isomer_labs/runtime/validation_checks.py).
...
The default storage design should keep a layered separation. Topic Actors work inside Topic Actor Workspace worktrees, formal team agents work inside Agent Workspace worktrees, and the operator can use the Topic Main Development Repository as the integration surface, but accepted research state belongs to the Topic Workspace record store. The storage authority is the semantic label or typed record ref, not the worker's current working directory and not a hard-coded relative path.
...
<topic-workspace>/
  ...
  actors/
    <topic-actor-name>/                # [exist.reuse] topic.actors.workspace, human-orchestrated worker cwd and worktree
      isomer-managed/
        actor-owned/
          artifacts/                   # [exist.reuse] topic.actors.private_artifacts, actor-owned outputs before promotion
          logs/                        # [exist.reuse] topic.actors.logs, actor-local diagnostics before promotion
        links/                         # [exist.reuse] topic.actors.links, generated conveniences and start-pack pointers
      tmp/                              # [exist.reuse] topic.actors.tmp, local ignored disposable material
  agents/
...
Topic Actor-local work should start under `topic.actors.private_artifacts`, `topic.actors.logs`, `topic.actors.links`, or ordinary files in `topic.actors.workspace` according to ownership and visibility. Formal Agent-local work should start under `agent.private_artifacts`, `agent.scratch`, `agent.logs`, or `agent.public_share`. That material is not accepted topic evidence until a promotion or record workflow records it as an Artifact, Evidence Item, Provenance Record, Decision Record, or package entry under `records/*`.
...
| `topic.actors_root` | `[exist.reuse]` | `actors` | Root for human-orchestrated Topic Actor Workspaces. |
| `topic.actors.workspace` | `[exist.reuse]` | `actors/<topic-actor-name>` | Topic Actor cwd and worktree for manually controlled workers; not an accepted research record store. |
| `topic.actors.private_artifacts` | `[exist.reuse]` | `actors/<topic-actor-name>/isomer-managed/actor-owned/artifacts` | Topic Actor-owned outputs before promotion or record creation. |
| `topic.actors.logs` | `[exist.reuse]` | `actors/<topic-actor-name>/isomer-managed/actor-owned/logs` | Topic Actor-local diagnostics before promotion. |
| `topic.actors.links` | `[exist.reuse]` | `actors/<topic-actor-name>/isomer-managed/links` | Generated conveniences and start-pack pointers, not storage authority. |
| `topic.actors.tmp` | `[exist.reuse]` | `actors/<topic-actor-name>/tmp` | Local ignored disposable material, not shared and not durable evidence. |
...
### Paper Writing Artifact Mapping

DeepScientist paper writing storage is a coordinated paper-line package rather than a single manuscript file. Its MCP server exposes read and validation tools such as `artifact.get_paper_contract`, `artifact.get_paper_contract_health`, `artifact.validate_academic_outline`, `artifact.validate_manuscript_language`, and `artifact.validate_manuscript_coverage`, plus state-changing tools such as `artifact.submit_paper_outline`, `artifact.compile_outline_to_writing_plan`, `artifact.create_analysis_campaign`, `artifact.record_analysis_slice`, and `artifact.submit_paper_bundle`. In source storage, those tools write under `paper/` and also create generic `report`, `run`, `milestone`, or delta artifacts; `schemas.py` does not define a first-class paper artifact kind.

The Isomer binding should preserve that paper-line semantics without copying DeepScientist's `quest_root/paper/` authority model. In Isomer, accepted paper state belongs to Topic Workspace records and Workspace Runtime lifecycle rows; editable source files may live in `topic.repos.main` or an Agent Workspace only while they are being authored. The v2 skills should keep placeholders in workflow prose, use each skill's `placeholder-bindings.md` as the local command guide, and use the mapping below as the shared paper-writing interpretation layer.

Current storage should use `topic.records.views` for active paper contract views, ledgers, matrices, validation state, and other readable control surfaces; `topic.records.artifacts` for raw bodies, manuscript drafts, review packets, rebuttal packets, bibliography files, PDFs, compile reports, and bundle manifests; `topic.records.tasks` for writing jobs or reviewer-linked work queues when those jobs need to be resumed as tasks; and `topic.records.runs` for executions that produced paper-facing results. Future `topic.records.evidence` and `topic.records.packages` labels should become the preferred storage surfaces for Evidence Items and bundle inventories once those labels are implemented.

| DeepScientist paper surface | Paper role | V2 placeholder surfaces | Current Isomer storage binding | Notes |
|---|---|---|---|---|
| `paper/outlines/candidates/*.json` and `paper/outlines/revisions/*.json` | Candidate and revised paper outlines before or after selection. | `<PAPER_STATE_SNAPSHOT>`, `<ONE_SENTENCE_PAPER_IDEA>`, `<PAPER_VIEW>`, `<EVIDENCE_VIEW>`, `<PAPER_OUTLINE>` | `artifact` under `topic.records.artifacts`, profiles `paper.outline.candidate` and `paper.outline.revision`. | Store the full source-like outline payload, including `paper_view`, `evidence_view`, and any reviewer or analysis plan fields. |
| `paper/selected_outline.json`, `paper/outline_selection.md`, `paper/outline/manifest.json`, and `paper/outline/sections/*` | Active paper contract and section-level evidence contract. | `<PAPER_CONTRACT>`, `<PAPER_OUTLINE>`, `<SECTION_WRITING_PLAN>`, `<CLAIM_EVIDENCE_BOUNDARY>` | Primary `view_manifest` under `topic.records.views`, profile `paper.contract.selected-outline`; raw selected outline and section bodies may also be `artifact` records under `topic.records.artifacts`, profile `paper.outline.selected`. | This is the main read surface for write, review, rebuttal, finalize, and Nature companion skills. It should carry `selected_outline_ref`, section ids, required and optional item ids, paper roles, claim links, and source Artifact refs. |
| `paper/evidence_ledger.json` and `paper/evidence_ledger.md` | Master paper evidence ledger tying runs, analyses, figures, claims, sections, and source paths together. | `<SOURCE_MATERIAL_LEDGER>`, `<CLAIM_EVIDENCE_BOUNDARY>`, `<EVIDENCE_VIEW>`, `<REBUTTAL_EVIDENCE_UPDATE>`, `<FINALIZE_CONTEXT_BRIEF>` | Ledger index as `view_manifest` under `topic.records.views`, profile `paper.evidence-ledger`; individual evidence rows as `evidence_item` records with bodies under `topic.records.artifacts` today, future `topic.records.evidence` later. | Treat the ledger as an index, not as the only evidence body. Each row should link to Run refs, Artifact refs, paper section ids, claim ids, status, metrics, and source paths. |
| `paper/paper_experiment_matrix.json` and `paper/paper_experiment_matrix.md` | Paper-facing experiment, analysis, display, and section matrix. | `<PAPER_EXPERIMENT_MATRIX_UPDATE>`, `<DISPLAY_PLAN>`, `<REVIEW_EXPERIMENT_TODO>`, `<REVIEWER_LINKED_EVIDENCE_TODO>` | `view_manifest` under `topic.records.views`, profile `paper.experiment-matrix`; reviewer-specific execution subsets may be `research_task` records under `topic.records.tasks`. | This matrix should remain the paper-facing master plan. Review and rebuttal TODOs are subsets or deltas, not replacements. |
| `paper/writing_plan.json` and `paper/writing_plan.md` | Concrete section jobs compiled from the selected outline. | `<WRITING_PLAN>`, `<SECTION_WRITING_PLAN>`, `<DRAFT_SECTION_SET>` | `research_task` under `topic.records.tasks`, profile `paper.writing-plan`, or `view_manifest` under `topic.records.views` when it is used as a board. | Use task records when jobs must be resumed or assigned; use a view when the plan is only a compiled read surface. |
| `paper/draft.md`, `paper/latex/`, section source files, and manuscript text deltas | Editable manuscript source and drafted section bodies. | `<DRAFT_SECTION_SET>`, `<MANUSCRIPT_TEXT_DELTA>`, `<PAPER_BUNDLE_CHECKPOINT>` | `artifact` under `topic.records.artifacts`, profiles `paper.draft`, `paper.latex-root`, `paper.section-draft`, and `paper.manuscript-text-delta`; editable copies may temporarily live in `topic.repos.main` or `agent.private_artifacts`. | Accepted draft records must not depend on agent scratch paths. If Git review matters, record the repo path, branch, commit, and Artifact ref together. |
| `paper/references.bib` and citation audit notes | Machine-usable bibliography and citation legitimacy state. | `<CITATION_LEDGER>` | `artifact` or `evidence_item` under `topic.records.artifacts`, profiles `paper.references-bib` and `paper.citation-ledger`. | Store BibTeX as a body file and keep provenance for DOI, arXiv, OpenAlex, DeepXiv, or other provider lookups. |
| `paper/claim_evidence_map.json` | Reader-facing claim to evidence map. | `<PAPER_CONTRACT>`, `<CLAIM_EVIDENCE_BOUNDARY>`, `<MANUSCRIPT_VALIDATION_REPORT>`, `<CLAIM_LEDGER>` | `view_manifest` under `topic.records.views`, profile `paper.claim-evidence-map`, with linked `research_claim` and `evidence_item` records where available. | This map is the bridge from paper claims to storage refs. It should use claim ids and Evidence Item refs rather than only prose summaries. |
| `paper/academic_outline_validation.json`, `paper/manuscript_language_validation.json`, `paper/manuscript_coverage.json`, and `paper/paper_line_state.json` | Validation reports, gate status, and active paper-line readiness. | `<OUTLINE_VALIDATION_REPORT>`, `<MANUSCRIPT_VALIDATION_REPORT>`, `<FINALIZE_CONTEXT_BRIEF>`, `<FINALIZE_BLOCKER_RECORD>` | `view_manifest` under `topic.records.views`, profiles `paper.validation.academic-outline`, `paper.validation.manuscript-language`, `paper.validation.manuscript-coverage`, and `paper.line-state`; blockers can also be `decision_record` or `gate` rows. | These records drive routing. `manuscript_ready=true` can support review, but finalize should require `submission_ready=true` and a submission package. |
| `paper/paper_bundle_manifest.json`, `paper/baseline_inventory.json`, and `paper/artifact_deltas/*.json` | Bundle inventory, baseline inventory, and state-change sidecars. | `<PAPER_BUNDLE_CHECKPOINT>`, `<REVISION_HANDOFF_BUNDLE>`, `<FINALIZE_CONTEXT_BRIEF>`, `<RESUME_PACKET>` | `artifact` under `topic.records.artifacts`, profiles `package.paper.bundle-manifest`, `paper.baseline-inventory`, and `paper.artifact-delta`; future `topic.records.packages` should own package manifests. | The bundle manifest is the package authority for draft checkpoints, review packages, and submission packages until native package records exist. |
| `paper/review/review.md`, `paper/review/revision_log.md`, and `paper/review/experiment_todo.md` | Independent review package and review-facing TODOs. | `<REVIEW_AUDIT_PLAN>`, `<REVIEW_REPORT>`, `<REVISION_LOG>`, `<REVIEW_EXPERIMENT_TODO>`, `<REVIEW_ROUTE_DECISION>` | `artifact` under `topic.records.artifacts`, profiles `review.audit-plan`, `review.report`, `review.revision-log`, and `review.experiment-todo`; route choices as `decision_record`. | Review TODOs should link back to the paper experiment matrix, section ids, claim ids, and Evidence Item gaps. |
| `paper/rebuttal/review_matrix.md`, `paper/rebuttal/action_plan.md`, `paper/rebuttal/response_letter.md`, `paper/rebuttal/text_deltas.md`, `paper/rebuttal/evidence_update.md`, and `paper/rebuttal/input/*` | Reviewer item normalization, rebuttal plan, response text, manuscript deltas, evidence updates, and source review packets. | `<REVIEW_PACKAGE_NORMALIZATION>`, `<REVIEWER_ITEM_MATRIX>`, `<REBUTTAL_ACTION_PLAN>`, `<REVIEWER_LINKED_EVIDENCE_TODO>`, `<REBUTTAL_EVIDENCE_UPDATE>`, `<MANUSCRIPT_TEXT_DELTA>`, `<RESPONSE_LETTER_DRAFT>`, `<REVISION_HANDOFF_BUNDLE>` | `artifact` and `evidence_item` records under `topic.records.artifacts`, profiles `rebuttal.review-package-normalization`, `rebuttal.reviewer-item-matrix`, `rebuttal.action-plan`, `rebuttal.response-letter`, `rebuttal.text-deltas`, and `rebuttal.evidence-update`; reviewer-linked execution tasks under `topic.records.tasks` when needed. | Preserve reviewer wording and stable reviewer item ids. Evidence updates should cite the reviewer id, manuscript target, claim id, Evidence Item ref, and any route-back decision. |
| `paper/build/compile_report.json`, `paper/paper.pdf`, figure/table catalogs, and final figure exports | Build/render evidence and paper-facing outputs. | `<MANUSCRIPT_VALIDATION_REPORT>`, `<FIRST_PASS_FIGURE>`, `<FINAL_FIGURE_EXPORT>`, `<FIGURE_PROVENANCE_RECORD>`, `<PAPER_BUNDLE_CHECKPOINT>` | `artifact` records under `topic.records.artifacts`, profiles `paper.compile-report`, `paper.pdf`, `figure.export`, and `figure.catalog`; render checks as `evidence_item` or `provenance_record` records. | A PDF or figure is not enough by itself. Keep compile logs, render-inspection notes, source data refs, and provenance refs with the exported body. |
| `release/open_source/manifest.json` and `release/open_source/cleanup_plan.md` | Optional source-release preparation packet. | `<PAPER_BUNDLE_CHECKPOINT>`, `<FINAL_SUMMARY>`, `<CLOSURE_DECISION>` | `artifact` under `topic.records.artifacts`, profile `release.open-source-manifest` and `release.open-source-cleanup-plan`; future package records should include release readiness. | Link release material to the paper bundle, baseline inventory, Decision Record, and finalization package. |

The transitional CRUD pattern should make the paper-line object explicit through `--profile`, `--semantic-label`, `--placeholder`, and structured metadata. Examples:

```bash
pixi run isomer-cli --print-json ext research records create --topic <topic> --record-kind view_manifest --semantic-label topic.records.views --placeholder '<PAPER_CONTRACT>' --profile paper.contract.selected-outline --skill isomer-rsch-write-v2 --producer 'isomer-rsch-paper-outline-v2 or isomer-rsch-write-v2' --consumer 'write, review, rebuttal, finalize, nature companion skills' --body-file <selected-outline-json> --content-name selected-outline.json --metadata-json '{"selected_outline_ref":"<outline-id>","paper_surface":"selected_outline"}'
pixi run isomer-cli --print-json ext research records create --topic <topic> --record-kind view_manifest --semantic-label topic.records.views --placeholder '<SOURCE_MATERIAL_LEDGER>' --profile paper.evidence-ledger --skill isomer-rsch-write-v2 --producer 'write, analysis, experiment, figure-polish, rebuttal' --consumer 'write, review, rebuttal, finalize' --body-file <evidence-ledger-json> --content-name evidence-ledger.json --metadata-json '{"selected_outline_ref":"<outline-id>","paper_surface":"evidence_ledger"}'
pixi run isomer-cli --print-json ext research records create --topic <topic> --record-kind view_manifest --semantic-label topic.records.views --placeholder '<PAPER_EXPERIMENT_MATRIX_UPDATE>' --profile paper.experiment-matrix --skill isomer-rsch-review-v2 --producer 'write, review, rebuttal, analysis' --consumer 'write, analysis, rebuttal, finalize' --body-file <paper-experiment-matrix-json> --content-name paper-experiment-matrix.json --metadata-json '{"selected_outline_ref":"<outline-id>","paper_surface":"experiment_matrix"}'
pixi run isomer-cli --print-json ext research records create --topic <topic> --record-kind research_task --semantic-label topic.records.tasks --placeholder '<WRITING_PLAN>' --profile paper.writing-plan --skill isomer-rsch-write-v2 --producer 'isomer-rsch-write-v2' --consumer 'write' --body-file <writing-plan-json> --content-name writing-plan.json --metadata-json '{"selected_outline_ref":"<outline-id>","paper_surface":"writing_plan"}'
pixi run isomer-cli --print-json ext research records create --topic <topic> --record-kind artifact --semantic-label topic.records.artifacts --placeholder '<PAPER_BUNDLE_CHECKPOINT>' --profile package.paper.bundle-manifest --skill isomer-rsch-write-v2 --producer 'isomer-rsch-write-v2' --consumer 'review, finalize, user' --body-file <paper-bundle-manifest-json> --content-name paper-bundle-manifest.json --metadata-json '{"package_type":"draft_checkpoint|review_package|submission_package","selected_outline_ref":"<outline-id>"}'
```

Working agents should read prior paper state by querying records before scanning files:

```bash
pixi run isomer-cli --print-json ext research records list --topic <topic> --profile paper.contract.selected-outline
pixi run isomer-cli --print-json ext research records list --topic <topic> --profile paper.evidence-ledger
pixi run isomer-cli --print-json ext research records list --topic <topic> --profile package.paper.bundle-manifest
pixi run isomer-cli --print-json ext research records show <record-id> --topic <topic> --include-body
pixi run isomer-cli --print-json ext research records update <record-id> --topic <topic> --record-kind view_manifest --semantic-label topic.records.views --placeholder '<PAPER_CONTRACT>' --profile paper.contract.selected-outline --body-file <updated-selected-outline-json> --metadata-json '{"selected_outline_ref":"<outline-id>","paper_surface":"selected_outline"}'
```

Paper package readiness should preserve DeepScientist's package-type semantics. A `draft_checkpoint` records a coherent draft surface but routes back to write or review; a `review_package` records a handoff for skeptical review and does not make the paper final; a `submission_package` should be accepted only when selected outline validation, manuscript language validation, manuscript coverage, evidence gate, bibliography, claim map, build outputs, and bundle inventory are ready. `isomer-rsch-finalize-v2` should treat a draft or review package as context, not as final submission evidence.

Topic Workspace initialization for paper-writing skills should prepare the generic labels first, then the research bootstrap should publish the paper profile catalog that agents can use in `placeholder-bindings.md`: `paper.contract.selected-outline`, `paper.outline.candidate`, `paper.outline.revision`, `paper.evidence-ledger`, `paper.experiment-matrix`, `paper.writing-plan`, `paper.claim-evidence-map`, `paper.validation.academic-outline`, `paper.validation.manuscript-language`, `paper.validation.manuscript-coverage`, `paper.line-state`, `paper.draft`, `paper.references-bib`, `package.paper.bundle-manifest`, `review.*`, `rebuttal.*`, `figure.export`, and `release.open-source-*`. The bootstrap should materialize `topic.records.artifacts`, `topic.records.views`, `topic.records.tasks`, `topic.records.runs`, and `topic.records.logs`; it should also advertise planned `topic.records.evidence` and `topic.records.packages` labels but should not require them until the storage layer implements those labels.
...
After common Topic Workspace preparation, and after Topic Actor or formal team preparation when those layers are selected, `isomer-rsch-workspace-mgr-v2` should perform the research-specific bootstrap pass. That pass validates the selected Topic Workspace context, records available and planned semantic labels, builds the v2 placeholder binding registry, prepares the topic-level placeholder binding index, prepares the Topic Actor or Agent Workspace access plan, and returns either a bootstrap validation report or a blocker record. The optional Topic Service Master is the preferred actor when it is running from the Topic Workspace cwd; otherwise the Project Operator Session or Operator Agent performs the same bounded bootstrap work.

Topic Actor readiness adds these checks on top of base topic readiness: the Topic Workspace Manifest has a selected Topic Actor binding, `topic.actors.workspace` resolves and is materialized, the actor branch follows `per-topic-actor/<topic-actor-name>/...`, actor support labels such as `topic.actors.private_artifacts`, `topic.actors.logs`, `topic.actors.links`, and `topic.actors.tmp` resolve or report blockers, and accepted-record command guidance includes `--topic-actor <topic-actor-name>` plus known actor kind, runtime kind, controller kind, and adapter ref metadata. Topic Actor readiness does not require `team-profile/`, Agent Team Instance records, Agent Instance records, formal Agent Workspace refs, Houmao launch dossiers, mailboxes, or gateways.

Actor start packs should be authoritative Topic Workspace research records, with actor-local copies or pointers only for startup convenience. A start pack should name the actor cwd, runtime kind, role kind, branch, selected v2 skills, skill-local `placeholder-bindings.md` files, topic-level placeholder binding index or readiness report, semantic labels, accepted-record command examples, blockers, next actions, authoritative record refs, and actor-local copy or pointer path. The actor-local copy belongs under `topic.actors.isomer_managed` or `topic.actors.links`; it must not become the authority for accepted artifacts.
...
Topic Actor Workspace and Agent Workspace setup should also prepare readable and writable pre-promotion surfaces for working actors and agents. At minimum, each Topic Actor should have path plans and directories for `topic.actors.workspace`, `topic.actors.private_artifacts`, `topic.actors.logs`, and `topic.actors.links`; each launched formal agent should have path plans and directories for `agent.workspace`, `agent.private_artifacts`, `agent.scratch`, `agent.logs`, `agent.public_share`, and `agent.links`. Durable records must not depend on actor-owned or agent-owned paths until a promotion command copies or registers the material under topic records and creates the matching typed refs.

## Storage Task Map by Skill

| Skill | Storage tasks described by the skill | Best current mapping | Missing or weak support |
|---|---|---|---|
| `isomer-rsch-workspace-mgr-v2` | Prepares post-preparation research bootstrap context, semantic label plan, placeholder binding registry, topic-level placeholder binding index, Topic Actor or Agent Workspace access plan, validation report, and blocker record before ordinary v2 skills run. | Workspace Path Resolution, Workspace Runtime inspect/validate, Path Plans, Topic Actor labels, Agent Workspace labels, and generic lifecycle rows can provide evidence for the bootstrap result. | No typed bootstrap record command, no public placeholder-binding registry command, no native actor start-pack command, no typed semantic-label readiness report, and no promotion readiness validator keyed to v2 placeholder kinds. |
...
| `isomer-rsch-paper-plot` | Stores source data Artifact or linked Evidence Item, figure-generation Artifact with script/notebook path, first rendered figure, inspection note, and optional handoff to figure polish. | Figure files and scripts can live under `topic.records.artifacts`; paper-facing display state should also update the paper experiment matrix and evidence ledger mappings above. | No source data Artifact kind, figure-generation Artifact kind, render-inspection note schema, plot output registration command, figure-to-paper-matrix link API, or handoff-to-figure-polish package profile. |
| `isomer-rsch-figure-polish` | Stores final figure exports as figure output Artifacts, source plotting Artifact/generation path, visual inspection note, self-review delta, and links to Evidence Items, Research Claims, report sections, reviewer items, Decisions, Gates, and Provenance. | Figure bodies can live under `topic.records.artifacts`; paper-facing exports should use the `figure.export`, `figure.catalog`, and `paper.experiment-matrix` profiles from the paper-writing artifact mapping. | No figure output Artifact kind, export manifest, visual inspection note type, figure-to-claim/reviewer/section link API, or provenance chain recorder for exported figures. |
| `isomer-rsch-paper-outline` | Stores candidate, selected, and revised outline Artifacts, paper view, evidence view, section-level writing plan, claim-evidence boundary list, reviewer objection list, analysis-plan roles, and Decision/Gate for unresolved route or paper scope. | Use `artifact` records for candidate/revised outline bodies, `view_manifest` for the selected outline and paper contract, and the `paper.contract.selected-outline`, `paper.outline.*`, `paper.claim-evidence-map`, and `paper.validation.academic-outline` profiles above. | No native outline Artifact kind, paper/evidence View Manifest writer, claim-boundary list schema, reviewer objection profile, or scope Gate command beyond the generic extension. |
| `isomer-rsch-write` | Stores updated paper, report, section, or manuscript package Artifact; aligned claim-evidence map; experiment or analysis matrix; bibliography; figure/table status; appendix bridge map; bundle status; optional rewrite plans; Decision Record for claim narrowing, route-back, blocker, review package, or submission-readiness Gate. | Use `view_manifest` for paper contract, evidence ledger, experiment matrix, claim-evidence map, validation, and paper-line state; use `artifact` for drafts, bibliography, compile reports, PDFs, and bundle manifests; use `research_task` for writing plans when resumable. | No native manuscript package command, claim-evidence map API, bibliography status record, figure/table status schema, package inventory recorder, review package handoff profile, or submission-readiness Gate workflow beyond the generic extension. |
| `isomer-rsch-review` | Stores review report Artifact, revision log Artifact, Evidence TODO Artifact, literature benchmark/comparison matrix, and Gate/Decision for route choice, claim downgrade, stop, review closure, and finalization readiness. | Use `artifact` profiles `review.audit-plan`, `review.report`, `review.revision-log`, and `review.experiment-todo`; use `view_manifest` profile `paper.experiment-matrix` for experiment matrix updates and `decision_record` for route choice. | No native review report Artifact profile, revision-log schema, Evidence TODO profile, literature benchmark matrix profile, or review closure Gate command beyond generic lifecycle records. |
| `isomer-rsch-rebuttal` | Stores reviewer item matrix, rebuttal action plan, evidence update, text delta, response letter or revision package, and Gate/Decision for supplementary evidence, baseline recovery, claim downgrade, and final handoff. | Use `artifact` and `evidence_item` profiles `rebuttal.*` for reviewer matrix, action plan, response letter, text deltas, and evidence updates; use `research_task` for reviewer-linked evidence TODOs and `artifact` profile `handoff.revision-handoff-bundle` for final handoff. | No native reviewer item matrix schema, response package Artifact kind, evidence update link API, text delta profile, rebuttal package recorder, or claim downgrade workflow beyond generic lifecycle records. |
...
| `isomer-rsch-finalize` | Stores final claim ledger or status summary, final report/handoff Artifact, final Decision Record, completion Gate when required, resume packet, closure handoff, and package inventory. | Final reports and inventories can be Artifacts; paper finalization should read `package.paper.bundle-manifest`, `paper.evidence-ledger`, `paper.claim-evidence-map`, validation views, review/rebuttal packets, and `report.claim-ledger` before closure. | No native claim-ledger API, package inventory recorder, finalization Gate workflow, closure handoff package profile, or final report bundle validation beyond generic lifecycle records. |

# GPU Analytical Modeling User-Toolbox Feature Requirement

## Goal

Create a project-local toolbox named `gpu-analytical-modeling` at `skillset/toolboxes/gpu-analytical-modeling` that supplies User Skill Callback instruction material for GPU performance research. The Toolbox should steer participating DeepSci system skills toward hardware-grounded analytical models: explicit equations, stated assumptions, measurable validation, and source trails for GPU internal architecture and execution behavior.
...
- Do not implement a new callback registry, Toolbox installer, or first-class toolbox runtime unless a later design stage decides that current `--skill-dir` registration is insufficient.
...
- Do not make the Toolbox override system instructions, developer instructions, owning system skill rules, current user requests, evidence Gates, validation obligations, or Isomer domain constraints.
...
- Project Operator Session: creates or updates `skillset/toolboxes/gpu-analytical-modeling`, reviews the callback skills, and registers selected callback directories with `pixi run isomer-cli --print-json project skill-callbacks register`.
...
- Research reviewer: uses the Toolbox's evidence and model-quality checklist to decide whether a proposed GPU analytical model is explanatory, falsifiable, and sufficiently validated for the Research Topic.
...
1. The Project Operator Session creates the toolbox directory under `skillset/toolboxes/gpu-analytical-modeling`.
2. The Toolbox exposes one or more callback skill directories, each with a `SKILL.md` body that can be used as `--skill-dir` source material for a User Skill Callback.
...
5. The research agent follows the Toolbox instructions within the owning skill's authority, reports conflicts, and records analytical-model assumptions, equations, validation evidence, and unresolved gaps in its normal artifacts.

## Functional Requirements

- The toolbox shall live at `skillset/toolboxes/gpu-analytical-modeling`.
- The Toolbox shall include a README that explains the Toolbox purpose, supported callback skill directories, expected target system skills, suggested registration commands, and topic versus project scope guidance.
...
- The Toolbox shall define at least three callback concerns: model expectation, evidence gate, and source priority.
...
- The Toolbox shall reject pure black-box regression, ungrounded coefficient fitting, vague "memory bound" labels, and roofline-only explanations unless they are explicitly marked as baselines rather than the analytical model.
...
- The Toolbox shall include a suggested callback-target map for DeepSci skills such as `isomer-deepsci-scout`, `isomer-deepsci-analysis`, `isomer-deepsci-experiment`, `isomer-deepsci-review`, `isomer-deepsci-write`, `isomer-deepsci-baseline`, and `isomer-deepsci-science`.
- The Toolbox shall distinguish begin-stage instructions from end-stage instructions: begin callbacks should shape source search, model framing, and experiment plans; end callbacks should check whether tentative outputs include equations, assumptions, evidence, limitations, and unresolved gaps.
- The Toolbox shall document how to register callbacks as topic-scoped for a single Research Topic and project-scoped for general GPU modeling preference.
- The Toolbox shall keep callback bodies free of credentials, API keys, benchmark-private results, and large copied source documents.
- The Toolbox shall provide enough structure for later usecase and interface design to decide whether a manifest file, registration helper, or validation command is needed.

## System Boundaries

- In scope: project-local callback skill material under `skillset/toolboxes/gpu-analytical-modeling`, documentation for registering it with existing User Skill Callback commands, and quality criteria for GPU analytical modeling.
...
- Out of scope: changing packaged DeepSci system skill bodies, changing User Skill Callback storage semantics, adding a Toolbox marketplace, launching agents, running experiments, or writing topic-specific research conclusions.
...
- The Toolbox should not rely on network access at callback resolution time; it should name source classes and preferred repositories rather than requiring live fetches inside the callback.
- The Toolbox should be useful even when a Research Topic lacks all desired evidence by making gaps explicit instead of blocking ordinary progress.
...
- The existing `project skill-callbacks register --skill-dir` flow is sufficient for the first version, so a toolbox can be represented as a curated set of callback skill directories plus documentation.
- The initial users care most about GPU white-box runtime and bottleneck modeling for modern accelerator kernels, especially FlashAttention-style workloads, but the Toolbox should generalize to other CUDA and GPU performance studies.
...
- Should the Toolbox include a small manifest that maps callback directories to target system skills and stages, even if the current CLI does not consume it?
...
- Should architecture-specific profiles, such as Blackwell or B200 guidance, live inside this Toolbox now or in topic-specific callback material later?
- Should the Toolbox include reusable checklists for Nsight Compute counter selection, microbenchmark design, and calibration or validation splits?

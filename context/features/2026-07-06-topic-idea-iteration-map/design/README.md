...
| Topic Graph Read Model | Convert query-index rows and canonical lineage into the `idea-lineage` relationship graph used by Idea Graph and Idea Timeline. |
...
| Idea Timeline Viewer | Render a sortable, filterable table of Research Ideas using short display keys and parent display-key references. |

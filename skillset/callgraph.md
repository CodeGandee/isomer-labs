...
flowchart TD
    ...
    TP["isomer-admin<br/>topic-prepare"]:::skill
    MRS["isomer-admin<br/>manual-research-session"]:::skill
    ...
    C0["C0<br/>prepare-topic<br/>common prep"]:::route
    C5["C5<br/>manual-research<br/>start actor session"]:::route
    C10["C10<br/>topic actor CRUD<br/>and workspace repair"]:::route
...
| --- | --- | --- | --- | --- |
| C0 | `isomer-admin-project-mgr` | `prepare-topic` common prep | `isomer-admin-topic-prepare` | The operator wants reusable Research Topic preparation for manual research or later Topic Team Specialization. The common-prep skill resolves or creates the Research Topic, ensures Topic Workspace registration, validates Workspace Runtime, delegates topic environment setup, reports `topic.repos.main` readiness, and delegates the default `operator` Topic Actor Workspace unless explicitly opted out. |
...
| C5 | `isomer-admin-project-mgr` | `manual-research` start actor session | `isomer-admin-manual-research-session` | The operator wants human-orchestrated research with one or more Topic Actors and no required formal Topic Agent Team. The manual session skill consumes prepared-topic evidence, delegates missing Topic Actor topology to the workspace manager, runs the v2 research bootstrap, and writes authoritative per-actor start-pack records plus actor-local copies or pointers. |
...
| C7 | `isomer-srv-topic-service-agent-support` | bounded Service Request | `isomer-srv-topic-env-setup` | A Topic Service Agent or optional Topic Service Master receives a bounded Service Request for topic-scoped environment readiness checks, Pixi configuration, diagnostics, or support artifacts. If no Topic Service Master is started, the Project Operator Session or Operator Agent performs the same workspace-manager duty directly. Concrete setup or verification still routes through topic env setup when that specialized workflow is required. |
...
| C9 | `isomer-srv-topic-service-agent-support` | bounded Service Request | `isomer-srv-agent-env-setup` | A Topic Service Master receives a bounded request from a topic agent team member for Agent Workspace support that requires topic-owned context, such as generated link or symlink introduction, cwd repair, or per-agent worktree readiness. If no Topic Service Master is started, the Project Operator Session or Operator Agent performs the same workspace-manager duty directly. Concrete Agent Workspace setup or verification still routes through agent env setup when that specialized workflow is required. |
| C10 | `isomer-admin-topic-prepare` or `isomer-admin-manual-research-session` | Topic Actor CRUD and workspace repair | `isomer-admin-topic-workspace-mgr` | Common preparation and manual research sessions need Topic Actor binding registration, update, archive, materialization, repair, actor-scoped path diagnostics, or branch validation. The workspace manager owns these topology mutations through `project topic-actors ...`. |

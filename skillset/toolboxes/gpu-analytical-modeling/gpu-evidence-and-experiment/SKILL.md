...
description: Use when a GPU kernel analytical model needs falsifiable evaluation, profiler or counter evidence, component bottleneck proof, or model-refinement routing.
---
...
This callback skill turns GPU analytical models into testable claims. It focuses experiments and analysis on predicted-versus-measured latency, hard observations for bottleneck predictions, counter trends, saturated hardware components, blocking execution paths, and honest mismatch handling.
...
1. **Check the evaluation contract** before collecting or interpreting results, especially the predicted-versus-measured latency gate. See `commands/evaluation-contract.md`.
2. **Apply the profiler/counter protocol** when profiling or discussing profiler evidence. See `commands/ncu-protocol.md`.
3. **Require component and path proof** when the model predicts saturated units or blocking execution paths, using experiments that stress one predicted component or path when feasible. See `commands/component-bottleneck-proof.md`.
4. **Classify mismatches** when predictions and evidence disagree, or when measured latency or bottleneck observations are missing. See `commands/failure-refinement.md`.
...
- Reporting latency accuracy without predicted latency, measured latency, metric definition, and evidence class in the same claim.
...
- Claiming bottleneck proof without an input or regime designed to stress the predicted component or path.
- Hiding profiler failures behind vague "profiling was infeasible" language.

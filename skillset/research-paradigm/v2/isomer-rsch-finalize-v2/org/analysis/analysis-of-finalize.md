...
```python
@skill(name="finalize", description="Source process for Isomer Research Finalize V2 migration.")
...
| --- | --- | --- |
| `isomer-rsch-finalize-v2` | <FINALIZE_CONTEXT_BRIEF>: Closure context for the Research Topic or Inquiry. | `closure gate` |
| `isomer-rsch-finalize-v2` | <CLAIM_LEDGER>: Claim status, evidence, caveats, and safe-surface decision. | `final summary, writing, archive` |
| `isomer-rsch-finalize-v2` | <FINAL_LIMITATIONS_REPORT>: Final limitations, failures, deferred risks, and unsupported outcomes. | `final summary and user` |
| `isomer-rsch-finalize-v2` | <FINAL_SUMMARY>: Responsible final or pause-state summary. | `user and future work` |
| `isomer-rsch-finalize-v2` | <RESUME_PACKET>: Clean continuation path when later work is plausible. | `future work` |
| `isomer-rsch-finalize-v2 or decision` | <CLOSURE_DECISION>: Durable closure or route-back decision. | `user and future work` |
| `isomer-rsch-finalize-v2` | <FINALIZE_BLOCKER_RECORD>: Why finalization is not responsible yet. | `decision or upstream skill` |
| `isomer-rsch-finalize-v2` | <FINALIZE_CONTINUITY_UPDATE>: Continuity record after finalization. | `future work` |

## Self-Containment Check

- The document defines the important source terms needed to understand the process.
- The process order matches the rewritten v2 skill contract.

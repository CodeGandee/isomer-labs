...
```python
@skill(name="decision", description="Source process for Isomer Research Decision V2 migration.")
...
| --- | --- | --- |
| `isomer-rsch-decision-v2` | <DECISION_CONTEXT_BRIEF>: Decision-ready summary of current state. | `route judgment` |
| `isomer-rsch-decision-v2` | <ROUTE_QUESTION>: The explicit route choice being judged. | `decision evidence packet` |
| `isomer-rsch-decision-v2` | <DECISION_EVIDENCE_PACKET>: Evidence used to select the route. | `route decision record` |
| `isomer-rsch-decision-v2` | <ROUTE_DECISION_RECORD>: Durable verdict, action, reason, evidence, rejected alternatives, and next route. | `any v2 research skill` |
| `isomer-rsch-decision-v2` | <DECISION_CHECKPOINT_MEMORY>: Resume memory for route-changing decisions. | `future work` |
| `isomer-rsch-decision-v2` | <USER_DECISION_REQUEST>: User-facing choice request when local evidence cannot resolve preference, scope, or cost. | `user` |
| `isomer-rsch-decision-v2` | <DECISION_BLOCKER_RECORD>: Why the route cannot be decided responsibly. | `user or continued decision work` |

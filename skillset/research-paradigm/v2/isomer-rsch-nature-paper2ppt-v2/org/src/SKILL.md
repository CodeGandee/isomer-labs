...
This stack must work on macOS, Linux, and Windows. Use `pathlib` paths, project-local output directories, and Office-safe fonts or theme fonts. Do not hardcode OS font paths or platform-specific file locations. If Python packages are missing, stop before deck construction and route a natural-language package request to `$isomer-admin-topic-workspace-mgr install-packages`; do not perform package setup here or install broad document suites just to finish a normal deck.

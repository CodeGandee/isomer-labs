...
Use this reference when v2 research skills need shared vocabulary or route discipline before storage binding exists.
...
5. **Record route meaning**. If the object changes the next route, state the evidence basis, rejected alternatives, blocker if any, and next action.
6. **Return the shared contract**. Give the semantic object id, required content, producer, consumer, and unresolved binding status.
...
- Workspace bootstrap questions route to `isomer-rsch-workspace-mgr-v2`; shared only names the semantic object.

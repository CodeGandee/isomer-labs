...
- A v2 skill needs the latest-context preflight before durable record work trusts prompt context, chat memory, old rendered prose, or remembered research state.
...
2. **Run latest-context preflight when needed**. If the question affects accepted durable record work or current research context, read `references/latest-context-preflight.md` before trusting prompt context, chat memory, old rendered prose, or remembered research state.
3. **Check the semantic registry**. Read `references/semantic-placeholders.md` and choose the closest existing semantic object.
4. **Preserve skill ownership**. Route domain work back to the owning v2 skill; use shared only for vocabulary and coordination consistency. Read `references/coordination-contract.md`.
5. **Route bootstrap questions**. Use `isomer-rsch-workspace-mgr-v2` when the issue is post-preparation placeholder binding, semantic record surfaces, Topic Actor Workspace access posture, formal Agent Workspace access posture, current cwd context, or missing bootstrap support. Read `references/coordination-contract.md`.
6. **Avoid premature storage binding**. Keep unresolved records semantic until an Isomer storage-binding pass assigns Artifact, Evidence Item, Run, Gate, Decision Record, Provenance Record, path, or schema bindings. Read `references/semantic-placeholders.md` and `references/coordination-contract.md`.
7. **Return the coordination result**. State the chosen semantic object, owning producer, intended consumer, and any unresolved binding or route question. Read `references/coordination-contract.md`.
...
Read `references/latest-context-preflight.md` whenever a v2 skill needs current topic context, runtime state, durable record freshness, duplicate-record handling, prompt-versus-durable-context conflict routing, or the storage-neutral `latest-context-snapshot`. Read `references/semantic-placeholders.md` whenever a v2 skill needs shared handoff vocabulary. Read `references/coordination-contract.md` whenever the question is about ownership, routing, unresolved storage binding, post-preparation bootstrap routing, cwd context, or handoff shape.

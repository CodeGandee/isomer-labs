...
- Target skill: `skillset/research-paradigm/v2/isomer-rsch-scout-v2`.
...
| scout stage | `isomer-rsch-scout-v2` as a bounded research-framing skill. |
| baseline next anchor | `isomer-rsch-baseline-v2`, or a baseline route recorded through `<NEXT_ROUTE_DECISION>`. |
| idea next anchor | `isomer-rsch-idea-v2`, or an idea route recorded through `<NEXT_ROUTE_DECISION>`. |
...
- Source `baseline` maps to `isomer-rsch-baseline-v2`.
- Source `idea` maps to `isomer-rsch-idea-v2`.
- Source continued `scout` maps to `isomer-rsch-scout-v2` with `<SCOUT_BLOCKER_RECORD>` or `<NEXT_ROUTE_DECISION>`.

...
6. Produce <RSCH_BOOTSTRAP_VALIDATION_REPORT> when the v2 research loop can start, or <RSCH_WORKSPACE_BLOCKER_RECORD> when a missing input or platform surface would force later skills to guess.
...
Do not imply v2 bootstrap setup will survive Topic Workspace reset unless the selected reset checkpoint has been updated. Unrecorded v2 preparation is valid only as redo-after-reset behavior.

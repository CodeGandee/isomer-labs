...
| source command execution | Execution Adapter Command Request with Research Operation Extension Point, Capability Binding, Skill Binding projection, Gate Policy, Scheduler Policy, workspace, and recording refs |
| source literature lookup | Literature Provider Binding; record context-only provider output first as a provider-output Artifact before deriving Findings or Evidence Items |
...
| package-manager or container tactic | Execution Adapter Command Request implementation detail, not a skill requirement |
| service endpoint | evaluation Capability Binding plus Execution Adapter Command Request surface, not a hard-coded endpoint |
...
| baseline waiver call | Baseline-Waiver Policy ref, with a Gate or Decision Record when waiver requires human-return judgment |
...
| `continuation_policy` | use Scheduler Policy refs for dispatch behavior, and recommend the next Workflow Stage Cursor, Gate, Decision Record, observation, or pause for durable routing |
...
When a source behavior implies execution, literature lookup, storage, lifecycle state, route branching, state mutation, monitoring, publication, chart generation, service launch, or agent launch, describe the intended Isomer record, semantic Artifact kind, workspace scope, Capability Binding, Skill Binding projection, Research Operation Extension Point, Execution Adapter Command Request, Scheduler Policy, Gate Policy, Literature Provider Binding, or Baseline-Waiver Policy. Mark the concrete surface as unsettled only when it is outside accepted Isomer contracts.

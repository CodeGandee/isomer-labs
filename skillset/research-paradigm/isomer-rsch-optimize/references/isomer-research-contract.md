...
| durable line | promoted Research Inquiry direction with a Decision Record |
| implementation attempt | bounded Run, Evidence Item, or implementation-attempt Artifact inside one Research Inquiry direction |
| frontier record | View Manifest or frontier Artifact |
| route promotion | Research Inquiry Relationship decision and Decision Record |
...
When a source behavior implies execution, storage, state mutation, monitoring, or chart generation, describe the intended Isomer record, semantic Artifact kind, workspace scope, or capability. Mark the concrete surface as unsettled only when it is outside accepted Isomer contracts.

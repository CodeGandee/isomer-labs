...
- The task is already a mature section rewrite; use `isomer-rsch-write`.
...
8. **Compile writing plan**. When validation passes, produce `<SECTION_WRITING_PLAN>` for `isomer-rsch-write`, including section jobs, required displays, citation needs, and evidence limits.

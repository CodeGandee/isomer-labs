...
4. Run a minimal smoke test when the package supports it and when cost or safety is acceptable.
...
Use an approved Execution Adapter to run a short check that writes durable JSON or text evidence. The check should capture import status, version, backend, and smoke-test result without relying on conversation recall.
...
- `blocked` means the agent could not check because credentials, data, modules, licenses, network access, privacy approval, queue access, or Operator Agent confirmation are missing.

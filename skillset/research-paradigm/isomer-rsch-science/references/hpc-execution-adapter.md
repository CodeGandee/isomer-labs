...
Isomer science skills do not embed an HPC scheduler. Operate SSH, queue, scheduler, SLURM, remote logs, and file checks through an approved Capability Binding and Execution Adapter.

## Pattern

1. Verify remote access, local scheduler availability, allocation, module, license, credential, and data requirements.
2. Write or identify a small job script, command file, notebook, or solver input Artifact.
3. Submit only through an approved Execution Adapter.
...
- Treat missing modules, allocations, licenses, credentials, SSH access, or private data approval as blockers.
- Open or record a Gate when remote execution may incur cost, privacy risk, credential use, or data export.

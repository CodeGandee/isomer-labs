...
| --- | --- |
| plotting workspace | figure-generation Artifact through Workspace Path Resolution |
...
| script execution | Execution Adapter Command Request with applicable extension, capability, policy, workspace, and recording refs |
...
This skill describes figure-generation judgment. It does not define Isomer ordinary path layouts covered by Workspace Path Resolution, command APIs, notebook runtimes, asset registries, or chart export schemas. When concrete output locations are needed, name the semantic Artifact kind or workspace scope; use TBD placeholders only for unsettled non-path surfaces.

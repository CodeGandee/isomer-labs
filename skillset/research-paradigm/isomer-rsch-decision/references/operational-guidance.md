# Operational Guidance

Use this reference when decision needs longer tactical guidance than the entrypoint.

## Readiness

Do not use decision to paper over state ambiguity. If the active mainline, decisive result, stale-route state, or blocker is unclear, route through intake first.

## Evidence Gathering

Gather only evidence that can change the route. If shell, repository, environment, or git evidence is needed, gather it through a Capability Binding and Execution Adapter. If the concrete command surface must be named but remains unsettled, use `[[tbd-surface:api-execution-command]]`.

## Baseline Reuse

When the route lands on comparator reuse, separate three questions: whether the candidate can be attached as relevant, whether it can be accepted as active, and whether a waiver is needed. If acceptance or waiver rules are unsettled, use `[[tbd-surface:policy-baseline-waiver]]`.

## User Input

Ask the Operator Agent only when multiple options remain plausible and the choice depends on preference, scope, cost, privacy, safety, finality, or missing user-held source. Use a structured Gate with one to three concrete options, tradeoffs, and a recommended option first.

## Paper Routes

For paper-facing decisions, judge method fidelity, evidence support, story coherence, coverage state, and reader value. A draft checkpoint is not final. A review package routes to review or rebuttal. If publishability has collapsed, recommend stop or branch and ask for an Operator Agent Gate before executing a preference-sensitive stop.

## Algorithm-First Routes

When a frontier Artifact exists, treat it as the primary optimize-state summary unless newer durable evidence clearly dominates. Map explore to widening candidate briefs, exploit to advancing the strongest line, fusion to one bounded fusion candidate, and stop to an explicit stop Decision Record with reopen condition.

## Decision Quality

Good decisions are evidence-backed, name tradeoffs, say what happens next, explain rejected alternatives, and avoid launching analysis when information gain is weak. Weak decisions hide uncertainty, lack evidence, approve vaguely, or pretend a blocker is progress.

...
3. If the request is about blank-state topic creation, manual-research-ready setup, or human-orchestrated research, hand off to `isomer-admin-topic-creator plan` or `isomer-admin-topic-creator create` with the selected Project, Research Topic, Topic Workspace, requested setup scope, requested actors, and any explicit opt-out for the default `operator` Topic Actor Workspace.
4. If an advanced operator explicitly requests the deprecated compatibility common-preparation workflow, hand off to `isomer-admin-topic-prepare` with a deprecation warning and the same selected refs.
...
- Do not create Topic Actor bindings here; route normal setup through `isomer-admin-topic-creator` and let it delegate Topic Actor work to `isomer-admin-topic-workspace-mgr`.

...
3. Run `pixi run isomer-cli --print-json runtime prepare --topic <topic-id>` to record Topic Environment Readiness.
4. Run `pixi run isomer-cli --print-json runtime validate --topic <topic-id> --require-ready-readiness` to check launch-facing readiness.

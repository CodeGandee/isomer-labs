...
2. Run `pixi run isomer-cli --print-json project context show --topic <topic-id>` first when the selected topic is unclear.
3. Run `pixi run isomer-cli --print-json project runtime init --topic <topic-id>` once the target topic is explicit.

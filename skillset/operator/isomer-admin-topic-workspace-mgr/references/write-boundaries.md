...
3. Write or update per-agent Agent Workspace boundary material under each prepared `<topic-workspace-dir>/agents/<agent-key>` when the path is ready or explicitly planned.
4. State write ownership for the owning agent key and Peer Read Access expectations for other topic-local agents.

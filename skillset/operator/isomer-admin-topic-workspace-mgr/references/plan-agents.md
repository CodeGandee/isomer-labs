...
1. Require selected Topic Workspace context and a role binding source:
   - Accept Topic Team Instantiation Packet, Topic Agent Team Profile material, or explicit operator-provided role-to-agent-name mapping.
...
5. Resolve per-agent semantic paths:
   - Resolve `agent.workspace` for each Agent Name through Workspace Path Resolution.
   - Resolve required support labels such as `agent.isomer_managed`, `agent.private_artifacts`, `agent.runtime`, `agent.scratch`, `agent.public_share`, and `agent.links`.
   - Record each label's path and source.
6. Plan packet or profile role-binding fields:
   - Plan `role_bindings.<role-id>.agent_name` and `role_bindings.<role-id>.agent_branch`.
   - Derive `role_bindings.<role-id>.agent_workspace_ref` only when the target packet or profile schema still needs compatibility path material.

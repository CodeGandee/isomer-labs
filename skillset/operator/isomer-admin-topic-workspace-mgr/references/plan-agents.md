...
5. Plan each Agent Workspace path as `<topic-workspace-dir>/agents/<agent-name>`, each Isomer-managed support path as `<topic-workspace-dir>/agents/<agent-name>/isomer-managed/`, and each default branch as `per-agent/<agent-name>/main`.
...
Report role ids, agent names, Agent Workspace paths, `isomer-managed/` paths, derived compatibility `agent_workspace_ref` values, default branches, changed packet/profile files, blockers, and `next_operator_action`.

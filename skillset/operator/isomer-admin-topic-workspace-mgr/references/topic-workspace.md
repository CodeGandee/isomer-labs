...
Per-Agent Workspace cwd readiness belongs to `isomer-srv-agent-env-setup`. That service consumes this skill's `topic.repos.main`, `topic.repos.main.tmp`, `agent.workspace`, `agent.tmp`, branch plan, and support-path evidence, then verifies `user-intent/derived/isomer-agent-env-gate.md` from every authoritative Agent Workspace cwd. A selected-agent rerun from that service is partial evidence only and does not make the whole topic team ready.

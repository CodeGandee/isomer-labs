...
3. Run the `manage-actors` workflow when the request involves human-orchestrated Topic Actors, manual research, default `operator` actor readiness, actor-scoped path diagnostics, or `project topic-actors ...`:
   - List or inspect current Topic Actor bindings.
   - Register, update, archive, materialize, repair, or diagnose Topic Actors only when requested.
   - Resolve `topic.actors.workspace`, `topic.actors.tmp`, `topic.actors.isomer_managed`, `topic.actors.private_artifacts`, `topic.actors.logs`, and `topic.actors.links`.
   - Use `per-topic-actor/<topic-actor-name>/main` branches from `topic.repos.main`; reject alternate worktree sources in this change.
4. Run the `plan-agents` workflow:
...
5. Run the `create-worktrees` workflow only when explicit manual worktree support was requested:
...
6. Run the `write-boundaries` workflow for topic-level, per-Topic Actor, and per-agent advisory Workspace Boundary, Peer Read Access, owner/reader split, and generated-link notes.
7. Run the `validate-worktrees` workflow to check Git topology, branch namespace, duplicate checkout state, local tmp posture, `isomer-managed/` layout, generated links, actor bindings, and packet/profile refs.
8. Handle per-Agent Workspace environment readiness only when requested:
...
9. Run the `summarize` workflow and report Essential Output by default, with blockers and next operator action; use Complete Output when requested.
...
The full flow provides Topic Actor topology management plus optional topology inspection and support. It does not create topic-main in the normal flow, create Agent Instances, launch Houmao agents, run Execution Adapters, or prove that every Agent Workspace cwd can run a gate command. It may write Workspace Runtime audit/provenance records for Topic Actor registration or materialization when runtime is available, while keeping the Topic Workspace Manifest as topology authority.

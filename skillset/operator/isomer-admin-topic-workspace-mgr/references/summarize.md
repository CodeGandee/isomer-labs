...
3. Summarize each agent key, role id, Agent Workspace path, current branch, expected branch namespace, and `agent_workspace_ref`.
...
Report `research_topic_ref`, `topic_workspace_ref`, `topic_main_repo_path`, `agent_workspace_paths`, `agent_workspace_refs`, `branch_plan`, `boundary_material_paths`, `validation_status`, `blockers`, and `next_operator_action`.

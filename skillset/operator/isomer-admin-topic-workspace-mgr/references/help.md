...
4. Name the required inputs: Project Manifest context, Research Topic, Topic Workspace, optional Topic Actor names, optional packet/profile material, any requested agent-name mapping, and a package request prompt or description file for `install-packages`.
5. State the output contract:
   ...
   - Include optional package install plan and verification evidence, optional `isomer-srv-agent-env-setup` evidence when already available, validation status, blockers, and next operator action.
6. State the key guardrails:
   ...
   - No local virtualenv, ambient `pip`, unrecorded user R libraries, `sudo`, or system package manager mutation for package installation; use the selected Topic Workspace Pixi environment or report a blocker.
...
| `install-packages` | Infer, install, and verify packages in the selected Topic Workspace Pixi environment from a prompt or description file. | Package install plan, Pixi commands, verification evidence, changed files, and blockers. |
...
For user-facing or research-skill package-add requests, use this skill's `install-packages` subcommand. Full gate-driven environment setup remains with `isomer-srv-topic-env-setup`, but ad hoc package additions should enter through the workspace manager so the selected Topic Workspace Pixi target, install route, and verification evidence are explicit.

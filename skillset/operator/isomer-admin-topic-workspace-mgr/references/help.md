...
4. Name the required inputs: Project Manifest context, Research Topic, Topic Workspace, optional packet/profile material, and any requested agent-key mapping.
5. State the output contract: topic repo path, agent workspace paths, branch plan, `agent_workspace_ref` values, boundary docs, validation status, blockers, and next operator action.
...
| `plan-agents` | Normalize agent keys and map active role bindings to worktree paths and branches. | Agent key map, `agent_workspace_ref` plan, `per-agent/<agent-key>/main` branch plan. |
...
| `create-agent-branch` | Create a future per-agent branch under the owning agent prefix. | `per-agent/<agent-key>/<branch-name>` branch result, base ref, blockers. |

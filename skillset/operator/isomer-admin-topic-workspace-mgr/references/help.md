...
5. State the output contract:
   - Include semantic paths with labels and sources, topic repo path, `isomer-managed/` regime status, projection roots, local tmp posture, records root, runtime root, agent workspace paths, branch plan, derived compatibility `agent_workspace_ref` values, boundary docs, and generated links.
...
This skill complements `isomer-admin-topic-team-specialize`: specialization defines topic-team material and static setup evidence, while this skill provides optional topology inspection, branch helpers, boundary summaries, manual compatibility operations, and legacy diagnostics.

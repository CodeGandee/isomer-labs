...
1. Require successful `resolve-workspace`, usable `ensure-main-repo` output, and a `plan-agents` result with role ids, agent names, resolved semantic paths, path sources, branches, and `agent.tmp` posture when available.
...
6. Create missing branches and worktrees only for safe planned entries, using `topic.repos.main` as the Git anchor and the resolved `agent.workspace` path as the Agent Workspace path.
...
8. Prepare or validate `agent.tmp` as a local ignored disposable path under the resolved `agent.workspace`, using the Topic Main Repository root ignore policy because Agent Workspaces are Git worktrees.

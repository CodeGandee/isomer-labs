...
1. Require successful `resolve-workspace`, usable `ensure-main-repo` output, and a `plan-agents` result with role ids, agent names, paths, and branches.
2. For each planned agent name, inspect existing Git worktrees for `<topic-workspace-dir>/repos/topic-main`.
3. If `<topic-workspace-dir>/agents/<agent-name>` already exists as a worktree of `topic-main` on `per-agent/<agent-name>/main`, report it as ready.
...
6. Create missing branches and worktrees only for safe planned entries, using `topic-main` as the Git anchor and `<topic-workspace-dir>/agents/<agent-name>` as the Agent Workspace path.
7. Ensure each prepared worktree has the standard `isomer-managed/` support layout: `.gitignore`, `tracked/{shared,artifacts,tasks,runs,views,tools,boundaries,manifests}/`, `agent-owned/{runtime,scratch,logs,artifacts,public,inbox}/`, `topic-owned/{readonly,writable}/`, and `links/`, or report why it could not be prepared.

...
1. Require successful `resolve-workspace` output, including selected Topic Workspace path and resolved `topic.main_repo` plus `topic.main_repo.isomer_managed` labels with sources.
...
3. If the path is missing and the operator requested creation, create the resolved parent directory, initialize the repository, make the owner-managed branch `topic-owner/main` available, and prepare or validate `topic.main_repo.isomer_managed` plus tracked sublabels without hiding command failures.

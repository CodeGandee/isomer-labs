...
3. If the path is missing and the operator requested creation, create parent `repos/`, initialize `topic-main`, make the owner-managed branch `topic-owner/main` available, and prepare or validate `isomer-managed/.gitignore` plus `isomer-managed/tracked/{shared,artifacts,tasks,runs,views,tools,boundaries,manifests}/` without hiding command failures.
...
The only standard Isomer-specific worker-facing namespace at the repository top level is `isomer-managed/`. Its tracked regime is `isomer-managed/tracked/`; do not create top-level Isomer collaboration directories in `topic-main`.

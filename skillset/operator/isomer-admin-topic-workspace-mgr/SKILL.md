...
- `next_operator_action`: usually rerun a specific subcommand, update packet/profile material, make a caller-requested `isomer-srv-agent-env-setup` call after validated topology evidence exists, run `isomer-admin-topic-team-specialize validate-topic-team`, create an Agent Team Instance through runtime workflow, or stop on blockers.

...
- `semantic_paths`: resolved labels, paths, sources, readiness, and blockers for `topic.main_repo`, `topic.main_repo.tmp`, `topic.main_repo.isomer_managed`, `topic.agents_root`, `topic.records`, `topic.runtime`, `agent.workspace`, `agent.tmp`, and required agent support labels.
...
- `local_tmp_path_status`: readiness for `topic.main_repo.tmp` and `agent.tmp`, including resolved paths, ignored posture, tracked-content diagnostics, and confirmation that tmp material is not shared or durable evidence.

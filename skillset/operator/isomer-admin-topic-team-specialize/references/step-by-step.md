...
2. Prepare a progress tracker with these steps: `init-topic`, `clarify-topic` when needed, `ensure-topic-registration`, `specialize-team`, `clarify-topic-team` when needed, `setup-topic-env`, `setup-agent-workspace`, `validate-topic-team`, `finalize-topic-team`, and optional later approval or materialization boundaries. If the user has not supplied concrete Research Topic substance, ask for it before confirming or running `init-topic`.
3. Before each step, tell the user what will happen, what inputs or files may be read or changed, and what output should exist after the step. For `ensure-topic-registration`, explain that it may run supported Isomer Project registration commands and will block instead of hand-editing Project Config or inventing missing bindings. For `setup-topic-env`, explain that the operator step prepares or checks `env-gate.md`, then delegates heavy setup to `$isomer-srv-env-setup setup-for-topic-workspace <research_topic_id> manual`.

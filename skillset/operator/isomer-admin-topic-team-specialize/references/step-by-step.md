...
3. Before each step, tell the user what will happen, what inputs or files may be read or changed, and what output should exist after the step. For `ensure-topic-registration`, explain that it may run supported Isomer Project registration commands and will block instead of hand-editing Project Config or inventing missing bindings. For `setup-topic-env`, explain that the operator step only needs registration, a Pixi binding, and an `env-gate.md` or clear runnable target; it then delegates heavy setup to `$isomer-srv-topic-env-setup setup-topic-env <research_topic_id> manual`. For `setup-agent-workspace`, explain that it first records or delegates Git-backed topology evidence, then prepares `<topic-workspace>/user-intent/src/agent-env-gate.md` from the user task if per-Agent Workspace cwd proof is requested and the file is missing, and only then delegates readiness proof to `$isomer-srv-agent-env-setup setup-agent-env <research_topic_id>`.

...
| `setup-agent-workspace` | Create or report per-agent Agent Workspace directories and boundaries, delegating Git-backed worktree setup to `isomer-admin-topic-workspace-mgr` when requested. | `semantic_paths`, `agent_workspace_paths`, `agent_workspace_ref` evidence, ownership notes, workspace blockers, validation refs. |

...
4. Print the user-facing flow: `init-topic`, optional `clarify-topic`, `ensure-topic-registration`, optional independent `setup-topic-env` when `env-gate.md` is available or a runnable target is clear, `specialize-team`, optional `clarify-topic-team`, optional repeated `setup-topic-env` when specialization changes runnable requirements, `setup-agent-workspace`, `validate-topic-team`, `finalize-topic-team`, then explicit `approve-profile` or `materialize-profile`.
...
6. Name the required inputs and outputs: Research Topic, topic workspace directory, Project Manifest registration evidence, Topic Workspace Pixi binding status, semantic path evidence, Domain Agent Team Template, `topic-overview.md`, copied template material, `team-specialization-guide.md`, `team-specialization-plan.md`, `Final Report`, `env-gate.md`, `isomer-srv-topic-env-setup` service output, environment setup status, Agent Workspace paths, delegated Git-backed workspace evidence from `isomer-admin-topic-workspace-mgr` when requested, delegated agent environment service output from `isomer-srv-agent-env-setup` when per-agent cwd proof is requested, validation status, `isomer-topic-summary.md`, and next operator action.
...
| `setup-topic-env` | Prepare the `env-gate.md` handoff and delegate heavy Topic Workspace environment setup to `isomer-srv-topic-env-setup`. | `topic_environment_status`, `env_gate_path`, `derived_gate_path`, service setup commands, changed files, validation refs, setup blockers. |
| `setup-agent-workspace` | Create or report per-agent Agent Workspace directories and boundaries, delegating Git-backed worktree setup to `isomer-admin-topic-workspace-mgr` and per-agent cwd proof to `isomer-srv-agent-env-setup` when requested. | `semantic_paths`, `agent_workspace_paths`, `agent_workspace_ref` evidence, `agent_environment_service_output`, ownership notes, workspace blockers, validation refs. |
...
Use `setup-topic-env` after `ensure-topic-registration` whenever `<topic-workspace>/user-intent/src/env-gate.md` exists or the user provides a clear runnable target. It prepares or checks the source gate, delegates repo, dependency, Pixi, and verification work to `isomer-srv-topic-env-setup`, and records the service output as durable Topic Workspace preparation. It can run before team specialization, and it can run again after specialization if copied material adds runnable requirements. Use `setup-agent-workspace`, `validate-topic-team`, and `finalize-topic-team` after specialization to prepare durable topic-team setup state and write `isomer-topic-summary.md`. If the user wants Git-backed `topic.repos.main` plus per-agent `agent.workspace` worktrees, `setup-agent-workspace` delegates that setup to `isomer-admin-topic-workspace-mgr` and records its static semantic label output as evidence. If the user also needs `agent-env-gate.md` or proof that commands pass from each Agent Workspace cwd, `setup-agent-workspace` delegates that gate-driven proof to `isomer-srv-agent-env-setup` and records readiness by Agent Name. Live team operation is outside this skill's scope.

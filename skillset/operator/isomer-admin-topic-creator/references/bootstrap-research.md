...
1. Require registered Topic Workspace, valid Workspace Runtime, `topic.intent.overview`, verified or fast-forward accepted `topic.intent.topic_env_requirements`, topic environment readiness, ready `topic.repos.main`, `topic.intent.actor_definitions`, `topic.env.actor_env_gates`, actor cwd verification evidence, materialized research record labels, and selected Topic Actor bindings.
...
Do not write accepted research artifacts only to local scratch, `topic.repos.main`, Topic Actor Workspace files, or Agent Workspace files without recording or linking them through Topic Workspace research records.

...
| `isomer-admin-topic-workspace-mgr` | Prepare and validate the Git-backed Topic Workspace collaboration layout. It owns `<topic-workspace-dir>/repos/topic-main`, per-agent worktrees under `<topic-workspace-dir>/agents/<agent-name>`, `topic-owner/main`, `per-agent/<agent-name>/main` branch planning, future `per-agent/<agent-name>/<branch-name>` branches, `agent_name`, `agent_branch`, derived compatibility `agent_workspace_ref` planning or validation, `.isomer-agent/` support dirs, `records/*` visibility diagnostics, advisory Workspace Boundary notes, and summary output. It does not create Agent Instances, mutate Workspace Runtime, launch Houmao agents, or replace `isomer-srv-topic-env-setup` environment setup. |

...
This subtree contains skills intended for Project Operator Sessions and Operator Agents. Operator skills use the `isomer-admin-<purpose>` naming convention because they operate project control surfaces: project discovery, common topic preparation, human-orchestrated Topic Actor research, Topic Team Specialization orchestration, approval provenance, profile materialization, and team launch orchestration.
...
| Skill | Purpose |
| --- | --- |
| `isomer-admin-project-mgr` | Run the operator Project lifecycle workflow. It exposes short local subcommands such as `help`, `init-project`, `check-project`, `list-topics`, `show-context`, `init-runtime`, `prep-runtime`, `prepare-topic`, `manual-research`, and `specialize-team`; it initializes `.isomer-labs/` and the Isomer-managed `.isomer-labs/.houmao/` overlay through `isomer-cli project init`, checks Project health, resolves context, prepares runtime, routes common topic preparation to `isomer-admin-topic-prepare`, routes human-orchestrated research to `isomer-admin-manual-research-session`, and hands full topic-team specialization to `isomer-admin-topic-team-specialize fast-forward`. |
| `isomer-admin-topic-prepare` | Prepare reusable Research Topic state before manual research or team specialization. It resolves or creates a Research Topic, ensures Topic Workspace registration, validates Workspace Runtime, delegates topic environment setup, reports `topic.repos.main` as the Git anchor and integration surface, preserves existing team material, and delegates the default reserved `operator` Topic Actor Workspace to `isomer-admin-topic-workspace-mgr` unless the user explicitly opts out. |
| `isomer-admin-manual-research-session` | Start or repair a human-orchestrated research session with one or more Topic Actors and no required formal Topic Agent Team. It consumes prepared-topic evidence, delegates missing Topic Actor bindings and Topic Actor Workspaces to `isomer-admin-topic-workspace-mgr`, runs `isomer-rsch-workspace-mgr-v2` over base topic, Topic Actor, and optional formal team layers, and writes authoritative per-actor start-pack records plus actor-local copies or pointers. |
...
6. When the user asks to prepare a Research Topic for manual or future research work, call `prepare-topic`; it hands off to `isomer-admin-topic-prepare` and creates or reuses the default `operator` Topic Actor Workspace unless the user explicitly opts out.
7. When the user asks to start manual research or several manually controlled coding agents, call `manual-research`; it routes actor topology through `isomer-admin-topic-workspace-mgr` and session start packs through `isomer-admin-manual-research-session`.
...
1. Call `isomer-admin-project-mgr prepare-topic` for the selected Research Topic. It resolves or creates the topic, ensures Topic Workspace registration, validates Workspace Runtime, delegates topic environment setup, reports `topic.repos.main` readiness, and delegates the reserved `operator` Topic Actor Workspace unless the user explicitly opted out.
2. Call `isomer-admin-project-mgr manual-research` with the requested actor names and roles, such as `operator`, `claude-scout`, `codex-exp-a`, or `houmao-writer-a`.
3. The manual session workflow delegates actor registration, materialization, repair, and diagnostics to `isomer-admin-topic-workspace-mgr` through `project topic-actors ...`.
4. The manual session workflow runs `isomer-rsch-workspace-mgr-v2` so v2 skills have base topic readiness, Topic Actor readiness, optional formal team readiness, and placeholder-binding guidance.
5. The operator gives each manually controlled worker its own `topic.actors.workspace` cwd and a start-pack record. `topic.repos.main` remains the Git anchor and integration surface, not the required shared cwd for every worker.
...
`isomer-admin-project-mgr` is the canonical entrypoint for Project lifecycle operations before topic work. It owns Project bootstrap, Project checks, topic and workspace listing, context inspection, runtime initialization, readiness preparation, and routing to common topic preparation, manual research sessions, or formal topic-team adaptation.

`isomer-admin-topic-prepare` is the reusable common topic-preparation boundary. It does not specialize a team or start research; it produces the topic state that both manual Topic Actor research and Topic Team Specialization can consume.

`isomer-admin-manual-research-session` is the human-orchestrated Topic Actor research boundary. It does not own actor topology mutation, does not create formal team material, and does not fabricate Agent Instance refs.

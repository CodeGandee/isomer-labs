...
sequenceDiagram
    ...
    Operator->>Service: route-service when support is needed
...
3. The local `route-service` subcommand dispatches preparation work when a Topic Service Agent must prepare environments, check Agent Workspace readiness, or collect diagnostics before launch.
4. The local `launch-team` subcommand creates or selects the Agent Team Instance, preserves profile bundle and packet provenance, and routes adapter launch materialization.
...
sequenceDiagram
    ...
    alt Service preparation needed
        Operator->>Service: route-service

...
| `isomer-admin-topic-team-specialize` | Run the module-level Topic Team Specialization workflow. It exposes procedural subcommands from `init-topic` through `finalize-topic-team`, five helper subcommands for lower-level specialization work, and misc commands such as `help`, `step-by-step`, and `fast-forward`; it creates `topic.intent.overview`, resolves topic and agent environment intent, copies and adapts Domain Agent Team Template material under `<topic-workspace>/team-profile/`, orchestrates Topic Workspace predecessor setup, Git-backed Agent Workspace topology, and per-agent environment proof when requested, validates readiness, writes `isomer-topic-summary.md`, and keeps approval, materialization, and launch as explicit boundaries. |
| `isomer-admin-topic-workspace-mgr` | Prepare and validate the Git-backed Topic Workspace collaboration layout. It owns `<topic-workspace-dir>/repos/topic-main`, `isomer-managed/` tracked, agent-owned, topic-owned, and generated-link regimes, per-agent worktrees under `<topic-workspace-dir>/agents/<agent-name>`, `topic-owner/main`, `per-agent/<agent-name>/main` branch planning, future `per-agent/<agent-name>/<branch-name>` branches, `agent_name`, `agent_branch`, derived compatibility `agent_workspace_ref` planning or validation, `records/*` visibility diagnostics, advisory Workspace Boundary notes, and summary output. It does not create Agent Instances, mutate Workspace Runtime, launch Houmao agents, replace `isomer-srv-topic-env-setup` environment setup, or claim per-Agent Workspace environment readiness. |
...
3. The normal procedural flow is `resolve-project`, `resolve-topic-intent`, `ensure-topic-registration`, `resolve-topic-env-gate`, `setup-topic-env`, `specialize-team`, optional `clarify-topic-team`, `resolve-agent-env-gate`, `setup-agent-workspace`, `validate-topic-team`, and `finalize-topic-team`. `finalize-topic-team` writes `isomer-topic-summary.md` with the topic team, goal, working logic, environment setup, Agent Workspace layout, validation status, blockers, and next actions.
...
For Topic Workspace environment setup, `setup-topic-env` delegates to `isomer-srv-topic-env-setup` with `topic.intent.topic_env_requirements` or an explicit topic env target spec and records Topic Workspace predecessor evidence, not per-agent cwd proof. For Git-backed Agent Workspace setup, call `isomer-admin-topic-workspace-mgr` from the `setup-agent-workspace` stage. `isomer-admin-topic-team-specialize` remains responsible for static topic-team evidence and final summaries; the workspace manager handles the concrete `repos/topic-main` repository, `isomer-managed/` layout, Git worktrees, branch namespace, generated links, and `agent_workspace_ref` validation evidence. If the caller also requests per-Agent Workspace cwd proof, `setup-agent-workspace` resolves `topic.intent.agent_env_requirements` or receives an explicit agent env target spec and delegates to `isomer-srv-agent-env-setup` only after `topic.env.topic_setup_target_spec`, authoritative Agent Names, and Git topology evidence exist.
...
sequenceDiagram
    ...
    Operator->>Store: setup-topic-env for Topic Workspace predecessor evidence
...
`isomer-admin-topic-workspace-mgr` is the canonical entrypoint for Git-backed Topic Workspace repository and Agent Workspace worktree preparation. It may be called from Topic Team Specialization setup, but it stays out of Workspace Runtime mutation and live launch.

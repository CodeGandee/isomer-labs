...
| `isomer-admin-topic-creator` | Create or resume a Research Topic from empty or partial Project state to a prepared Topic Workspace with a readiness summary. It exposes commands such as `help`, `fast-forward`, `step-by-step`, `run-to`, `ensure-project`, `resolve-topic-input`, `register-topic`, `init-runtime`, `create-research-intent`, `define-topic-env`, `setup-topic-env`, `define-actors`, `setup-actors`, `finalize`, `status`, and `repair`; it delegates Project lifecycle to `isomer-admin-project-mgr`, topic environment setup to `isomer-srv-topic-env-setup`, and Topic Actor topology to `isomer-admin-topic-workspace-mgr`. |

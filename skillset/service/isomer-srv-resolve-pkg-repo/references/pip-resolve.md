...
3. **If PyPI is not reachable**, inspect local pip configuration. Check `PIP_INDEX_URL`, `PIP_EXTRA_INDEX_URL`, `~/.config/pip/pip.conf`, `~/.pip/pip.conf`, and `pip.ini` on Windows-style paths.

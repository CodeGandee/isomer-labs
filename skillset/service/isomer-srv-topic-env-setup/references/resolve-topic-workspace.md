...
4. **Resolve setup semantic paths** for the selected Topic Workspace. Required read-only labels are `topic.workspace`, `topic.main_repo`, `topic.records`, and `topic.runtime`; report each path source and any manifest diagnostics.

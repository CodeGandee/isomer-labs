...
Use this subcommand to ensure independent repos required by the source gate exist under the resolved topic repository root.
...
| `repos_root` | Use the resolved topic repository root from semantic path evidence; under `isomer-default.v1` this is normally `<topic-workspace-dir>/repos/` for independent setup repos. Create it only when this step needs to materialize or inspect required repos. |

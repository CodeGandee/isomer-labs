...
5. **Route missing or stale dependency readiness** back to `isomer-srv-topic-env-setup` instead of mutating dependencies in this service.
...
- `ready`: the resolved manifest path exists, `pixi.lock` exists, `.pixi/` exists, `user-intent/derived/isomer-env-gate.md` exists, and topic env predecessor evidence is not blocked.
...
- Report next action as `isomer-srv-topic-env-setup` when dependency readiness is missing or stale.

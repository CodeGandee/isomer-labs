...
4. **Resolve topic semantic labels before mutation**. Required labels are `topic.main_repo`, `topic.main_repo.isomer_managed`, `topic.agents_root`, `topic.records`, and `topic.runtime`. Record each path source, diagnostic, and blocker.
...
| `semantic_paths` | Resolved labels, paths, sources, diagnostics, and blockers for `topic.main_repo`, `topic.main_repo.isomer_managed`, `topic.agents_root`, `topic.records`, and `topic.runtime`. |

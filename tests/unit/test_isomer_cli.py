...
class IsomerCliTests(unittest.TestCase):
    ...
    def test_system_skills_cli_lists_and_projects_packaged_skills(self) -> None:
        ...
        self.assertEqual(
            ...
            [(public["name"], public["role"]) for public in core_pack["public_skills"]],
        )
        ...
        self.assertEqual("isomer-labs-skill-manifest.v5", installed["manifest"]["schema_version"])
        ...
        self.assertEqual(
            ["isomer-op-welcome", "isomer-op-entrypoint"],
            [item["name"] for item in preserved["preserved_existing"]],
        )
        ...
        self.assertFalse((skill_root / "isomer-op-welcome").exists())

    def test_system_skills_install_defaults_to_exact_project_scope(self) -> None:
        project_root = self.make_root()
        (project_root / ".git").mkdir()
        write(
            project_root / ".isomer-labs" / "manifest.toml",
            """
            schema_version = "isomer-project-manifest.v1"
            project_id = "ancestor-project"
            """,
        )
        nested = project_root / "packages" / "service"
        nested.mkdir(parents=True)
        user_home = project_root / "user-home"
        ...
        self.assertEqual(0, status, output)
        installed = json.loads(output)
        expected_skill_root = nested / ".agents" / "skills"
        self.assertEqual("project", installed["scope"])
        self.assertEqual(str(expected_skill_root), installed["skill_root"])
        self.assertEqual([{"target": "generic", "scope": "project"}], installed["bindings"])
        self.assertEqual(["isomer-op-entrypoint"], installed["manifest"]["skill_names"])
        self.assertTrue((expected_skill_root / "isomer-op-entrypoint" / "SKILL.md").is_file())
        self.assertTrue((expected_skill_root / "isomer-op-welcome" / "SKILL.md").is_file())
    ...
    def test_system_skill_extension_discovery_explains_kaoju_entry_surface(self) -> None:
        ...
        self.assertEqual("$isomer-ext-kaoju-welcome", kaoju["welcome_invocation"])
        ...
        self.assertEqual(["isomer-ext-kaoju-welcome", "isomer-ext-kaoju-entrypoint"], extension["skills"])
        self.assertEqual(
            ["welcome", "entrypoint"],
            [public["role"] for public in extension["public_skills"]],
        )
        self.assertIn(
    ...
    def test_system_skill_extension_help_distinguishes_agent_and_runtime_extensions(self) -> None:
        ...
        self.assertIn("--scope [user|project]", install_help.output)
        self.assertIn("[default: project]", install_help.output)
        self.assertNotIn("--home", install_help.output)
    ...
    def test_system_skills_cli_force_and_upgrade_refresh_manifest_tracked_skills(self) -> None:
        ...
        self.assertEqual(
            ...
            upgraded["stale_removed_skills"],
        )
        self.assertTrue((skill_root / "isomer-op-entrypoint" / "SKILL.md").is_file())
        self.assertFalse((skill_root / "isomer-ext-kaoju-entrypoint").exists())
        self.assertFalse((skill_root / "isomer-ext-kaoju-welcome").exists())
        ...
        self.assertEqual(["isomer-op-welcome", "isomer-op-entrypoint"], forced["replaced_skills"])
        self.assertNotEqual("local edit\n", (skill_root / "isomer-op-entrypoint" / "SKILL.md").read_text(encoding="utf-8"))

    def test_system_skills_cli_deduplicates_all_target_and_rejects_home(self) -> None:
        root = self.make_root()
        status, output = self.run_cli(
            [
                "--print-json",
        ...
        self.assertEqual(
            [
                ...
                [{"target": "codex", "scope": "project"}, {"target": "generic", "scope": "project"}],
                [{"target": "kimi-code", "scope": "project"}],
            ],
            [item["bindings"] for item in data["targets"]],
        )
        ...
        explicit = json.loads(output)
        self.assertEqual(
            [(item["scope"], item["skill_root"], item["bindings"]) for item in data["targets"]],
            [(item["scope"], item["skill_root"], item["bindings"]) for item in explicit["targets"]],
        )

        rejected_root = self.make_root()
        status, stdout, stderr = self.run_main(
            [
                "--print-json",
                "system-skills",
        ...
        self.assertIn("No such option '--home'", error["diagnostics"][0]["message"])
        self.assertFalse((rejected_root / ".agents").exists())

    def test_system_skills_scope_requirements_are_command_specific(self) -> None:
        install_root = self.make_root()
        status, stdout, stderr = self.run_main(
            ["--print-json", "system-skills", "install", "--skill", "isomer-op-entrypoint"],
            cwd=install_root,
        )
        self.assertEqual(2, status)
        self.assertEqual("", stderr)
        missing_target = json.loads(stdout)
        self.assertFalse(missing_target["ok"])
        self.assertIn("Missing option '--target'", missing_target["diagnostics"][0]["message"])
        self.assertFalse((install_root / ".agents").exists())

        for command in ("status", "upgrade", "uninstall"):
            with self.subTest(command=command):
                root = self.make_root()
                status, stdout, stderr = self.run_main(
                    ["--print-json", "system-skills", command, "--target", "generic"],
                    cwd=root,
                )
                self.assertEqual(2, status)
                self.assertEqual("", stderr)
                missing_scope = json.loads(stdout)
                self.assertFalse(missing_scope["ok"])
                self.assertIn("Missing option '--scope'", missing_scope["diagnostics"][0]["message"])
                self.assertFalse((root / ".agents").exists())

    def test_system_skills_cli_resolves_and_renders_user_scope(self) -> None:
        root = self.make_root()
        kimi_home = root / "kimi-home"
        env = {"HOME": str(root / "user-home"), "KIMI_CODE_HOME": str(kimi_home)}
        ...
        self.assertEqual(0, status, output)
        installed = json.loads(output)
        self.assertEqual("user", installed["scope"])
        self.assertEqual(str(kimi_home / "skills"), installed["skill_root"])
        self.assertTrue((kimi_home / "skills" / "isomer-op-entrypoint" / "SKILL.md").is_file())
        self.assertFalse((root / ".kimi-code").exists())
        ...
        self.assertEqual(0, status, output)
        self.assertIn(f"- kimi-code/user: {kimi_home / 'skills'}", output)
    ...
    def test_system_extensions_cli_remembers_and_forgets_project_declarations(self) -> None:
        ...
        self.assertEqual(
            [("isomer-ext-deepsci-welcome", "welcome"), ("isomer-ext-deepsci-entrypoint", "entrypoint")],
    ...
    def test_system_extension_detection_uses_only_explicit_roots_and_init_does_not_scan(self) -> None:
        root = self.make_root()
        status, output = self.run_cli(
            [
                "system-skills",
        ...
        data = json.loads(output)
        self.assertEqual(0, status, output)
        self.assertEqual("not_performed_by_direct_cli", data["system_extension_reconciliation"])
        self.assertNotIn("system_extension_observations", data)
        manifest_path = root / ".isomer-labs" / "manifest.toml"
        manifest_before = manifest_path.read_text(encoding="utf-8")
        self.assertNotIn("operator.system_extensions", manifest_before)

        status, output = self.run_cli(
            ["project", "system-extensions", "detect", "--json"],
            cwd=root,
        )
        no_roots = json.loads(output)
        self.assertEqual(0, status, output)
        self.assertEqual([], no_roots["skill_roots"])
        self.assertEqual([], no_roots["observations"])
        self.assertEqual(["deepsci", "kaoju"], [item["extension_id"] for item in no_roots["extensions"]])
        ...
        self.assertEqual([str(root / ".agents" / "skills")], detected["skill_roots"])
        kaoju = next(item for item in detected["observations"][0]["extensions"] if item["extension_id"] == "kaoju")
        self.assertEqual("complete", kaoju["coverage_status"])
        self.assertEqual("current", kaoju["version_status"])
        self.assertFalse(kaoju["declared_installed"])
        ...
        self.assertEqual(0, status, output)
        kaoju = next(item for item in partial["observations"][0]["extensions"] if item["extension_id"] == "kaoju")
    ...
    def test_skill_callback_insertion_points_query_catalog_and_project_declarations(self) -> None:
        ...
        self.assertEqual("protected", point["public_role"])

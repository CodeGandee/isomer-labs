...
Full execution checklist for the experiment route, beyond the short control surface in the
main `SKILL.md` Workflow. Map any legacy tool names (`artifact.*`, `bash_exec`) to the
`$HARNESS` surface for your role.
...
Prefer to write this contract first in `PLAN.md` using the main-experiment plan template, then keep the current execution state visible in `CHECKLIST.md` using the main-experiment checklist template.
...
- if `Result/metric.md` was used during the run, treat it as optional scratch memory only and reconcile it against the final submitted metrics before the durable main-experiment record

Before you begin a substantial run, send a concise threaded progress update naming:
...
The normal experiment workspace is the current active idea worktree returned when the idea was submitted.
...
- if the idea package changes materially before execution, submit a new durable idea branch (lineage intent `continue_line` or `branch_alternative`) instead of silently mutating the old node
- after a real main run finishes, record it with the durable main-experiment record before moving to analysis or writing
...
- after the main-experiment record, if QQ milestone media is enabled and the metrics are stable enough to summarize honestly, prefer one concise summary PNG over multiple attachments
...
- prefer managed shell execution instead of ephemeral shell invocations
...
- once the path is verified, launch the real run detached and normally leave the timeout unset for that long run
- monitor through durable logs rather than only live terminal output
- a managed-log read returns the full rendered log when it is 2000 lines or fewer; for longer logs it returns the first 500 lines plus the last 1500 lines and a hint to inspect omitted sections with `start` and `tail`
- if the middle of a long saved log matters, inspect that omitted region with an explicit `start`/`tail` read
- list managed commands and read newest-first to monitor or revisit them while focusing on the newest evidence first
- after the first read, prefer an incremental read keyed on the last seen sequence so later checks only fetch new evidence
- if you need to recover ids or sanity-check the active session ordering, use the session history
- launch important runs with a structured `comment` such as `{stage, goal, action, expected_signal, next_check}`
- use `silent_seconds`, `progress_age_seconds`, `signal_age_seconds`, and `watchdog_overdue` as your default watchdog signals
...
- if needed, use an explicit bounded wait between checks
- canonical sleep choice:
  - if you only need wall-clock waiting between checks, use a bounded sleep-await
  - keep a real buffer on that sleep timeout; do not set the timeout exactly equal to the sleep length
  - if you are waiting on an already running managed session, prefer awaiting that session instead of starting a new sleep command
...
- after every completed sleep or await cycle, inspect logs first; only send a progress update when the user-visible state, frontier, blocker status, or ETA materially changed
...
- if the run is clearly invalid, wedged, or superseded, kill it; if it must die immediately, force it, record the reason, fix the issue, and relaunch cleanly
...
Always preserve the managed execution log and export it into the experiment artifact directory when the run artifact is written.
...
Every meaningful main run must be recorded through the durable main-experiment record (legacy: `artifact.record_main_experiment(...)`).

That record is responsible for writing:
...
The durable main-experiment record should include at least:
...
After the main-experiment record succeeds, do not assume the same branch should absorb the next round by default.
...
Create an analysis campaign only when the extra slices have clear academic or claim-level value relative to their resource cost.
...
If the extra work should happen on an older durable branch rather than the current head, first switch the runtime back there (activate that branch), then launch the analysis campaign from that activated workspace.

When the main-experiment record succeeds, send a richer threaded `milestone` update rather than a generic one-line progress ping.
...
Do not treat a main run as durably complete until the main-experiment record succeeds.

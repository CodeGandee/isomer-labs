...
Path surfaces include reserved labels such as `topic.workspace`, `topic.runtime.db`, `topic.intent.kaoju_mindsets`, `topic.repos.main`, `topic.repos.main.isomer_managed`, `topic.repos.main.tracked.*`, `topic.agents_root`, `topic.records.*`, `topic.runtime`, `topic.tmp`, `topic.actors.output_root`, `agent.workspace`, `agent.output_root`, `agent.private_artifacts`, `agent.public_share`, `agent.links`, and `agent.tmp`. `topic.intent.kaoju_mindsets` is an owner-editable durable directory that defaults to `intent/derived/mindsets`; Kaoju resolves it by label and stores deterministic topic-owned Mindset Source JSON children there. User-defined storage lives under `custom.*`, while additional topic repositories may use grouped `topic.repos.*` labels. Helper-created non-main topic repositories default under `repos/extern/...`, but the labels remain the storage API. Topic Workspace Manifest bindings use `label`, `path`, and explicit `storage_profile`; `storage_profile` supplies the context, lifecycle, visibility, safety, and Git semantics that a path string cannot safely imply.

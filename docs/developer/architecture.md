...
Path surfaces include reserved labels such as `topic.workspace`, `topic.runtime.db`, `topic.intent.kaoju_mindsets`, `topic.paper.template_exchange_root`, `topic.repos.main`, `topic.repos.main.isomer_managed`, `topic.repos.main.tracked.*`, `topic.agents_root`, `topic.records.*`, `topic.runtime`, `topic.tmp`, `topic.actors.output_root`, `agent.workspace`, `agent.output_root`, `agent.private_artifacts`, `agent.public_share`, `agent.links`, and `agent.tmp`. `topic.intent.kaoju_mindsets` is an owner-editable durable directory that defaults to `intent/derived/mindsets`; Kaoju resolves it by label and stores deterministic topic-owned Mindset Source JSON children there. `topic.paper.template_exchange_root` defaults to plural `intent/derived/writing-templates` and contains non-canonical role and name working copies. Mutable canonical template stock remains in Workspace Runtime. Checked packaged templates provide immutable fallback without creating topic state. Explicit singular bindings remain authoritative with a migration advisory, while unbound legacy singular content requires previewed conflict-safe migration. User-defined storage lives under `custom.*`, while additional topic repositories may use grouped `topic.repos.*` labels. Helper-created non-main topic repositories default under `repos/extern/...`, but the labels remain the storage API. Topic Workspace Manifest bindings use `label`, `path`, and explicit `storage_profile`; `storage_profile` supplies the context, lifecycle, visibility, safety, and Git semantics that a path string cannot safely imply.

The canonical `isomer-default.v1` Topic Workspace and Agent Workspace layout, including default paths such as `repos/topic-main`, `repos/extern/...` for helper-created non-main repositories, `repos/topic-main/isomer-managed/`, `agents/<agent-name>` worktrees, per-agent branch namespaces, tracked Isomer material, agent-owned untracked shares, topic-owned projections, generated links, topic-owned Pixi task channels, and launch cwd expectations, is defined in [Topic Workspaces](../manual/topic-workspaces.md).

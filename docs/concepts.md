...
**Semantic Workspace Surface Label**: a stable dotted label such as `topic.repos.main`, `topic.records.artifacts`, `topic.runtime.db`, `agent.workspace`, `agent.private_artifacts`, `agent.public_share`, or `agent.scratch`. Users, agents, skills, and adapters query labels through Workspace Path Resolution instead of relying on fixed directory names. Isomer owns reserved roots such as `project`, `topic`, and `agent`; user-defined labels live under `custom.*`; additional topic repositories may use the grouped reserved family `topic.repos.<group...>.<repo-name>` and helper-created non-main repositories default under `repos/extern/...`.

**Default Layout Profile**: the built-in `isomer-default.v1` mapping from semantic labels to the standard paths shown in Topic Workspace documentation, such as `repos/topic-main`, `records/artifacts`, `runtime`, and `agents/<agent-name>`. Repository helper commands use `repos/extern/...` as the default physical namespace for non-main topic repositories. It is a fallback and materialization profile, not the only valid Topic Workspace shape. Use `project paths default` to query a built-in default path, `project paths materialize-default` to create default-owned targets, and manifest binding commands to register custom paths explicitly.

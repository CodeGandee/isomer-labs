...
**Topic Workspace Manifest**: the topic-owned `topic-workspace.toml` file at the root of a Topic Workspace. It binds semantic workspace surface labels to concrete paths or bounded agent templates for that Topic Workspace, and it can carry topic-scope User Plugin registrations and runtime params. Active bindings use `label`, `path`, and `storage_profile`; the storage profile defines required context, path kind, lifecycle, visibility, safety policy, and Git semantics. It is not Project Config Directory state and is not a replacement for Workspace Runtime records.

...
Each rendered row is keyed by `idea_id` and shows `created_at`, `display_key`, `title`, parent display keys, and status. The GUI uses `display_key` in the `I-<index>` format as the short visible identity and falls back to `idea_id` only when old data has not been repaired yet.
...
All visible columns are sortable. Sorting uses the selected column first and then falls back to creation time, updated time, title, and idea id. The in-view entry-count control is applied after filtering and sorting.

...
`template export --kind content` defaults to `<exchange-root>/content/main/`; `--kind latex` defaults to `<exchange-root>/latex/main/`. When `--name` is omitted, valid topic-owned `main` wins and verified absence selects the checked packaged `main` without creating topic stock. Explicit names remain strict. Reserved `.isomer-template-export.json` records the kind, name, selection source, topic ref and token or packaged identity and version, digests, path, actor, and time. Clean recognized targets refresh in place; edited or unrecognized targets are preserved. `template exports --kind KIND` reports only that role's `unchanged`, `edited`, `missing`, `identity-invalid`, or `canonical-changed` working copies. `--observe --target PATH` records an assessed topic-stock tree without promoting it. `promote-export` updates matching topic stock with its state token or creates named stock from a verified packaged-origin export.

For the request “export the LaTeX template for me to edit,” use the selected Research Topic explicitly and run `template export --topic my-topic --kind latex` without `--target`. The CLI resolves `<topic-workspace>/intent/derived/writing-templates/latex/main/`; it does not use cwd, Topic Main, a Topic Actor Workspace, or an Agent Workspace. This exchange is separate from `init-tex`, which snapshots the exact topic or packaged selection and creates paper-local `KAOJU:PAPER-DRAFT-TEX` for agent filling or repair. A typed failure reports compact `selected_context` metadata. Correct the selector or readiness problem without searching sibling topics or substituting an unmanaged filesystem copy.
...
`template ensure-defaults` creates missing content and LaTeX `main` records from checked packaged resources and exports only absent safe working copies. It preserves existing valid stock and edited, stale, identity-invalid, or unrecognized exchange targets and reports each role independently. `template migrate --kind content` previews existing records and legacy export paths; `--apply` annotates old named content records without changing stable refs, state tokens, or tree bytes. `template migrate --kind latex` reports historical TeX candidates. Adoption requires `--apply --record EXACT_REF --metadata-file FILE`, copies the selected tree into LaTeX stock, and leaves the source record unchanged. `template migrate-exchange-root` previews exact singular and plural inventories; apply requires `--expected-preview`, blocks non-equivalent dual-root content, publishes the plural root atomically, preserves historical observations, and removes the singular root only after verification. The retired `paper export-template`, `paper apply-template`, and legacy `ext research templates` mutation surfaces remain non-mutating.

`paper validate` checks canonical MyST. `derive-markdown` creates a non-canonical review view. `init-tex` accepts exact content and LaTeX selectors; omitted selectors resolve role-local topic `main` and then checked packaged `main`. It snapshots the complete selected presentation tree, records topic or packaged provenance, and composes a self-contained TeX draft through preamble, marker, or include mode without materializing exchange content. `tex-status` reports stocked-template and paper-local repair drift. `build-pdf` verifies the pinned snapshot, uses the declared entrypoint and build profile, executes `document_build`, and records the Run, compile log, drift, PDF inspection, and publication Gate. Named-template commands plus `init-tex`, `tex-status`, and `build-pdf` include compact selected Research Topic and Topic Workspace metadata in JSON successes and structured failures; the metadata makes selection visible without changing lookup scope or build Gates.
...
isomer-cli --print-json ext kaoju paper template update --topic my-topic --kind latex --from revised-publisher-template --expected-state template-state-... --actor agent:writer
...
isomer-cli --print-json ext kaoju paper template promote-export --topic my-topic --kind latex --from topic-workspace/intent/derived/writing-templates/latex/main --actor agent:writer
isomer-cli --print-json ext kaoju paper template ensure-defaults --topic my-topic --actor agent:topic-creator
...
isomer-cli --print-json ext kaoju paper template migrate-exchange-root --topic my-topic
isomer-cli --print-json ext kaoju paper template migrate-exchange-root --topic my-topic --apply --expected-preview exchange-migration-preview-... --actor agent:writer
...
Inspect legacy non-canonical LaTeX writing-template records that predate mutable named MyST trees. `list` and `show` remain read-only. `create`, `refresh`, `compile`, and `remove` return `legacy_template_mutation_disabled` plus named Kaoju CRUD, migration, or agentic reconciliation guidance and do not write the plural `intent/derived/writing-templates` exchange root or its legacy singular predecessor.

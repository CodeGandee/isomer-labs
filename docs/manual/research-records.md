...
Catalog-backed extension payloads contain non-empty `title`, `summary`, `artifact_family`, `semantic_id`, `artifact_type`, and `sections`. Extension-owned durable artifact identities use exact uppercase `EXTENSION-NAME:WHAT` syntax. A provider-neutral [literature observation](literature-observations.md) instead uses `isomer-literature-provider-observation.v1`, an observation id, and the `literature/provider-output` profile because it records provider output rather than a Kaoju-owned semantic Artifact type. Kaoju producers inspect the installed contract with `ext kaoju bindings describe KAOJU:WHAT` and let the typed Artifact service infer record kind, profile, semantic label, content mode, scope policy, and managed locator:

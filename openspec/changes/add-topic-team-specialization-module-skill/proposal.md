...
- Affected docs and validators: operator skillset docs and validation should recognize the module skill, the no-`evals/` bundle shape, required local subcommands, local support references, required guide and plan terms, no `route-service` subcommand, `fast-forward`, and Imsight workflow structure for the entrypoint and subcommand pages.

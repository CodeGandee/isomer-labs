...
- [ ] 5.5 Implement `isomer-cli project service-requests create`, `dispatch`, and `status` with deterministic lifecycle and support-artifact output.
...
- [ ] 7.1 Implement `choose-directions` in frame guidance with bounded proposals, stable direction fields, multi-selection, custom directions, rejection, revision, and explicit human confirmation.
...
- [ ] 7.3 Implement `build-reading-list` with one scope per direction, three priority plus three secondary targets, five source classes, papers and reports as primary works, version families, query provenance, and non-blocking shortage warnings.
...
- [ ] 7.8 Refactor source-digest and claim-evidence guidance for paper locators, immutable repository commit plus file and line locators, source statement versus interpretation, approval, and refinement.
...
- [ ] 8.1 Implement `ingest-source-code` resolution for URLs, names, paper refs, and reading-item refs with bounded discovery, relationship verification, ambiguity handling, and source-access blockers.
...
- [ ] 8.7 Create durable `kaoju:smoke-run-script` and `kaoju:smoke-run-result` artifacts and require a successful task-critical smoke observation before environment readiness.
- [ ] 8.8 Implement `run-code-trial` prerequisite resolution, `kaoju:method-trial-plan`, human Gate checkpoint, durable minimal wrapper, and no-ambient-environment enforcement.
...
- [ ] 8.10 Preserve failed, repaired, and adapted attempts as separate Runs and prevent a later result from overwriting earlier fidelity or verdict.
...
- [ ] 9.2 Refactor `isomer-kaoju-write` to require accepted audit and synthesis inputs and create `kaoju:paper-structure-myst`, `kaoju:paper-draft-myst`, `kaoju:citation-map`, and `kaoju:paper-revision-log` as canonical paper state.
...
- [ ] 9.4 Register `isomer-cli ext kaoju paper export-template` with actor-selected or semantic-label-resolved targets, base digest, source revision, paper line, tied draft, source refs, and export Artifact registration.
- [ ] 9.5 Register `apply-template` with manifest validation, optimistic concurrency, required-section and placeholder checks, canonical template revision, draft regeneration handoff, and no-mutation failure behavior.
...
- [ ] 9.7 Implement `init-tex` using the selected MyST parser to create TeX template and draft manifests, conversion diagnostics, citation inputs, included files, and lineage without claiming build readiness.
...
- [ ] 9.9 Implement `build-pdf` through `document_build`, capture toolchain and fallback rationale, create distinct build Runs and compile logs, inspect PDF output, and apply publication Gate policy.
...
- [ ] 10.3 Implement an idempotent Markdown page and JSON or YAML mapping exporter with create, update, and overwrite policies that preserve unrecognized human files.
...
- [ ] 10.6 Implement viewer deployment to an actor-selected directory with recognized-target update handling, checksums, wiki target validation, and `kaoju:llm-wiki-viewer` and `kaoju:llm-wiki-viewer-manifest` registration.
...
- [ ] 10.9 Add UC-07 integration fixtures for subset and default export, provenance mapping, idempotent update, protected human files, installed-package operation, viewer deployment and update, stale wiki targets, port conflict, local launch, and forbidden external skill routing.

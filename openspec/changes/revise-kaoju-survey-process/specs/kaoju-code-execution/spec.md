...
The system SHALL resolve a supplied repository URL, repository name, paper ref, or reading-item ref to an unambiguous Source Identity before registering associated source code.

#### Scenario: Actor supplies a repository URL
- **WHEN** the actor requests ingestion for an accessible repository URL
- **THEN** the system verifies the remote identity, records the requested and resolved locators, and proceeds through canonical repository acquisition
...
- **AND** ambiguous or unverified candidates require clarification or produce a blocker
...
- **THEN** the system records `kaoju:source-access-blocker` with attempts, impact, and resume condition
- **AND** it does not register a nonexistent checkout

### Requirement: Source Repositories Use Canonical Acquisition
The system SHALL acquire selected source code through the Project repository service into a canonical external repository location resolved by semantic label.

#### Scenario: Repository is acquired
- **WHEN** an accessible repository identity passes preflight
- **THEN** the repository service clones with depth one by default, records the remote identity and immutable commit, and registers the checkout only after success
...
- **THEN** the service removes or marks the staged checkout for recovery and returns a blocker
- **AND** no successful canonical repository registration is exposed
...
- **WHEN** the actor requests `prepare-code-run` for acquired source
...
- **AND** source acquisition is routed first when no verified repository ref exists
...
The system SHALL start UC-10 only with a verified source ref and accepted environment-preparation result.

#### Scenario: Trial prerequisites are ready
- **WHEN** `run-code-trial` resolves the acquired repository commit and accepted Pixi environment and smoke result
...
- **THEN** the procedure routes to the applicable acquisition or preparation use case or reports a blocker

...
Milestone 6 should create View Manifest records for the literature matrix, claim graph, and inquiry comparison. Each View Manifest should reference the underlying Artifacts, Evidence Items, Findings or claim candidates, inquiries, and Decision Record. It should not require a GUI renderer or AG-UI event transport.
...
- View Manifest scope expands into GUI rendering -> Store semantic manifest records only and defer rendering to later milestones.
...
- How much of the literature matrix and claim graph should be deterministic fixture content versus adapter-produced content from the simulated handoff replies?

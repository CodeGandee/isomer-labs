...
- [UC-02: Topic Service Agent Drafts an Instantiation Packet](uc-02-topic-service-agent-drafts-instantiation-packet.md) — a Houmao-backed Topic Service Agent supports Topic Team Specialization for `deepsci-mini`, resolves topic placeholders, plans copied template material and topic edits, and returns a reviewable Topic Team Instantiation Packet.
- [UC-03: Approved Packet Materializes and Launches a Topic Team](uc-03-approved-packet-materializes-and-launches-topic-team.md) — the Project Operator Session approves packet-backed profile bundle materialization, copies and specializes template material inside the Topic Workspace, creates runtime records, and launches or simulates the Agent Team Instance with provenance.

## Scope

The use cases assume the canonical domain terms in `.imsight-arts/project-explore/domain-concepts/dc-isomer-platform-language.md`, especially Project Operator Session, Topic Service Agent, Topic Service Master, Service Request, Topic Team Specialization, Topic Team Instantiation Packet, Topic Agent Team Profile, Topic Agent Team Profile Bundle, Agent Team Instance, and Execution Adapter.

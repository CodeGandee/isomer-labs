...
The GPU analytical-modeling user-plugin guidance SHALL require bottleneck-oriented experiments to connect each predicted saturated component or blocking path to an experiment that stresses one component or path at a time and collects hard evidence that supports or refutes the prediction.
...
The GPU analytical-modeling user-plugin guidance SHALL require any proposed analytical model that predicts latency or runtime to compare predicted latency against measured latency under a declared workload and hardware scope before making latency-accuracy claims.
...
The GPU analytical-modeling user-plugin guidance SHALL require final papers, reports, and closure summaries to show hard evidence for central runtime, saturated-component, and blocking-path claims rather than only stating conclusions.
...
- **THEN** only `skillset/user-plugins/gpu-analytical-modeling/` and change artifacts are modified, with no edits to packaged system skills, callback registry behavior, CLI behavior, runtime behavior, or distribution manifests

#### Scenario: No topic-specific hard-coding
- **WHEN** the strengthened guidance uses examples
- **THEN** examples may name generic GPU evidence roles and hardware components but must not require a specific kernel, GPU SKU, topic workspace, host setup, command wrapper, profiler command, paper venue, paper artifact, or artifact path as general plugin context

...
Managed Artifact content uses a generic record-owned layout below the binding-resolved Semantic Workspace Surface Label, normally `topic.records.artifacts`. The artifact service allocates opaque internal paths from record kind and record or revision identity; binding entries and skills do not declare, construct, or parse Kaoju-specific subdirectories. The first implementation may reuse the current `research-records/<record-kind>/<record-id>/` shape, but that shape is not a public contract. Semantic organization comes from state-DB queries. Human-readable organization comes from on-demand rendering, registered views, or explicit exports and worker-visible projections, none of which replace canonical structured content.

...
- Move required-input contracts into each executable subcommand page so direct subcommand calls are self-contained, leaving the parent `SKILL.md` as a router rather than a central input table.
...
- Make the intended setup result explicit: given a Project Manifest-declared Topic Workspace, environment setup leaves the Topic Workspace directory with `.pixi/`, `.gitignore`, `pixi.toml`, and `pixi.lock` directly under `<topic-workspace-dir>/`.
...
- Add Python version selection rules: recover the version from prompt, gate, and repo evidence when possible; if unspecified, choose the previous stable Python minor release at execution time; when sources conflict, choose the highest required version and adapt environment requirements toward it where service-safe.
- Add starter Python dependencies through PyPI: `scipy`, `mdutils`, `ruff`, `mkdocs-material`, `mypy`, `attrs`, `omegaconf`, `imageio`, `matplotlib`, `jsonschema`, and `jinja2`.
- Add Topic Workspace VCS ignore rules for `.pixi/`, `tmp/`, and `.git/`, without adding `extern/orphan` rules.
- Require Topic Workspace setup and verification commands to use explicit `pixi run --manifest-path <manifest_path> --environment <pixi_environment>` command style.

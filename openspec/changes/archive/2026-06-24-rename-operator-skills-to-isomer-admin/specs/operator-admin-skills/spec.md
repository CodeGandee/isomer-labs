...
- **THEN** Project Operator Session project discovery uses `isomer-admin-project-aware` instead of `isomer-rsch-project-aware`

#### Scenario: Service request routing skill is renamed
- **WHEN** active skill refs are inspected
- **THEN** Project Operator Session Service Request routing uses `isomer-admin-service-request-route` instead of `isomer-rsch-service-request-route`

#### Scenario: Template inspection skill is renamed
- **WHEN** active skill refs are inspected
- **THEN** operator template inspection uses `isomer-admin-template-inspect` instead of `isomer-rsch-template-inspect`

#### Scenario: Topic context resolution skill is renamed
- **WHEN** active skill refs are inspected
- **THEN** operator topic context resolution uses `isomer-admin-topic-context-resolve` instead of `isomer-rsch-topic-context-resolve`

#### Scenario: Placeholder reconciliation skill is renamed
- **WHEN** active skill refs are inspected
- **THEN** operator placeholder reconciliation uses `isomer-admin-placeholder-reconcile` instead of `isomer-rsch-placeholder-reconcile`

#### Scenario: Topic profile drafting skill is renamed
- **WHEN** active skill refs are inspected
- **THEN** operator topic profile drafting uses `isomer-admin-topic-profile-draft` instead of `isomer-rsch-topic-profile-draft`

#### Scenario: Profile review approval skill is renamed
- **WHEN** active skill refs are inspected
- **THEN** operator profile review and approval uses `isomer-admin-profile-review-approval` instead of `isomer-rsch-profile-review-approval`

#### Scenario: Profile materialization skill is renamed
- **WHEN** active skill refs are inspected
- **THEN** operator profile materialization uses `isomer-admin-profile-materialize` instead of `isomer-rsch-profile-materialize`

#### Scenario: Team launch orchestration skill is renamed
- **WHEN** active skill refs are inspected
- **THEN** operator team launch orchestration uses `isomer-admin-team-launch-orchestrate` instead of `isomer-rsch-team-launch-orchestrate`

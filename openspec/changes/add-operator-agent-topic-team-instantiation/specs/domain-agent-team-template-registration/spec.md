...
- **WHEN** a user or Operator Agent inspects a Domain Agent Team Template
...
### Requirement: Template Boundary for Operator Agent Instantiation
The system SHALL preserve the reusable template boundary when Operator Agent instantiation metadata is added.
...
- **WHEN** a Domain Agent Team Template source file contains an approved instantiation packet, concrete Topic Agent Team Profile, concrete Agent Workspace path, live adapter ref, or user approval decision
- **THEN** template validation rejects the file as crossing from template material into topic or runtime material

...
### Requirement: Packet-Backed Profile Materialization
The system SHALL materialize authoritative Topic Agent Team Profiles from validated instantiation packets approved by a Project Operator Session or Operator Agent.
...
- **THEN** the profile or adjacent metadata records the packet ref, project operator actor or session ref, Topic Service Agent refs when used, approval ref, source template ref, and validation result
...
The system SHALL prove profile materialization through deterministic project-operator and Topic Service Agent packet fixtures before relying on live Houmao.

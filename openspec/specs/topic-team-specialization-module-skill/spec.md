...
- **THEN** `ensure-topic-registration` verifies that the selected Research Topic has an active `topic_standalone_pixi_bindings` entry, a Pixi-resolvable implicit default Topic Workspace binding, or another supported environment binding accepted by `isomer-srv-env-setup`

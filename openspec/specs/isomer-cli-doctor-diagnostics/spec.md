...
## Purpose
Define the read-only `isomer-cli project doctor` diagnostics contract for host dependency, Project, and Research Topic readiness checks.
## Requirements
### Requirement: Read-only Doctor Command
The system SHALL provide a top-level `isomer-cli project doctor` command that reports dependency, Project, and topic readiness diagnostics without mutating Project files, Topic Workspaces, Workspace Runtime state, or Pixi environments.

#### Scenario: Doctor appears in help
- **WHEN** a user runs `isomer-cli --help`
- **THEN** the command help lists `project doctor` as a top-level command

#### Scenario: Doctor does not mutate runtime state
- **WHEN** a user runs `isomer-cli project doctor` for a Project or Research Topic
...
- **WHEN** a user inspects `isomer-cli project doctor --help`
...
- **THEN** `project doctor` reports the Pixi executable path and version as a passing host check

#### Scenario: Pixi executable is missing
- **WHEN** `pixi` is not available on `PATH`
- **THEN** `project doctor` reports a failing host check with an Isomer diagnostic that names Pixi as a required Project dependency

#### Scenario: Pixi requirement is verified when declared
- **WHEN** the Project-level Pixi manifest declares `requires-pixi`
- **THEN** `project doctor` verifies the running Pixi version against that requirement or reports that the check was skipped because Pixi is unavailable
...
- **THEN** `project doctor` reports the manifest path, available Pixi environments when known, and whether `pixi.lock` is present
...
- **THEN** `project doctor` reports a failing Project check because Pixi is a required dependency for this Isomer version

#### Scenario: Lockfile warning is reported
- **WHEN** a Project-level Pixi manifest exists but `pixi.lock` is missing
- **THEN** `project doctor` reports a warning check rather than creating or updating the lockfile
...
- **THEN** `project doctor` verifies that the standalone Pixi manifest path resolves inside the Project root and reports whether the manifest exists without running installation
...
- **THEN** `project doctor` reports a failing topic check without creating the standalone manifest or installing the standalone environment

### Requirement: Doctor Output Contract
The system SHALL emit deterministic text and versioned JSON output for `project doctor` suitable for CI, unit tests, and future Operator Agent consumption.

#### Scenario: JSON output is wrapped
- **WHEN** a user runs `isomer-cli --print-json project doctor`
...
- **WHEN** `project doctor` emits JSON checks
...
- **WHEN** a user runs `isomer-cli project doctor` without JSON output
...
- **THEN** `project doctor` reports the offending field or path without printing the secret value

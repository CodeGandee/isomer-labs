...
- **WHEN** an agent reads `skillset/service/isomer-srv-env-setup/SKILL.md`
...
The service environment setup skill SHALL require `derive-gate` to record the enclosure strategy that later dependency installation and verification must follow.

#### Scenario: Dependency plan includes enclosure choices
- **WHEN** `derive-gate` writes or updates `<topic-workspace-dir>/user-intent/derived/isomer-env-gate.md`
...
- **WHEN** `derive-gate` writes `Pixi Install Commands`
...
- **THEN** `derive-gate` records the service-safe portion of setup when possible
...
The service environment setup skill SHALL require `install-deps` to enforce the enclosure strategy before mutating Topic Workspace environment files or running setup commands.

#### Scenario: Install step refuses unclassified dependencies
- **WHEN** `install-deps` reads `isomer-env-gate.md`
...
- **THEN** `install-deps` uses Pixi dependency commands against the selected Topic Workspace manifest
...
- **THEN** `install-deps` records the exact environment variables, source commands, runtime paths, and setup commands in the derived gate execution log
...
- **THEN** `install-deps` places the fallback under `<topic-workspace-dir>/.isomer-user-env/`
...
The service environment setup skill SHALL require `verify-gate` to judge readiness from recorded Pixi-scoped commands and recorded runtime wiring, not from ambient shell state.

#### Scenario: Verification command is replayable
- **WHEN** `verify-gate` runs a command from `isomer-env-gate.md`

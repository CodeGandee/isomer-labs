...
When the current Artifact Format Profile or producer contract marks the record as idea-bearing, set `idea_effects_required` and include the exact authored `research_idea_effects` object. Invoke `$isomer-research-idea-recording` for its schema and validation rules. Record lineage in `parents` does not imply Idea Lineage, and the coordinator never synthesizes concept state from the payload.

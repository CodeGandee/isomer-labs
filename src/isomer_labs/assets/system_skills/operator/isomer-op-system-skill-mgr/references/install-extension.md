...
5. Direct CLI install defaults to project scope when `--scope` is omitted, but this manager always passes `--scope <selected-scope>` to preserve its recorded choice. Run `isomer-cli --print-json system-skills install --target <host-known-target> --scope <selected-scope> --extension <extension-id>`. Add `--force` only when the user authorized replacing an invalid, stale, or incompatible Isomer-managed projection.

...
3. Run `isomer-cli project topic-reset plan --topic <research-topic-id> <checkpoint-id>` to create a read-only structured reset plan.
...
5. Report checkpoint id, reset plan id, managed payload file path, explicit Markdown export path when present, blocker count, action counts, and the next safe operator action.
...
`reset-plan` is read-only with respect to reset candidates. It may create a structured reset plan record and managed payload file, but it must not delete runtime rows, managed payload files, explicit exports, or other files.

...
- MUST treat Workspace Boundaries and Peer Read Access as advisory collaboration contracts, not filesystem-grade security isolation.

- MUST reset checkpoint, reset plan, and reset apply workflows use structured records and Workspace Runtime state only. Use `isomer-cli project topic-reset ...` commands, require selected Research Topic and Topic Workspace context, inspect blockers before mutation, and never depend on project-root Git state for reset behavior.

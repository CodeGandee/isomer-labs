...
10. Build v2-independent actor onboarding context for each selected Topic Actor: actor name, actor kind, runtime kind, role kind, controller kind, cwd, branch, integration surface, worktree evidence, support labels, boundary notes, verification evidence, Topic Service Master preparation state, and blockers.
11. Report actor roster, each actor cwd, branch, worktree evidence, support labels, derived actor env gate path, actor onboarding context, Topic Service Master preparation state, verification evidence, blockers, and readiness state.

...
4. Include Topic Service Master preparation state when setup-actors reached Houmao-backed work: `ready`, `skipped`, `blocked`, or `not_configured`, plus the Project Manifest Houmao integration evidence used for that state.
...
- **Topic Service Master**: Preparation state, skip reason or blocker, Project Manifest Houmao integration evidence, returned skill route when enabled, and explicit note that launch remains a later lifecycle operation.

...
2. Resolve read-only Houmao integration policy with `isomer-cli --print-json project integrations houmao status` when Topic Service Master preparation is in scope.
...
Report Project root, Research Topic ref, Topic Workspace ref, `topic.workspace.summary` path and freshness, ladder table, ready surfaces, verified checks, blockers, skipped stages, and Houmao-backed Topic Service Master preparation state.

...
- Delegate post-init extension reconciliation to `isomer-op-system-skill-mgr` during operator-controlled `init-project`, because direct `isomer-cli project init` does not know the active agent inventory or register extensions; skip mutation when the user explicitly opts out.
...
- Use Project Manifest-backed CLI surfaces.
- Use `isomer-op-topic-creator`, with Topic Manager actor management underneath, for other manual-research requests.
...
- Generic topic preparation, launch-facing work, readiness gaps, missing summaries, or missing Agent Workspaces do not establish that intent.

## Guardrails

- DO NOT scan unregistered directories as authority for Research Topics or Topic Workspaces.

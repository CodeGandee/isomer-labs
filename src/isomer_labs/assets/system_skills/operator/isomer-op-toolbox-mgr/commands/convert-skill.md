...
2. **Classify the conversion target**: prompt-file router plus routed Toolbox skill, direct supplemental skill-directory callback, or rewritten Toolbox-local instruction material.
...
7. **Validate and report** the source path, conversion choices, callback ids, invocation posture, Runtime Param ids, blockers, and next install or review command.
...
- Converted Toolbox skills default to `allow_implicit_invocation: false`; use prompt-file routers for normal callback routing.
- Treat direct `skill_dir` callback conversion as supplemental instruction material only when the user chooses that shape explicitly.

...
1. **Resolve intent** from the prompt, intent file, existing notes, or plain-language task. Identify desired callbacks, target skills, stages, Runtime Params, and install scope.
...
7. **Report Essential Output** with status, Toolbox ID, source path, authored files, callback ids, Runtime Param ids, scope, validation result, blockers, rollback hint, and next action.

...
`isomer-admin-topic-creator` owns topic initialization. If no initialized Research Topic and Topic Workspace can be resolved, route to the creator rather than deriving topic ids, registering topics, or writing topic intent from this skill.

`isomer-srv-topic-env-setup` owns full gate-driven topic environment setup, Topic Main Development Repository setup, canonical external repository acquisition, and external repo projection materialization. Ad hoc package install, update, remove, and package verification requests enter through `isomer-admin-topic-mgr env-*` commands.

...
Use this subcommand only for optional manual worktree support or compatibility diagnostics. The normal topic-team setup path delegates per-agent worktree creation and cwd proof to `isomer-srv-agent-env-setup`.
...
6. Create missing branches and worktrees only for safe planned entries:
   ...
   - Stop if mutation was not explicitly requested; report that canonical creation belongs to `isomer-srv-agent-env-setup`.
...
8. Prepare or validate `agent.tmp`:
   ...
   - Use the Topic Main Development Repository root ignore policy because Agent Workspaces are Git worktrees.
...
Do not create legacy support roots or top-level Topic Main Development Repository collaboration directories. Worker-facing Isomer material belongs under `isomer-managed/`.

Do not add `.gitkeep` or durable files under `agent.tmp`. Missing `agent.tmp` directories are repairable local posture; missing or ineffective Topic Main Development Repository ignore policy and tracked files under `agent.tmp` are blockers for this setup flow.

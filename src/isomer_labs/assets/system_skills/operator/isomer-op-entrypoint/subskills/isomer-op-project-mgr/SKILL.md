...
- Delegate post-init extension reconciliation through `isomer-op-entrypoint->system-skills` during operator-controlled `init-project`, because direct `isomer-cli project init` does not know host roots or register extensions; skip mutation when the user explicitly opts out.

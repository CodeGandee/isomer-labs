...
| Topic env predecessor | Prefer `require-topic-env-ready` and `require-topic-main-ready` output so the source gate can be interpreted against the selected Topic Workspace Pixi environment, prepared topic-main repository, and projection evidence. |
...
3. **Read the source intent**:
   - Extract source intent, required command set, expected results, success criteria, Topic Main Development Repository predecessor requirements, projection visibility requirements, agent plan constraints, cwd assumptions, and blockers.
...
6. **Report the source gate summary** and carry it to `derive-agent-env-gate`, `require-topic-main-ready`, `create-agent-worktrees`, and `verify-agent-env-gate`.

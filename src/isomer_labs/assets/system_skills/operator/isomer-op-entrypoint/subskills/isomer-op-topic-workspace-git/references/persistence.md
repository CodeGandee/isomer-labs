...
Support files may contain stable ids, Project-relative copy paths, credential-safe publication and GitHub reference locators, visibility, `exclusive_snapshot` fallback authority, semantic content classes, raw-byte settings, dispositions, source-identical relative mappings, fingerprints, component branches and commits, Topic Main anchor relationships, exact reference commits, complete remote refs and tags, observed remote HEAD, history-retention or history-purge posture, per-ref strategies, observed bases and leases, compatibility evidence, planned results, fallback reasons, identity-substitution categories, generated-navigation fingerprints, reproduction limitations, conflicts, outcomes, and resume state. Publication-plan v1 approvals are stale under history-aware synchronization; existing Publication Binding v1 files and supported legacy projection manifests and outcomes remain readable.
...
The tracked `.isomer-publication/` overlay is stable portable publication metadata. It records binding, canonical topology, projection, component pins, and reproduction information but omits plan ids, approval timestamps, observed remote state, push attempts, and outcomes so unchanged publication content remains a no-op. The ignored `.isomer/topic-git/` support root contains that volatile local operation state. Neither restores source worktree administration, Workspace Runtime, local identity, or a working Topic Workspace automatically.

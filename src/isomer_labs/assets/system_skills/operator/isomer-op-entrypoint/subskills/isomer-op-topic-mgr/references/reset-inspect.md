...
5. Report checkpoint status, plan status, blockers, managed payload file paths, explicit Markdown export paths when present, preserved setup evidence, and whether a fresh read-only plan is needed before apply.
...
`reset-inspect` is read-only. It consumes structured records and Workspace Runtime state, names payload files and optional explicit exports, and must not mutate records or files.

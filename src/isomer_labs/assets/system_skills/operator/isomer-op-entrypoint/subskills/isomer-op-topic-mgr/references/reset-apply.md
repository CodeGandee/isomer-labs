...
3. Confirm the operator understands that reset apply is destructive for only the runtime rows, managed payload files, explicit export files, and managed actor or agent workspace paths named by the approved plan.
4. Run `isomer-cli project topic-reset apply --topic <research-topic-id> --yes <checkpoint-id> <plan-id>` only after explicit confirmation.
5. Report reset outcome id, status, applied/skipped/failed action counts, managed payload file path, explicit Markdown export path when present, diagnostics, and any follow-up blocker.

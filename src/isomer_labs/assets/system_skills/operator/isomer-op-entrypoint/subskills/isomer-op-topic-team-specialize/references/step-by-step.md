...
3. Before each step, tell the user what will happen, what inputs or semantic labels may be read or changed, and what output should exist after the step:
   ...
   - For `setup-agent-workspace`, also explain that it delegates per-agent worktree creation and readiness proof to `isomer-op-entrypoint->agent-env setup-agent-env <research_topic_id>` after source intent, Topic Main Development Repository predecessor evidence, required projection predecessor evidence, and authoritative Agent Names exist.

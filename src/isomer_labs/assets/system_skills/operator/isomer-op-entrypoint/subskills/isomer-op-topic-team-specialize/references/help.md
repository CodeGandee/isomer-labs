...
1. Print a concise description:
   - `isomer-op-topic-team-specialize` initializes and specializes one Research Topic into static Topic Team material and durable setup state from one Domain Agent Team Template.
...
4. Print the user-facing flow:
   ...
   - Route Houmao loop explanation, adapter customization guidance, template mapping, mailbox or gateway support, and runtime inspection to `isomer-srv-houmao-interop` only as bounded service support.

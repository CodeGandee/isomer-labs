...
Inspect only project-scope roots known to the current agent. First interpret the receipt. Treat `receipt.status=current`, `coverage_status=complete`, `evidence_basis=managed_receipt`, and a usable compatibility state as managed v4 evidence. Then interpret explicit-root integrity. `coverage_status=unmanaged_complete` verifies material but does not establish Isomer ownership or permit automatic registration. Legacy flat receipt rows identify migration candidates without proving nested integrity. Partial, invalid, malformed, unsupported, drifted, obsolete, and newer-than-CLI results require advice, upgrade, or repair rather than silent registration.

Last, classify the live skill inventory. Use names from the host-provided inventory, not a guessed directory listing. `coverage_status=entrypoint_seen` means the public entrypoint name is visible but its protected integrity is unverified. `coverage_status=legacy_observed` means old flat member or alias names are visible. Neither status establishes a complete pack or permits `remember`.

...
3. Collect project-scope skill roots from the current host context and pass each root explicitly to `internals inspect-system-skill-root`. Interpret current v4 receipt evidence before explicit-root integrity evidence.
...
5. Report managed complete, unmanaged complete, partial, missing, legacy migration candidate, incompatible, `entrypoint_seen`, and unknown states. Provide registration, upgrade, repair, refresh, or installation advice but make no mutation.

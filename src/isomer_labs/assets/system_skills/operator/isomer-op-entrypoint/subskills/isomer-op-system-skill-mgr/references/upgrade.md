...
2. Inspect the exact host-known root. If it has a current v5 receipt and complete compatible public pairs, skip file mutation and finish any authorized Project registration.
...
5. Read `skill_root` from the result and verify it with `isomer-cli internals inspect-system-skill-root --skill-root <resolved-root> --extension <extension-id>`. Require a current v5 receipt, both public roles, complete nested protected coverage, managed receipt evidence, and usable compatibility.
...
7. Unless the user opted out, call Project `remember` after v5 verification succeeds.
8. Report that the current agent session may cache old discovery. Require a host refresh or new session before claiming the new public welcome and entrypoint pair is live.

...
2. Inspect only host-known roots supplied for this request. Report current v4 receipt status before explicit-root pack integrity.
...
Do not call `remember`, `forget`, install, upgrade, or repair commands. A declared extension remains `declared` even if the current host lacks matching skills; label a confirmed load failure as stale user-controlled state and point to `repair`. Never render `entrypoint_seen` or `legacy_observed` as complete pack coverage.

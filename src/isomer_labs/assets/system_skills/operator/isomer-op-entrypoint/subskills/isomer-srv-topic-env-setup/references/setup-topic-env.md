...
6. **Report the combined result** using the parent skill's **Essential Output** by default and **Complete Output** when requested:
   - Include the selected subcommand and mode, step results, Topic Main Development Repository Git state, Isomer-managed namespace posture, `AGENTS.md` and `CLAUDE.md` guidance posture and version, external repository method, verification, registration, provenance, partial-result, and projection evidence, resource and bounded-run decisions, commands, changed files, enclosure and runtime wiring, topic-local fallbacks, Topic Workspace readiness, whether per-Agent readiness was not checked, blockers, and next action.
...
| 2 | Read the source intent | [read-env-gate.md](read-env-gate.md) | `topic.intent.topic_env_requirements` metadata, source intent summary, runnable target, repo hints, repo history needs, and blockers. Skip only when an explicit manual target spec is supplied. |
| 3 | Derive or validate the target spec | [derive-env-gate.md](derive-env-gate.md) | `topic.env.topic_setup_target_spec` metadata, target spec source, repository source, revision, feature, user-command, and agent-selection requirements, projection access intent, dependency plan, enclosure strategy, resource check plan, verification commands, and blockers. |
| 4 | Ensure Topic Main Development Repository | [ensure-topic-main-repository.md](ensure-topic-main-repository.md) | Topic-main Git state, owner branch posture, Isomer-managed namespace posture, root `AGENTS.md` and `CLAUDE.md` guidance posture, guidance block version, changed files, commands run, and blockers. |
| 5 | Ensure canonical external repos | [ensure-topic-repos.md](ensure-topic-repos.md) | Semantic labels, candidate targets, user-supplied or agent-selected external methods, verification and registration evidence, immutable identities, source warnings, inspection notes, partial-result posture, and blockers. |
...
Readiness means the desired command from the resolved `topic.env.topic_setup_target_spec` runs successfully through recorded Pixi-scoped commands for a single agent or operator working in the selected Topic Workspace root, the resolved Topic Main Development Repository, or a repo-specific working directory named by the target spec. It also means required topic-main setup, root `AGENTS.md` and `CLAUDE.md` agent guidance posture, Canonical External Repositories, external method and immutable-identity evidence, post-verification semantic registration, typed provenance when required, and external projections are ready or explicitly blocked. It does not merely mean that Pixi files exist, a repository directory exists, or a command passed because the ambient shell already had a global tool, activated environment, PATH entry, library path, or sourced script. It also does not mean a Topic Agent Team Profile, per-Agent Workspace cwd, or live Agent Team Instance is ready. A generic smoke test can support diagnostics but cannot replace bounded real-path verification of each source-intent runnable target.
...
If the caller needs every Agent Workspace cwd verified, state naturally that per-Agent readiness was not checked and list the ready Topic Workspace predecessor evidence that a separate Agent readiness workflow can consume. That predecessor evidence includes the ready Topic Workspace Pixi binding, resolved `topic.env.topic_setup_target_spec`, Topic Main Development Repository Git state, Isomer-managed namespace posture, root `AGENTS.md` and `CLAUDE.md` agent guidance posture, projection metadata, enclosure records, commands run, changed files, and blockers when present. Do not read `topic.intent.agent_env_requirements` or trigger Agent environment setup from this topic-scoped flow.
...
- Keep Isomer-owned direct mutation scoped to the selected Topic Workspace Pixi environment, the resolved Topic Main Development Repository Isomer-managed namespace, post-verification semantic repository registration, and external projections under `isomer-managed/topic-owned/{readonly,writable}/extern/`. Repository source-control and copy commands remain external user or agent operations.
...
- DO NOT run repository source-control commands through Isomer or ask Isomer to clean partial external content. Registration requires completed external verification. Later external operations against existing repositories require the derived gate or user request and a new Artifact revision when immutable identity changes.

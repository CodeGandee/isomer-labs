...
4. **Extract setup intent**:
   - Include topic-level source intent, runnable target, desired command or commands, expected outputs, success criteria, repo hints, any exact user-supplied repository procedure, requested branch or commit, sparse or partial needs, submodules, LFS, history or provider requirements, dependency hints, native tool requirements, and any out-of-scope requests.
...
6. **Report the source gate summary** using the parent skill's natural-language output contract.

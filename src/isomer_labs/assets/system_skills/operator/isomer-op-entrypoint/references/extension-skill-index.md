...
3. If current-host readiness is unknown, delegate through `isomer-op-entrypoint->system-skills`. Its ordered evidence is Project declaration, current v5 receipt, explicit-root public-pair integrity, then limited live skill inventory. Use read-only detection for inspection; register only a complete current-v5 pack when the concrete request authorizes additive Project bookkeeping.

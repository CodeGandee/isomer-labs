...
4. For identity inputs, distinguish Topic Actor names from Agent Names and use `isomer-op-entrypoint->identity` when the task must run from `topic.actors.workspace` or `agent.workspace`.

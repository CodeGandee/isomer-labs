...
| Optional Source Topic Workspace root history or sanitized remote publication, including status, exact plans, initialization, local commits, publication-copy preparation, reconstruction, or synchronization. | `isomer-op-entrypoint->topic-git` | `$isomer-op-entrypoint use topic-git to <task>`. |

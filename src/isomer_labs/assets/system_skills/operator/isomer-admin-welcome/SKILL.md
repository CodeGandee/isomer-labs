...
| --- | --- | --- | --- | --- |
| `start-research-manually` | Create or prepare a Research Topic for human-orchestrated work with Topic Actors | `isomer-admin-topic-creator` | `Use $isomer-admin-topic-creator fast-forward` when the user has approved setup, or `Use $isomer-admin-topic-creator step-by-step` for guided setup | [references/start-research-manually.md](references/start-research-manually.md) |
...
Route Project setup, Project checks, topic listing, context inspection, runtime initialization, and Project-level routing to `isomer-admin-project-mgr`.

Route blank or partial Research Topic creation and manual-research-ready topic preparation to `isomer-admin-topic-creator`.

Route initialized-topic storage, Topic Actors, package mutation, environment verification, reset checkpoints, and diagnostics to `isomer-admin-topic-mgr`.

Route Topic Team Specialization to `isomer-admin-topic-team-specialize` only when the user asks for a Domain Agent Team Template or formal Agent Team path.

Route Houmao loop, runtime, launch profile, mailbox, gateway, and template-mapping questions to `isomer-admin-houmao-interop`.

Do not ask users or agents to invoke `isomer-admin-topic-workspace-mgr`, `isomer-admin-topic-prepare`, or `isomer-admin-manual-research-session`; those are retired operator compatibility skills, not active routes.

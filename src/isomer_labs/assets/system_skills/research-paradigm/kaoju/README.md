...
Paper retrieval has a separate execution and ownership boundary. `paper-search` invokes a bound external provider tool directly and maps the result into one `isomer-literature-provider-observation.v1` Artifact. A selected bundle-local implementation supplies the provider approach. `discover` retains strategy, candidate disposition, version-family, Discovery Ledger, and Reading List ownership; `acquire` and `examine` retain material and source-evidence ownership. `isomer-cli ext research literature` only validates, records, indexes, and queries local normalized data and never invokes a provider.

Kaoju topic creation keeps packaged 8/6/12/8 mindset defaults for `paper.deep-dive`, `paper.skimming`, `paper.lecture`, and `source-code.ingest` inside the protected topic-creator bundle and copies or specializes them into `topic.intent.kaoju_mindsets` only after one concrete `topic.intent.overview` exists. These directly editable JSON files are Mindset Sources, derived-intent question catalogs with reusable question sets and `additional_notes`, not Artifacts or Workflows. Every v2 Source has one `default` set, and each set uses `"all"` or an ordered question-id list plus one descriptive triggering condition. A Run selects one specialized set or falls back to `default`. The `paper.lecture` route creates a durable detailed-presentation commitment: examination records lecture exposition in the Source Digest, synthesis carries its active, blocked, or explicitly superseded section job in the Field Summary, and writing gives every active lecture-ready paper a dedicated evidence-grounded section. The write owner also packages checked immutable generic MyST `content/main` and neutral ACM `acmart` two-column `latex/main` trees; the latter carries its `sigconf,nonacm` marker entrypoint, local class and source files, upstream sample, license, and provenance metadata. Explicit Kaoju `create-topic` creates missing mutable named stock from those resources and exports non-canonical working copies under `intent/derived/writing-templates/<kind>/main/`. Repeated initialization preserves valid stock and edited or stale exports.
...
## Review and Protected Authorization

Kaoju uses human review by default. An explicit prompt may delegate agent review for a named target at these checkpoints only: explore handoff, Direction Set review, Reading List review, Comparison Intent and Proceed Decision, bounded trial-plan review and execution authorization within accepted pins, paper structure and draft review, and bounded local PDF-build authorization. The agent may inspect, revise, narrow, reject, or accept a candidate within that boundary; delegation does not mean automatic acceptance. Every delegated decision records the review mode, prompt basis, reviewing actor, rationale, affected refs, and resume posture through existing owner-supported provenance.

Silence, ordinary continuation, or generic run-to authorization does not delegate review. Delegation expires after the named checkpoint or target completes, the candidate changes outside the accepted boundary, or the procedure reaches a checkpoint outside the closed set. Ambiguous delegation pauses for the actor. Credentials, restricted data, material license terms, destructive or irreversible actions, unexpected resource or cost, public exposure, publication acceptance, publication, and submission remain protected actor Gates.

## Canonical Paper Graph

```text
accepted audit and synthesis
Field Summary lecture commitments
            |
...
If the user chooses “run to the PDF target,” the current agent maintains a prompt-scoped dependency plan. Audit, each required repair, a fresh audit, synthesis, drafting, and PDF construction retain separate procedure Runs, callbacks, checkpoints, Artifacts, and terminal reports. The controller consumes routine in-closure recovery routes only after refreshing durable state. Generic run-to authorization alone still pauses for default-human review, including paper structure and local-build authorization. A prompt that explicitly delegates those named reviews may continue through them inside the accepted paper and build boundaries. Evidence-scope ambiguity, unexpected dependencies or resource use, the publication Gate, and any external publication or submission still pause for the actor. Once the validated local PDF target completes, traversal stops; it does not publish or start later recommended work.

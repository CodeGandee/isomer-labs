...
1. **Draft comparison intent**. Use `$isomer-kaoju-frame` and `$isomer-kaoju-compare` to record candidate identities, desired conclusions, prior-evidence reuse, data, environment, reproduction or reimplementation routes, metrics, fairness, resources, Gates, and unresolved decisions.
...
3. **Check readiness after approval**. Use `$isomer-kaoju-workspace-mgr`; query registered datasets first, validate reusable Runs, and identify candidate blockers.
4. **Acquire, examine, and prepare**. Use `$isomer-kaoju-acquire`, `$isomer-kaoju-examine`, and `$isomer-kaoju-reproduce` only as required by the accepted intent.

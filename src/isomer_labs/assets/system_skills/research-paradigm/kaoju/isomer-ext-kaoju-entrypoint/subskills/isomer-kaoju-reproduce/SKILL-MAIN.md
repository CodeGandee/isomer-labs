...
3. **Reject non-reproduction routes**. If intended data, evaluator, metric semantics, or an honest fidelity mapping is unavailable, route a bounded observation or generated-data `capability-probe` to `isomer-ext-kaoju-entrypoint->trial`; do not weaken the reproduction label.
...
6. **Delegate execution mechanics**. Use `isomer-ext-kaoju-entrypoint->trial` for the approved plan, durable wrapper, Service Requests, Execution Adapter Command Request, Run checkpoints, attempts, and result Artifacts. Use bounded-run guidance for resource-heavy work.
7. **Preserve fidelity evidence**. Keep upstream-faithful, adapted, repaired, failed, and blocked Runs separate with patches, logs, outputs, purpose, fidelity, input basis, and quality checks. A material repair requires a revised plan and fresh review authorization. Human review is the default; explicit target-scoped prompt delegation permits agent review only when the repair stays inside the accepted reproduction and execution pins. Out-of-bound or protected authorization changes pause for the actor.
...
Resolve `KAOJU:METHOD-TRIAL` through `ext kaoju bindings describe KAOJU:WHAT`. Persist the compatibility reproduction summary only through `project artifacts put`; all active trial execution records belong to `isomer-ext-kaoju-entrypoint->trial` and its typed operations.

## Reference Routing

Use `isomer-ext-kaoju-entrypoint->shared` for evidence, lineage, Gate, owner, and terminal contracts. Use `isomer-ext-kaoju-entrypoint->acquire` for pinned materials, `isomer-ext-kaoju-entrypoint->trial` for preparation and execution, and `isomer-op-entrypoint->bounded-run` before resource-heavy execution.
...
| “Synthetic inputs are close enough.” | Route to `isomer-ext-kaoju-entrypoint->trial` as a capability probe; do not call it reproduction. |

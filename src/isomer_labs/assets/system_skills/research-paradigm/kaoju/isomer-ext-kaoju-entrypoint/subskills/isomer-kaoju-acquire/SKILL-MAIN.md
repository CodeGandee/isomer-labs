...
4. **Resolve the requested source**. Resolve a supplied locator, repository name, paper ref, or Reading List item to one accepted source identity and record the requested and resolved locators. Invoke `isomer-ext-kaoju-entrypoint->paper-search` for bounded paper identity or metadata lookup before acquisition judgment, and preserve ambiguity as a clarification or blocker.
...
6. **Resolve source-code requests**. Accept a URL, repository name, paper ref, or Reading List item ref. Search the Artifact Library first. When a paper association is unknown, invoke `isomer-ext-kaoju-entrypoint->paper-search` for bounded paper metadata and paper-to-repository association candidates, then present normal candidates for selection and approval and preserve ambiguity or access blockers. Candidate metadata receives no evidentiary authority before verification and normal intake approval.
...
Use `isomer-ext-kaoju-entrypoint->shared` for identity, evidence, owner, lineage, Gate, and terminal contracts. Use `isomer-ext-kaoju-entrypoint->paper-search` for bounded paper identity, metadata, and association retrieval before this skill makes acquisition judgments. Use `isomer-ext-kaoju-entrypoint->workspace` when workspace posture is unknown; use `isomer-op-entrypoint->topic-manage` for semantic path registration or managed-link mutation; use the applicable environment skill and `isomer-op-entrypoint->bounded-run` for build or resource-heavy preparation. Repository source-control commands remain direct external user or agent operations.

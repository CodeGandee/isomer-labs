...
1. **Accept audited inputs and pinned context**. Consume the entrypoint's reconciled Research Topic and applicable worker target. Require an accepted `KAOJU:AUDIT-REPORT`, the exact accepted synthesis refs, source and display refs, a paper-line scope, target reader or venue, and publication Gate policy. Read the accepted Field Summary's explicit `lecture_commitment_basis` and one-to-one `lecture_section_commitments` inventory, including empty lists. A missing or not-ready audit, incomplete lecture inventory, or unresolved competing synthesis blocks drafting. Retain `--topic <topic>` and the applicable worker selector on every downstream typed command; rerun context alignment before any intentional topic, worker, or operation-scope change.
...
3. **Select and review an adaptive structure profile**. Choose and explain a typed structure based on the accepted direction, target reader, evidence shape, and venue. Map every active lecture-ready commitment to one dedicated named section with the paper as primary subject, its reader outcome, Source Digest and lecture Run refs, accepted evidence boundary, required equation and display jobs, and limitations. A citation, list item, or short shared related-work paragraph cannot satisfy that job. Keep every blocked commitment as an explicit unresolved structure obligation with its recovery route, and preserve actor-approved supersessions with their exact prior Run or Source Digest and provenance. Human review is the default. Under explicit target-scoped prompt delegation, the agent may inspect, revise, reject, or accept the structure within the accepted paper boundary without another user turn. Record section jobs, display jobs, evidence boundaries, required limitations, review mode, prompt basis, reviewing actor, rationale, affected refs, and resume posture through existing supported provenance in `KAOJU:PAPER-STRUCTURE-MYST`; do not impose one empirical-paper outline on every survey.
4. **Lock the paper contract**. Record survey questions, scope, contribution posture, paper line, accepted evidence, canonical MyST policy, typed structure profile, citation roles, display expectations, quality checks, Gates, and the resolved structure and draft review policy. Record the accepted lecture commitment basis, active dedicated-section jobs, blocked obligations, explicit supersessions, validation policy, and exact Field Summary ref. Record content-template and LaTeX-template selection policies as separate fields; the latter may remain unresolved until PDF construction.
5. **Create and review canonical MyST state**. Resolve an explicitly named content template exactly. For an omitted selector, use the paper service's role-local resolver: valid topic-owned content `main` wins, while verified absence selects checked packaged content `main` without creating topic stock or exchange content. Record `selection_source`, role, name, digest, stable ref and state token for topic stock, or packaged identity and resource version for fallback. Snapshot the exact selected tree into paper-local state. Use the role-explicit named-template service for mutable template state; never use generic Artifact revise. Create or revise `KAOJU:PAPER-DRAFT-MYST`, `KAOJU:CITATION-MAP`, and `KAOJU:PAPER-REVISION-LOG` through their binding-permitted operations. For every active lecture commitment, author one dedicated substantial section that covers the paper's survey role, problem and prerequisites, definitions and notation, core intuition, ordered method or architecture, applicable worked trace, essential equations and displays, supporting results, comparison, limitations, failure modes, and unresolved boundaries from accepted evidence. Record machine-checkable `lecture_sections` coverage in canonical MyST frontmatter. Author essential equations in MyST with every used symbol defined and exact Citation Map evidence. For figures and tables, follow the accepted `reproduce`, `adapt`, `redraw`, `describe`, or `omit` posture; store every selected display as a separate file-backed `KAOJU:PAPER-DISPLAY`, refer to it through a typed `{{figure:<artifact-ref>}}` or `{{table:<artifact-ref>}}` placeholder, and record source, transformation posture, attribution, evidence refs, caption or interpretation status, and insertion locator. Do not imply that an adapted or redrawn display is the original. Before accepting a draft revision, apply default human review or valid explicit target-scoped agent review within the accepted paper boundary and record its review provenance. Missing or ambiguous delegation pauses for the actor. Existing drafts retain the content-template state they observed; later content-template, LaTeX-template, or package updates do not rewrite them.
6. **Validate before promotion**. Run `isomer-cli ext kaoju paper validate` for MyST syntax, required sections, lecture-section coverage, directives, roles, citations, placeholders, displays, source refs, evidence boundaries, and explicit not-applicable postures. Whenever the accepted lecture basis is nonempty, pass the exact Field Summary and Citation Map refs so validation can reconcile each active, blocked, or superseded commitment. Report structured file and line diagnostics. Invalid staging content never mutates canonical state.
...
- An active lecture commitment has no dedicated named section or is reduced to a citation or shared short paragraph.
- A blocked lecture commitment disappears without an explicit actor-approved supersession.
- A lecture equation lacks symbol definitions or a selected display loses its handling posture, attribution, or evidence lineage.
...
- A material repair runs without a revised plan and fresh review authorization.
- Agent review accepts publication, publishes, or submits a paper under generic review delegation.

...
4. **Inspect current posture**. Query relevant candidates through `project artifacts latest|list|show` with exact semantic ids and binding-defined scope keys. For repositories, require both the registered non-main `topic.repos.*` semantic label and typed caller-observed immutable identity; do not infer accepted identity from directory contents. Inspect legacy unscoped ambiguity, duplicates, supersession, stale or corrupt content links, Topic Dataset Manifest state, access, licenses, storage, compute, time, actor posture, worker output policy, required Gates, and selected reset checkpoint. Never scan directories as a discovery fallback.
...
The Workspace Readiness Artifact records the resolved Topic Workspace, current survey refs, registered repository semantic labels and their typed immutable-identity evidence, datasets, identity or staleness checks, access and license posture, available resource envelope, Gate requirements, owner handoffs, blockers, and accepted next stage. A repository path binding without accepted identity evidence remains incomplete research readiness. Empirical procedures query the Topic Dataset Manifest before asking the user for data or proposing a download.
...
Use `isomer-ext-kaoju-entrypoint->shared` for evidence and terminal semantics. Route Topic Workspace and managed-link mutation to `isomer-op-entrypoint->topic-manage`; route environment setup to the applicable service skill; use `isomer-op-entrypoint->bounded-run` before resource-heavy execution.

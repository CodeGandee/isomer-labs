...
10. **Support inspection and review**. Render the current scoped Reading List from the state DB and present effective and achieved category counts. Human review is the default and may request additions, removals, thread expansion, target revision, or shorter-list approval. Under explicit target-scoped prompt delegation, the agent may perform the same review, revise or accept the list, and continue without another user turn. Record review mode, prompt basis, reviewing actor, rationale, warnings, and accepted refs through existing supported provenance and revise the same scoped object with lineage.
...
12. **Return status**. Report accepted output refs, effective and achieved counts, shortage warnings, blockers, human or agent-delegated review posture, prompt basis when applicable, and the acquire or examine handoff.

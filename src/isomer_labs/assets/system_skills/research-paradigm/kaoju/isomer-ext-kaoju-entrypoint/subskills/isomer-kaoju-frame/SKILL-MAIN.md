...
Before accepting durable output, read the shared artifact semantics and recording rules plus this skill's `artifact-bindings.md`. For `KAOJU:DIRECTION-SET`, invoke `isomer-op-entrypoint->research-ideas` and read the Kaoju mapping reference exposed by `isomer-ext-kaoju-entrypoint->shared`. Use `KAOJU:DIRECTION-SET`, `KAOJU:SURVEY-CONTRACT`, or `KAOJU:COMPARISON-INTENT` exactly. If a binding or required surface is unavailable, return a storage blocker; never invent a path, profile, canonical Markdown file, or untracked JSON state.
...
4. **Clarify when requested or material**. Use the A/B/C/D clarification contract, mark exactly one suggested option, then ask whether to clarify more or proceed under human review. Explicit prompt-delegated agent review may resolve only choices inside the named target and accepted boundary; unresolved material intent still pauses.
5. **Propose and review directions when requested**. Propose three directions by default. Give each a stable source-local direction id, canonical semantic `idea_id`, title, summary, scoped question, boundary, source classes, coverage date, expected depth, deliverables, empirical-feasibility annotation, exact proposal path, one shared generation id, option outcome, disposition rationale, and closure reason when applicable. Mark one suggestion, accept multiple selections and custom directions, and revise proposals. Human review is the default and requires actor confirmation. Under explicit target-scoped prompt delegation, the agent may revise, select, leave open, defer, close, reject, or accept proposals without another user turn. Current host capability may annotate feasibility but cannot filter or rank directions by itself.
6. **Persist the reviewed set atomically**. Resolve the active v2 binding and put or revise one `KAOJU:DIRECTION-SET` in its topic scope only after human confirmation or valid prompt-delegated agent review. Include `research_idea_effects` with every proposal concept, exact realization, generation membership, Decision Record option, and justified transition, plus the review mode, prompt basis, reviewing actor, and rationale through existing supported provenance. A merely unselected proposal remains open. Verify all returned canonical refs and validation before checkpointing; never conflate the Direction Set with a Research Idea or frozen Survey Contract.
...
8. **Add empirical intent when applicable**. For actual-run comparison, create the Comparison Intent Document and require a Proceed Decision before preparation or Runs. Ask the human actor by default; use agent review without another turn only under explicit target-scoped prompt delegation.
...
Use `isomer-ext-kaoju-entrypoint->shared` for interaction, evidence, Artifact, owner-routing, lineage, terminal, and Kaoju-to-portfolio mapping contracts. Use `isomer-op-entrypoint->research-ideas` for canonical idea facets, exact realizations, decisions, transitions, generation membership, verification, and migration posture. Use `isomer-ext-kaoju-entrypoint->workspace` when readiness evidence is missing or stale.
...
| “The user asked to proceed quickly.” | Human review remains the default. Use agent review only under explicit target-scoped delegation, and keep protected Gates and comparison intent. |

...
3. **Resolve bindings through the extension query**. Read only the pages selected in **Reference Routing**. Before an accepted write, run `isomer-cli --print-json ext kaoju bindings describe KAOJU:WHAT`, then read `references/artifact-semantics.md`, `references/artifact-recording.md`, and the producer's concise `artifact-bindings.md`. Read `references/mindset-contract.md` whenever an applicable action uses a Mindset Record or the user targets a Mindset Source or Record. If the output creates or changes a durable research concept, invoke `isomer-op-entrypoint->research-ideas` and read `references/research-idea-recording.md`. The extension query is authoritative; local pages are bundle-local projections and procedures.
...
5. **Preflight target prerequisites and review posture**. Before target mutation, resolve accepted input refs, audit state, readiness, known producer routes, required Gates, and any explicit prompt-scoped review delegation. Human review remains the default. For a producible gap, apply `references/prerequisite-recovery.md`: pause an ordinary request before prerequisite mutation, or maintain a target-scoped dependency plan after explicit run-to authorization.
...
7. **Route governed work and checkpoint it**. Apply the worker output policy. Use human review by default or explicit target-scoped agent review for eligible Kaoju review checkpoints, a Service Request for supported Isomer-owned mutation, an Execution Adapter Command Request for registered research execution, a human Gate for nondelegable material authorization, and an immutable Run plus checkpoint for claim-bearing or resumable work. Record review mode, prompt basis, reviewing actor, rationale, affected refs, and resume posture through existing owner-supported provenance. Repository acquisition and source verification are external user or agent commands; only their verified result, semantic registration, sanitized evidence, or blocker enters Isomer.
8. **Persist through typed operations**. Use `project artifacts put` for a new ordinary current-state object or append-only event and `project artifacts revise` only when the binding permits revision. A binding with `mutable_state` requires its named owner service. Use `ext kaoju paper template --kind content` for `KAOJU:PAPER-TEMPLATE-MYST` and `--kind latex` for `KAOJU:PAPER-TEMPLATE-LATEX` so role-local identity, name uniqueness, expected-state checks, atomic managed-tree replacement, and kind-qualified mutation audit remain service-owned. Let the service infer record kind, profile, semantic label, content mode, scope policy, and managed locator.
...
10. **Return terminal state**. Report `complete`, `paused`, or `blocked` with durable refs, human or agent-delegated review posture when applicable, Run checkpoint, pending Gate, blocker and Service Request refs, and the first incomplete stage as the resume point. A bounded terminal report ends the current procedure; an explicitly authorized prompt-level controller may consume its recovery route under `references/prerequisite-recovery.md`.
...
A Gate records a human authorization decision; it does not execute work. An automatable Kaoju review checkpoint is distinct from a nondelegable Gate: it defaults to human review but may use agent review when the current prompt explicitly delegates that checkpoint for the named target. A Service Request records requested operational work and remains distinct from the Research Task, Workflow Stage, command request, and Run. The Service Team returns support Artifact, blocker, Gate, command request, Run, and provenance refs without exposing provider-specific dispatch payloads in Kaoju records.

Every Isomer-managed executable research stage uses the applicable Research Operation Extension Point and an Execution Adapter Command Request. Repository clone, fetch, copy, checkout, repair, feature setup, and source verification remain outside Isomer and use the acting user or agent's ordinary command surface. Begin or resume a Run before managed research execution, checkpoint completed refs and the first incomplete stage, and complete it with immutable command, inputs, outputs, logs, timing, status, and resume posture. Identical transient retries may use the approved bound. A material change inside explicit prompt-delegated review scope requires a revised plan and fresh recorded review; a change outside that scope or at a protected boundary requires the applicable human Gate and creates a separate Run.
...
| Clarification-first, review-mode, Proceed Decision, and Gate behavior | `references/interaction-and-gates.md` |
...
| Mindset Source, Mindset Record, immutable snapshots, additional notes, collector, and explicit-target routing | `references/mindset-contract.md` |
...
| Direction proposals, canonical portfolio effects, exact realizations, decisions, transitions, and idea guardrails | `isomer-op-entrypoint->research-ideas` and `references/research-idea-recording.md` |
...
- Expensive or private work starts without the required protected authorization, or an automatable review proceeds without human review or explicit prompt delegation.

...
5. **Load checked defaults**. Read only `assets/defaults/mindsets/paper.deep-dive.json`, `assets/defaults/mindsets/paper.skimming.json`, `assets/defaults/mindsets/paper.lecture.json`, and `assets/defaults/mindsets/source-code.ingest.json`. Require packaged defaults to use the closed Mindset Source v2 contract with exactly one `default` question set and `question_ids: "all"`. Accept an existing valid topic v1 Source as implicit-default compatibility state without rewriting it.
...
7. **Specialize conservatively**. Emit v2 for every new or explicitly regenerated Source. Preserve the `mindset_key`, exact `additional-questions` collector, and one nonempty question set named `default`. The agent may adapt canonical questions and `additional_notes` from stable overview concerns, retain ids for reused concepts, assign unique ids to new or changed concepts, and add specialized sets only for stable task variants supported by the overview. Give each set one low-authority triggering condition and `question_ids` equal to `"all"` or an ordered list; reuse canonical ids across lists instead of duplicating question objects. Refer dynamically to the active task and survey context and do not invent a future Direction Set, Survey Contract, evidence portfolio, comparison contract, or Run task.

...
skill_invocation_notation: >
  ...
  SKILL-MAIN.md and are loaded explicitly through their parent. Invoke this protected
  member as `isomer-ext-kaoju-entrypoint->topic-creator`.
---
...
Use for the public Kaoju `create-topic` derivation phase, the extension-local create-missing preflight before a concrete mutation-bearing Kaoju research Run, or an explicit Source inspection, regeneration, replacement, or reconciliation request. Installation, welcome, help, `explore`, and status-only management may report missing state but must not invoke this mutation workflow.
...
- DO NOT use a packaged default as runtime fallback when a topic Source is invalid or missing after ensure.
...
Lead with the topic and creation or preservation outcome. Name the semantic label, current paths, keys, digests, invalid fields, drift advisories, and exact repair route. Do not emit raw Source JSON unless the user requests it.

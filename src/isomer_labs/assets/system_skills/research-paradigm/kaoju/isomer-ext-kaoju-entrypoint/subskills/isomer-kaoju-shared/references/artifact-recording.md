...
Use `project artifacts put` for a new ordinary current-state object or append-only event. Use `project artifacts revise` for a current-state content change only when the resolved binding permits revision, so the prior version remains historical. A `mutable_state` binding is an explicit exception: invoke its owner service, require the current opaque state token, update the same stable record, and emit lightweight audit evidence without retaining old stock bytes. `KAOJU:PAPER-TEMPLATE-MYST` uses `ext kaoju paper template --kind content`; `KAOJU:PAPER-TEMPLATE-LATEX` uses `--kind latex`. Never write either SQL row or managed tree directly. Derived `KAOJU:PAPER-TEMPLATE-TEX` snapshots preserve every LaTeX state used by a paper, while paper-local `KAOJU:PAPER-DRAFT-TEX` repair does not mutate stock. Create deltas, audits, attempts, failures, repairs, and Runs as separate evidence records under their own bindings. Use metadata-only update only for lifecycle or routing fields that do not change accepted content. A repaired or adapted Run never revises a faithful Run.

...
5. **Preflight target prerequisites and mutation posture**. Resolve the command page's accepted inputs, audit state, readiness, known producer routes, and Gates before beginning the target Run. `create-topic` delegates generic state to `isomer-op-entrypoint->topic-create`, invokes `isomer-ext-kaoju-entrypoint->topic-creator`, and stops without a research Run. Installation, welcome, help, `explore`, and status-only management remain non-mutating: they may report uninitialized mindset intent and the exact `create-topic` or concrete-action route, but they do not enumerate topics, invoke create-missing, create a Run, or write a Source. If another producer can satisfy a gap and the user gave only an ordinary target request, return `paused` and present Run to the target, Execute the next prerequisite only, Inspect or choose another route, and Stop before prerequisite mutation.
6. **Ensure topic-owned Mindset Sources before concrete work**. For a mutation-bearing research command with one reconciled topic and concrete `topic.intent.overview`, invoke `isomer-ext-kaoju-entrypoint->topic-creator` in create-missing mode before Run creation. Preserve every existing valid Source. An invalid existing Source blocks with exact diagnostics and the Kaoju `create-topic` repair route; never overwrite it or substitute a packaged default. This extension-local preflight covers topics created before Kaoju installation and does not add callbacks or Kaoju data to the generic Topic Creator.
...
9. **Snapshot the applicable mindset**. Use checked process routes to select `paper.deep-dive` for deep or full-text paper examination, `paper.skimming` for skim or triage examination, and `source-code.ingest` for repository or source-tree examination. Pause on ambiguous paper depth. After Run creation, re-resolve and validate the selected topic Source, pin the current Research Topic and one current Survey Contract, create the exact Run-scoped `KAOJU:MINDSET-RECORD` snapshot, and include its ref in Run inputs and every focused-owner handoff. Actions without a declared route continue unchanged.
...
11. **Dispatch focused owners**. Invoke only the skills and typed CLI services named by the command page, retaining the pinned `--topic <topic>`, applicable worker selector, and Mindset Record ref across cwd changes. Consumers answer and checkpoint the immutable materialized questions and do not re-read a changed Source during the active Run. Operational support mutation uses a recorded Service Request and managed research execution uses an Execution Adapter Command Request. Repository acquisition and identity verification are the explicit exception: the acting user or agent runs prompt-sensitive commands externally, verifies the result, and only then invokes semantic registration and typed Artifact operations.
12. **Checkpoint each stage and mindset**. Record completed refs, pending Gate, blockers, Service Requests, terminal status, and the first incomplete stage as the resume hint. Before an applicable Run completes, pauses, or blocks, revise its Mindset Record with optimistic concurrency, mark every materialized or explicitly assigned supplemental question answered, unresolved, or not applicable, mark the collector checked, and retain evidence refs. Reject applicable claim-bearing acceptance or `complete` without a terminal Record.
13. **Route mid-reading questions by explicit target**. Ordinary paper and source-code follow-up questions remain in Source Digest, Claim-Evidence Ledger, Associated Source Code, or another applicable reading Artifact. Add supplemental Record rows only under explicit Record or both targeting. Source-only requests directly validate and edit the topic Source without a Record row; Record-only requests use `record_only`; both requests move `source_update_requested` to `source_updated` with the new relative path and digest while the active Record keeps its original snapshot. Clarify an ambiguous bare-mindset mutation request.
...
16. **Return one terminal report**. Report `complete`, `paused`, or `blocked`; accepted refs; Run, Mindset Record, Gate, Service Request, and blocker refs; limitations; and resume point. A bounded procedure does not choose another macro intent. An explicitly authorized prompt-level controller may consume an in-closure recovery route only after this report is recorded.

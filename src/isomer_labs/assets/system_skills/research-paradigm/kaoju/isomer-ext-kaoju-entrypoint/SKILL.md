...
5. **Preflight target prerequisites and mutation posture**. Resolve the command page's accepted inputs, audit state, readiness, known producer routes, and Gates before beginning the target Run. `create-topic` delegates generic state to `isomer-op-entrypoint->topic-create`, invokes `isomer-ext-kaoju-entrypoint->topic-creator` to initialize Mindset Sources plus content and LaTeX `main` stock, inventories the recognized derived material, and stops without a research Run. Route “I have modified the derived materials, now apply them” and equivalent wording to the same protected topic owner for a complete read-only preflight and owner-specific application. Installation, welcome, help, `explore`, and status-only management remain non-mutating: they may report uninitialized derived intent and the exact `create-topic` route, but they do not enumerate topics, invoke create-missing or ensure-defaults, create a Run, write a Source, create template stock, or materialize an exchange path. If another producer can satisfy a gap and the user gave only an ordinary target request, return `paused` and present Run to the target, Execute the next prerequisite only, Inspect or choose another route, and Stop before prerequisite mutation.
...
7. **Plan authorized run-to recovery**. Only after explicit target-scoped run-to authorization, maintain an internal dependency plan with the native planning tool. The current agent acts as prompt-level controller, invokes each prerequisite as a separate bounded procedure Run, refreshes durable state after each terminal report, resumes the original target when ready, and stops after that target. Run-to does not satisfy a Gate. Run-to alone also does not delegate review; carry only review checkpoints that the current prompt explicitly delegates for the named target.
...
9. **Persist the applicable Run mindset resolution**. Use checked process routes to select `paper.lecture` for lecture-depth paper examination, `paper.deep-dive` for deep or full-text paper examination, `paper.skimming` for skim or triage examination, and `source-code.ingest` for repository or source-tree examination. Pause on ambiguous paper depth. After Run creation, re-resolve the selected deterministic topic Source. For a present valid v2 Source, inspect every non-default question-set `triggering_condition` against the concrete task and pinned Run context, select the single closest match with rationale, or select `default` with `default-fallback` when none applies. Expand `question_ids: "all"` in canonical question-catalog order; preserve an explicit list in its declared order. For a valid v1 Source, use its complete flat question list as implicit set `default` with `legacy-default`. Create the exact Run-scoped `KAOJU:MINDSET-RECORD` snapshot containing the selected set, condition, selection kind, rationale, and expanded question rows, then run `isomer-cli project runs resolve-mindset <run-id> --mindset-key <key> --record-ref <record-ref>`. For an absent file, run the same command with `--source-missing`; the service verifies absence and stores `skipped_source_missing` with no placeholder Artifact. The selected `paper.lecture` key commits that paper to a dedicated detailed survey section whether the disposition is `recorded` or `skipped_source_missing`; optional Source absence never downgrades the route to deep-dive behavior. Treat the first resolution as immutable and pass the Run ref, key, disposition, and Record ref only when present in every focused-owner handoff. Actions without a declared route continue unchanged.
10. **Resolve review and protected authorization**. Human review is the default. Use prompt-delegated agent review only when the current prompt explicitly and unambiguously delegates an eligible checkpoint for the named target; otherwise present the candidate and pause. Eligible checkpoints are explore handoff, Direction Set review, Reading List review, Comparison Intent review, bounded trial-plan review, paper structure and draft review, and bounded local paper-build authorization. Agent review may revise, narrow, reject, or accept within the accepted evidence, meaning, validation, resource, attempt, and target boundaries; it is not automatic acceptance. Stop for unresolved material intent and every nondelegable credential, restricted-data, license, destructive-action, unexpected-resource, public-exposure, publication, external-publication, or submission boundary. Generic run-to or review-automation wording does not satisfy those protected authorizations.
11. **Dispatch focused owners**. Invoke only the skills and typed CLI services named by the command page, retaining the pinned `--topic <topic>`, applicable worker selector, Run ref, mindset key, and persisted disposition across cwd changes. For `recorded`, also pass the Mindset Record ref; consumers answer and checkpoint its immutable materialized questions and do not re-read a changed Source during the active Run. For `skipped_source_missing`, consumers skip Record loading, mindset questions, collector checks, revisions, and terminal Record production while preserving the ordinary reading workflow and every semantic obligation of the selected route. A `paper.lecture` consumer must produce or block on the lecture exposition and detailed-presentation commitment even when no Mindset Record exists. Paper consumers resolve omitted content or LaTeX selectors through role-local topic `main`, then checked packaged `main` only when topic stock is verified absent; they never create topic stock or exchange content implicitly. Operational support mutation uses a recorded Service Request and managed research execution uses an Execution Adapter Command Request. Repository acquisition and identity verification are the explicit exception: the acting user or agent runs prompt-sensitive commands externally, verifies the result, and only then invokes semantic registration and typed Artifact operations.
12. **Checkpoint each stage and mindset**. Record completed refs, review mode, prompt basis, reviewing actor, rationale, affected refs, pending Gate, blockers, Service Requests, terminal status, and the first incomplete stage as the resume hint through existing owner-supported Run, decision, Artifact, and provenance fields. Before a `recorded` Run completes, pauses, or blocks, revise its Mindset Record with optimistic concurrency, mark every materialized or explicitly assigned supplemental question answered, unresolved, or not applicable, mark the collector checked, and retain evidence refs. A `skipped_source_missing` Run may complete without a Record and must report the key, missing Source posture, and absent Record ref. Reject applicable claim-bearing acceptance or `complete` when neither a terminal referenced Record nor a verified skipped resolution exists.
...
16. **Return one terminal report**. Report `complete`, `paused`, or `blocked`; accepted refs; human or agent-delegated review posture and its prompt-scoped basis when applicable; Run mindset key and disposition; Mindset Record ref or explicit absence; Gate, Service Request, and blocker refs; limitations; and resume point. A bounded procedure does not choose another macro intent. An explicitly authorized prompt-level controller may consume an in-closure recovery route only after this report is recorded.
...
| --- | --- | --- | --- |
| `acquire` | `isomer-kaoju-acquire` | A selected source needs governed material access, checkout, full-text fetch, dataset or model acquisition, immutable revision, or access and license capture after any paper identity lookup. | `isomer-ext-kaoju-entrypoint->acquire` |
...
| `discover` | `isomer-kaoju-discover` | The survey needs search strategy, terminology and source-class coverage, seed-route planning, version-family judgment, candidate disposition, Reading List composition, or durable discovery outputs. | `isomer-ext-kaoju-entrypoint->discover` |
...
| `paper-search` | `isomer-kaoju-paper-search` | A direct task or another Kaoju owner needs bounded paper identity lookup, topic search, citation retrieval, citation-neighborhood traversal, or adjacent-paper retrieval as provider output before discovery judgment or evidence acceptance. | `isomer-ext-kaoju-entrypoint->paper-search` |
...
| `topic-creator` | `isomer-kaoju-topic-creator` | Generic topic state is ready and Kaoju must initialize, inspect, apply, or reconcile topic-owned Mindset Sources and writing-template intent. | `isomer-ext-kaoju-entrypoint->topic-creator` |
...
Use `isomer-ext-kaoju-entrypoint->shared` for Artifact, evidence, interaction, review-mode, Gate, owner-routing, terminal, and prerequisite-recovery contracts. The shared Interaction and Gates reference defines default-human and explicit prompt-delegated agent review. The shared Prerequisite Recovery reference defines ordinary pause, target-scoped run-to authorization, separate procedure Runs, delegable review checkpoints, and nondelegable boundaries. Use `isomer-ext-kaoju-entrypoint->workspace` when readiness is missing or stale, and load only the selected command page plus its named focused owners.

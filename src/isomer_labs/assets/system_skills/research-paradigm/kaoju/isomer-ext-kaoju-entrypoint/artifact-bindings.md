...
`KAOJU:MINDSET-RECORD` is Run-scoped current state produced only for a `recorded` Run mindset resolution. It requires `run` and `survey_contract` relationships, exact Source and selected question-set snapshot fields, terminal answer states, collector posture, and optimistic revisions that preserve the immutable snapshot. A `skipped_source_missing` Run records Source and Record absence in Run state and creates no placeholder Artifact. A topic Mindset Source is not a produced semantic id or Artifact.

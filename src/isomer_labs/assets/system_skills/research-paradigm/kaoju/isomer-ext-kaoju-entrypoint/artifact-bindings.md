...
`KAOJU:MINDSET-RECORD` is Run-scoped current state. It requires `run` and `survey_contract` relationships, exact Source snapshot fields, terminal answer states, collector posture, and optimistic revisions that preserve the immutable snapshot. A topic Mindset Source is not a produced semantic id or Artifact.

...
2. Route canonical drafting to `draft-paper` and `isomer-ext-kaoju-entrypoint->write`. Resolve an explicit content template or content `main`, record its stable ref, state token, and observed digest, and require adaptive typed structure, MyST draft, citation map, revision log, and structured MyST validation.

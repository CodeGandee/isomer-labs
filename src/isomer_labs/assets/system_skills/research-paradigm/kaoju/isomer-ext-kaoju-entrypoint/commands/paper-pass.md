...
2. Route canonical drafting to `draft-paper` and `isomer-ext-kaoju-entrypoint->write`. Resolve an explicit content template or content `main`, record its stable ref, state token, and observed digest, and require adaptive typed structure, MyST draft, citation map, revision log, and structured MyST validation. Structure and draft review default to the human actor; explicit target-scoped prompt delegation permits recorded agent review within the accepted paper boundary.
...
5. When PDF output is requested, compose `build-paper-pdf`. Resolve an explicit LaTeX template or LaTeX `main` independently, snapshot and compose it with canonical MyST, inspect derived TeX, obtain build authorization, execute through `document_build`, inspect the PDF, and apply the publication Gate. Human review is the default for local build authorization; explicit target-scoped prompt delegation may authorize a bounded local build within accepted pins. Publication acceptance remains a protected actor Gate.
...
A missing or not-ready audit pauses drafting when an audit or repair route is available; reserve `blocked` for an unavailable external state change. An ambiguous unnamed template update, stale token, invalid tree, or edited export refresh causes no mutation. TeX syntax and presentation repair may proceed only inside the approved build policy; material content, dependency, toolchain, or interpretation repair requires a revised plan and fresh review authorization. Agent review may authorize that revision only inside explicit target-scoped delegation and the accepted build boundary. Publication acceptance, publication, and submission always pause at their protected actor Gates. Resume at the first incomplete audit, repair, template discovery or construction, draft, exchange, conversion, inspection, build, validation, or publication stage.

...
4. Resolve review mode. Under default human review, let the actor select one or several proposals, add a custom direction, close or defer a proposal with reason, reject all, or request revision. Under explicit target-scoped prompt delegation, let the agent make those choices within the accepted boundary and record its rationale. Author one option outcome and disposition rationale per proposal. A merely unselected proposal remains open.
5. After human confirmation or valid prompt-delegated agent review, invoke `isomer-op-entrypoint->research-ideas` and persist `KAOJU:DIRECTION-SET` through the active v2 `project artifacts put` or `revise` binding. Include `research_idea_effects` with `atomic=true`, explicit facets, every exact realization, one complete generation, every Decision Record option, each justified transition with actor, rationale, closure reason, and terminal refs, and the review mode and prompt basis through existing supported provenance.
...
Direction review is mandatory and defaults to explicit human confirmation. Explicit target-scoped prompt delegation permits recorded agent review without another user turn. Material ambiguity outside delegated scope, missing workspace readiness, binding failure, or rejected proposals pause at `propose-directions` or `confirm-directions`. Resume from the first incomplete stage with the Run and Direction Set refs.

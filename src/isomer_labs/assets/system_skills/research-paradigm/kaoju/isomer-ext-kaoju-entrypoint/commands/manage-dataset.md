...
`manage-dataset()` is a terminal task selector when invoked without a child. It presents these actions and waits for or infers exactly one selection from the user's task.

| Child Command | Use For | Detail |
| --- | --- | --- |
| `isomer-ext-kaoju-entrypoint->manage-dataset()->register()` | Register one externally owned dataset in the canonical manifest. | [commands/manage-dataset/register.md](commands/manage-dataset/register.md) |
| `isomer-ext-kaoju-entrypoint->manage-dataset()->list()` | List canonical dataset registrations. | [commands/manage-dataset/list.md](commands/manage-dataset/list.md) |
| `isomer-ext-kaoju-entrypoint->manage-dataset()->show()` | Inspect one registration and its lineage. | [commands/manage-dataset/show.md](commands/manage-dataset/show.md) |
| `isomer-ext-kaoju-entrypoint->manage-dataset()->refresh()` | Reinspect one registration and preserve its prior observation. | [commands/manage-dataset/refresh.md](commands/manage-dataset/refresh.md) |
| `isomer-ext-kaoju-entrypoint->manage-dataset()->remove()` | Remove only the managed link and revise registration state. | [commands/manage-dataset/remove.md](commands/manage-dataset/remove.md) |

## Workflow

1. **Select an action**. Accept exactly one of `register`, `list`, `show`, `refresh`, or `remove` and resolve the Topic Workspace. When no child command or unambiguous task is supplied, return the Subcommands table as the terminal result instead of guessing.

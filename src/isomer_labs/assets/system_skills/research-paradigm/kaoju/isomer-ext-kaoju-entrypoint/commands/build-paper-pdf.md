...
3. Run `paper tex-status` and require the write agent to perform the fill directly in the derived TeX: frontmatter title and authors into the venue constructs, abstract and keywords into their environments, a bibliography materialized from the citation map so every `\cite` resolves, and real venue tables for marked Markdown tables. MyST holds all TeX content; TeX drift is presentation-only. Initialization never claims build readiness; `build-pdf` refuses a tree while unfilled-obligation diagnostics remain. Report stocked-template drift and paper-local repair drift without recomposition or stock mutation.

...
8. **Persist through typed operations**. Use `project artifacts put` for a new ordinary current-state object or append-only event and `project artifacts revise` only when the binding permits revision. A binding with `mutable_state` requires its named owner service; for `KAOJU:PAPER-TEMPLATE-MYST`, use `ext kaoju paper template` so stable identity, name uniqueness, expected-state checks, atomic managed-tree replacement, and mutation audit remain service-owned. Let the service infer record kind, profile, semantic label, content mode, scope policy, and managed locator.

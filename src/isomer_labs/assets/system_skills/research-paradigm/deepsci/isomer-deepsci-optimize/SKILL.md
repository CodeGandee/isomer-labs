...
5. **Promote or prepare one line**. Record <PROMOTED_OPTIMIZATION_LINE> when a brief deserves durable line status, or create <OPTIMIZATION_ATTEMPT_RECORD> for a within-line seed, loop, smoke, patch, or quick validation candidate. Read `references/operational-guidance.md`, `references/codegen-route-playbook.md`, and `references/candidate-board-template.md`.

...
4. **Preserve skill ownership**. Route domain work back to the owning production DeepSci skill; use shared only for vocabulary and coordination consistency. Read `references/coordination-contract.md`.
5. **Route bootstrap questions**. Use `isomer-rsch-workspace-mgr` when the issue is post-preparation placeholder binding, semantic record surfaces, Topic Actor Workspace access posture, formal Agent Workspace access posture, current cwd context, or missing bootstrap support. Read `references/coordination-contract.md`.
...
- Treating shared as the post-preparation bootstrap manager instead of routing to `isomer-rsch-workspace-mgr`.
...
- Bootstrap check: post-preparation workspace bootstrap questions route to `isomer-rsch-workspace-mgr`, with the current cwd classified as Topic Workspace, Topic Actor Workspace, `topic.repos.main`, or formal Agent Workspace when possible.

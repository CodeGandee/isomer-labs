...
7. **Validate and record the result**. Produce <EXPERIMENT_RESULT_SUMMARY>, <CLAIM_VALIDATION_RECORD>, and <EXPERIMENT_ARTIFACT_MANIFEST> with metric completeness, finite values, comparability, claim update, baseline relation, failure mode, caveats, and exact evidence pointers. Read `references/run-record-template.md`, `references/evidence-ladder.md`, and `references/operational-guidance.md`.

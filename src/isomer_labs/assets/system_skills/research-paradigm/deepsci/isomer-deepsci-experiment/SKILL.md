...
7. **Validate and record the result**. Produce DEEPSCI:EXPERIMENT-RESULT-SUMMARY, DEEPSCI:CLAIM-VALIDATION-RECORD, and DEEPSCI:EXPERIMENT-ARTIFACT-MANIFEST with metric completeness, finite values, comparability, claim update, baseline relation, failure mode, caveats, exact evidence pointers, and canonical lineage from the main run. If the result changes a Research Idea, invoke `$isomer-research-idea-recording` and commit an explicit exploration or evidence transition with terminal refs, or introduce a distinct follow-up concept with justified lineage; otherwise leave the idea facets unchanged. Read `references/run-record-template.md`, `references/evidence-ladder.md`, and `references/operational-guidance.md`.

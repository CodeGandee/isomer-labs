...
Idea-recording reminder: experiment evidence may suggest that a Research Idea is supported, refuted, narrowed, stale, or superseded, but it must not change idea status implicitly. When a result should update idea state or create a follow-up idea, make an explicit `isomer-cli --print-json ext research ideas upsert`, `realize --source-json-path <exact-object-path>`, and `lineage add` write after reading `isomer-deepsci-shared`; do not realize a Primary Idea to a whole result payload, metrics section, artifact list, or rendered Markdown.
...
7. **Validate and record the result**. Produce DEEPSCI:EXPERIMENT-RESULT-SUMMARY, DEEPSCI:CLAIM-VALIDATION-RECORD, and DEEPSCI:EXPERIMENT-ARTIFACT-MANIFEST with metric completeness, finite values, comparability, claim update, baseline relation, failure mode, caveats, exact evidence pointers, and canonical lineage from the main run. If the result changes a Research Idea, read `isomer-deepsci-shared` and make an explicit idea status or follow-up write; otherwise leave idea state unchanged. Read `references/run-record-template.md`, `references/evidence-ladder.md`, and `references/operational-guidance.md`.

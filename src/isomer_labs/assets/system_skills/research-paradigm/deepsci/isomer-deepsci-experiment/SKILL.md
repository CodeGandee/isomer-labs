...
7. **Validate and record the result**. Produce <EXPERIMENT_RESULT_SUMMARY>, <CLAIM_VALIDATION_RECORD>, and <EXPERIMENT_ARTIFACT_MANIFEST> with metric completeness, finite values, comparability, claim update, baseline relation, failure mode, caveats, exact evidence pointers, and canonical lineage from the main run. If the result changes a Research Idea, read `isomer-deepsci-shared/references/research-idea-recording.md` and make an explicit idea status or follow-up write; otherwise leave idea state unchanged. Read `references/run-record-template.md`, `references/evidence-ladder.md`, and `references/operational-guidance.md`.

...
- The paper idea, claim boundary, or evidence mapping is still immature; use `isomer-rsch-paper-outline` first.
- The requested change requires new results rather than writing from existing evidence; route to `isomer-rsch-analysis` or `isomer-rsch-experiment`.
- The task is only first-pass plotting or final figure QA; use `isomer-rsch-paper-plot` or `isomer-rsch-figure-polish`.
...
3. **Validate the outline before drafting**. Check `<PAPER_OUTLINE>` for a real reader-facing thesis, scoped claims, method abstraction, evaluation plan, analysis plan, and evidence map. Route to `isomer-rsch-paper-outline` when the outline cannot support writing.
...
7. **Plan displays before prose**. Create `<DISPLAY_PLAN>` for figures, tables, and appendix displays. Route first-pass standard plots to `isomer-rsch-paper-plot` and durable paper-facing figures to `isomer-rsch-figure-polish`.

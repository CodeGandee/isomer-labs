...
2. Invoke `$isomer-research-operation-set-recording` and inspect the operation set after end callbacks. Inventory every regular material file regardless of Git status. A Git commit, rendered Markdown file, terminal summary, or worker path does not make an artifact durable.
...
4. Keep the two lineage layers explicit. Record canonical immediate record parents for every created or revised record. For idea-bearing payloads, invoke `$isomer-research-idea-recording` and pass exact `research_idea_effects`, including exact realization paths, through the same atomic record transaction. Do not infer Idea Lineage Edges from record lineage.

...
- The figure already exists and needs final QA; use `isomer-rsch-figure-polish`.
...
7. **Route durable figures**. If the figure is paper-facing, appendix-facing, milestone-facing, or final, create `<FIGURE_POLISH_HANDOFF>` for `isomer-rsch-figure-polish`; otherwise stop with the first-pass figure and substitution record.

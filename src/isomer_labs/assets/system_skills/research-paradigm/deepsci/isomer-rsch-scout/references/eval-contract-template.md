...
3. **Name ambiguity honestly**. If two contracts conflict and would change downstream conclusions, record `<SCOUT_BLOCKER_RECORD>` or route to `isomer-rsch-decision`.
...
- Downstream sufficiency: `isomer-rsch-baseline`, `isomer-rsch-idea`, and `isomer-rsch-experiment` can use the contract without re-deriving task, split, metric, or fairness.

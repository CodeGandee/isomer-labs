...
6. **Generate a bounded frontier**. Run one deliberate divergence pass unless durable evidence already makes the route obvious. Produce DEEPSCI:RAW-IDEA-SLATE, DEEPSCI:CANDIDATE-IDEA-FRONTIER, and DEEPSCI:REJECTED-AND-DEFERRED-IDEAS with canonical parents from objective, board, survey, and mechanism records; use one generation group when sibling candidates share the same parent set. Read `references/controlled-brainstorming-playbook.md`, `references/idea-generation-playbook.md`, `isomer-deepsci-shared`, and `isomer-deepsci-shared` before narrowing.
...
- `isomer-deepsci-shared` for canonical Research Ideas, Primary Idea visibility, Idea Realizations, idea generation groups, `subsumes`, and idea lineage edges.

...
3. **Lock the paper contract**. Produce or update `<PAPER_CONTRACT>` with the central claim, venue or report target, evidence boundary, required figures, citation state, and bundle status. Use `references/oral_package_patterns.md` when the target is an oral-ready paper package.

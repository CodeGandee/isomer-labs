...
the Isomer Execution Adapter to perform an import, executable, version, and smoke-test check
...
Record the result with `science.package_check`. Use
...
- Do not call a value `computed` unless the corresponding run or analysis happened in the current Topic Workspace and evidence paths are recorded.
- Do not copy package knowledge-base material into the Research Topic without preserving its source and license context.

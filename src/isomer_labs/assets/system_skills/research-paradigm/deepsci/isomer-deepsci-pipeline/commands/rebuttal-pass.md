...
1. **Check entry context**. Ensure `DEEPSCI:REVIEW-REPORT` and `DEEPSCI:DRAFT-SECTION-SET` are available from the caller context. `DEEPSCI:COMPARATOR-CONTRACT` is required if the action plan routes through the `run` stage.

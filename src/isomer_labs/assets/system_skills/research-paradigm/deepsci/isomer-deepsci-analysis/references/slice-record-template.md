...
- Prefer semantic slice ids that can survive later write-back (if paper metadata exists, otherwise align ids with DEEPSCI:ANALYSIS-WRITEBACK-MAP).
...
- DEEPSCI:ANALYSIS-SLICE-RECORD must include question, intervention or inspection target, fixed conditions, metric or observable, evidence source, claim update, comparability verdict, and next action.

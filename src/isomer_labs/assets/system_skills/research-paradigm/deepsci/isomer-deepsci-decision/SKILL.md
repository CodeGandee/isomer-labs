...
Idea-recording reminder: when a decision selects, rejects, defers, supersedes, collapses, or branches ideas, record the Research Idea status, sibling generation group, Idea Realization, and idea lineage explicitly with `isomer-cli --print-json ext research ideas`. Use `realize --source-json-path <exact-object-path>` for the selected, rejected, or deferred idea object, not for a decision payload root, route notes, rationale section, or rendered Markdown. Use `subsumes` when one accepted idea covers another idea's mechanism, ablation, or test role; read `isomer-deepsci-shared/references/research-idea-recording.md` before the durable verdict is complete.
...
3. **State the route question**. Record <ROUTE_QUESTION> with the real choice, strongest support, strongest contradiction, main risk, main cost, new evidence, and canonical parents from the result, blocker, or route state being judged. Read `references/research-route-criteria.md` and `isomer-deepsci-shared/references/artifact-lineage-recording.md` before evidence compression.
4. **Choose the smallest canonical action**. Use <DECISION_EVIDENCE_PACKET> to select one action from the canonical action set without hiding rejected alternatives. When the action changes idea state, prepare explicit Research Idea status writes, sibling generation-group refs, and idea lineage edges before recording the verdict. Read `references/canonical-actions.md`, `references/research-route-criteria.md`, and `isomer-deepsci-shared/references/research-idea-recording.md`.
5. **Record the verdict**. Create <ROUTE_DECISION_RECORD> with verdict, action, reason, evidence, rejected alternatives, next route, and `follow_up_to` or `selected_from` lineage from the decisive parent records. Then write idea selection, rejection, deferral, branch, collapse, and `subsumes` relationships through `ext research ideas` so the GUI does not infer them from prose. Read `references/strategic-decision-template.md`.
6. **Preserve the resume point**. Write <DECISION_CHECKPOINT_MEMORY> when the decision changes the active route, or create <USER_DECISION_REQUEST> or <DECISION_BLOCKER_RECORD> when local evidence cannot decide safely; use `revision_of` through the revise command for content-changing checkpoint updates. Read `references/checkpoint-memory-template.md` and `references/operational-guidance.md`.

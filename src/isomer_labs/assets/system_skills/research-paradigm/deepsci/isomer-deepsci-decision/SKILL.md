...
3. **State the route question**. Record DEEPSCI:ROUTE-QUESTION with the real choice, strongest support, strongest contradiction, main risk, main cost, new evidence, and canonical parents from the result, blocker, or route state being judged. Read `references/research-route-criteria.md` and `isomer-deepsci-shared` before evidence compression.
4. **Choose the smallest canonical action**. Use DEEPSCI:DECISION-EVIDENCE-PACKET to select one action from the canonical action set without hiding rejected alternatives. When the action changes idea state, prepare explicit Research Idea status writes, sibling generation-group refs, and idea lineage edges before recording the verdict. Read `references/canonical-actions.md`, `references/research-route-criteria.md`, and `isomer-deepsci-shared`.
5. **Record the verdict**. Create DEEPSCI:ROUTE-DECISION-RECORD with verdict, action, reason, evidence, rejected alternatives, next route, and `follow_up_to` or `selected_from` lineage from the decisive parent records. Then write idea selection, rejection, deferral, branch, collapse, and `subsumes` relationships through `ext research ideas` so the GUI does not infer them from prose. Read `references/strategic-decision-template.md`.

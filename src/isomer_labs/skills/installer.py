...
class SystemSkillRecord:
    """One installable packaged Isomer public pack."""
...
def resolve_system_skill_selection(
    *,
    ...
    for skill_name in skills:
        ...
        if deprecated or public_skill != skill_name:
    ...
    *,
    ...
    if records_to_install:
        staging_root, staged_paths = _stage_pack_projections(
            target.skill_root,
            records_to_install,
            {record.name: projection_mode for record in records_to_install},
        )
        ...
        try:
            ...
            for record in records_to_install:
                ...
                if destination.exists() or destination.is_symlink():
                    backup_path = backup_root / record.name
                    destination.rename(backup_path)
                staged_paths[record.name].rename(destination)
...
def uninstall_system_skills(target: SystemSkillTarget, selection: SystemSkillSelection) -> SystemSkillUninstallResult:
    ...
    for record in selection.skills:
        ...
        if (destination.exists() or destination.is_symlink()) and (
            tracked is None or tracked.pack_id != record.pack_id
        ):
            raise SystemSkillInstallError(
                f"Refusing to uninstall untracked or legacy-only public pack path: {destination}. "
...
def upgrade_system_skills(
    ...
    *,
    ...
    """Stage complete public packs, commit receipt v4, then clean tracked stale paths."""
    ...
    tracked_names = set(current_records) | set(legacy_records) | set(existing_legacy_paths)
    for record in selection.skills:
        destination = target.skill_root / record.name
        if (destination.exists() or destination.is_symlink()) and record.name not in tracked_names:
            raise SystemSkillInstallError(
                f"Upgrade destination conflicts with an untracked path: {destination}. "
                "Preserve or relocate that path before retrying."
            )

    modes: dict[str, ProjectionMode] = {}
    for record in selection.skills:
        prior = current_records.get(record.name) or legacy_records.get(record.name)
        modes[record.name] = projection_mode or (prior.projection_mode if prior is not None else "copy")

    stale_candidates: dict[str, SystemSkillLegacyPathRecord] = dict(existing_legacy_paths)
    for tracked in (*current_records.values(), *legacy_records.values()):
        if tracked.name in selected_names:
    ...
    for name in sorted(known_legacy_names - set(stale_candidates) - selected_names):
    ...
    try:
        ...
        for record in selection.skills:
            ...
            staged_paths[record.name].rename(destination)
            committed.append((destination, backup_path))
            receipt_record = _manifest_record_for(record, modes[record.name])
            new_records[record.name] = receipt_record
            refreshed.append(_installed_record(record, destination, modes[record.name], receipt_record))
        try:
...
def _stage_pack_projections(
    ...
    try:
        for record in records:
            staged = staging_root / record.name
            _project_skill(record.source_path, staged, projection_mode=modes[record.name])
            errors = _pack_projection_errors(record, staged)
            if errors:
                raise SystemSkillInstallError(
                    f"Staged public pack {record.name!r} failed validation: {'; '.join(errors)}"
                )
            staged_paths[record.name] = staged
...
def _remove_stale_path(path: Path) -> None:
    """Remove one exact receipt-tracked stale path after v4 commit."""

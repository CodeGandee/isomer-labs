...
- Research skills request semantic workspace surface labels, Artifact kinds, or workspace scopes instead of inventing paths.
- Agent-scoped path queries use Effective Agent Context. If the current working directory is already inside a recorded Agent Workspace, commands can resolve labels such as `agent.workspace`, `agent.private_artifacts`, or `agent.tmp` without a separate Agent Name argument.

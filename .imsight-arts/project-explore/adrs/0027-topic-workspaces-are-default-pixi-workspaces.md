...
- The Project Manifest records each Topic Workspace Pixi manifest through `[[topic_standalone_pixi_bindings]]`, using a Project-root-relative `manifest_path`. The previously optional standalone binding table becomes the default mechanism for topic environment binding.
...
- `isomer-cli doctor` checks that each active Topic Workspace has a valid Pixi manifest, that declared environments exist, and that the manifest path is inside the Project root. It does not install environments.

...
Agent Instance ids remain globally unique runtime identities, but Isomer Labs uses topic-local Agent Names for launch-facing Agent Workspace paths. The flat default Agent Workspace layout is `<topic-workspace>/agents/<agent-name>/` inside each Topic Workspace. Each directory is a Git worktree for an agent-owned branch namespace such as `per-agent/<agent-name>/main`, while Workspace Runtime records bind the topic-local Agent Name to the globally unique Agent Instance id.
...
- Store Agent Workspaces directly under `<topic-workspace>/agents/<agent-name>/`.
...
- The Workspace Path Resolver should derive launch-facing Agent Workspace paths from the selected Topic Workspace and `agent_name` as `<topic-workspace>/agents/<agent-name>/`. It may report compatibility diagnostics when older material provides only `agent_workspace_ref`.

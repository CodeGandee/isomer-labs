...
**Toolbox**:
A project-local extension package that can provide User Skill Callback material, Toolbox runtime param definitions, and named default runtime-param bundles. A Toolbox has a durable `toolbox_id`, lives under an explicit user-selected source path such as `skillset/toolboxes/<toolbox-id>/`, and is registered through Project Manifest or Topic Workspace Manifest Toolbox configuration tables. Toolbox configuration is user-authored runtime configuration, not a packaged system skill, not Workspace Runtime state, and not an executable provider payload.
_Avoid_: Project plugin, generic plugin package, provider payload, installed system skill
...
An ignored, Project-local, disposable projection derived from one Source Topic Workspace for privacy review, sanitized component history construction, and publication to one credential-safe user-selected remote. It can exist after Topic Workspace registration and before Workspace Runtime. It is rebuildable from a Publication Binding, fetched compatible sanitized branches, and a Publication Projection Manifest after a successful publication. A clean copy based on the exact observed publication commit may support an incremental update; a missing copy may be reconstructed, while a dirty or divergent copy is preserved and blocks reuse or requires a separate disposable recovery repository. It is not a Topic Workspace, Topic Actor Workspace, Agent Workspace, canonical source, Workspace Runtime, Artifact authority, or research record source.
...
**Publication Binding**:
A credential-safe association among one Research Topic, its canonical Topic Workspace, one Project-relative Topic Publication Copy path, one remote name and locator, and an acknowledged remote visibility of `private`, `restricted`, or `public`. Its `exclusive_snapshot` authority permits exact planned fallback replacement and obsolete-ref deletion but does not make force replacement the default update strategy. Before Workspace Runtime exists, the binding may live only in the ignored copy-local Topic Git support root. A later approved publication mutation can validate and promote it to `<topic.runtime>/topic-git/`. A Publication Binding never stores credentials, signed locators, sensitive excerpts, raw private diffs, or source Git configuration.
_Avoid_: credential record, provider account, source remote, Workspace Runtime row

**Publication Projection Manifest**:
A tracked sanitized manifest inside `.isomer-publication/` in the Topic Publication Copy that records stable binding identity, canonical branch and history format, source-identical relative mappings, generated-entry origins, privacy dispositions, transformations, output fingerprints, selected component branches, Topic Main anchor relationships, and exact sanitized component commits for one approved projection. It omits plan ids, approval timestamps, push attempts, outcomes, absolute source paths, credentials, sensitive content, excluded content, source remote configuration, and source Git ancestry. It supports compatibility checks, conflict detection, and disposable publication-copy recovery, but it neither makes the Topic Publication Copy canonical nor restores an operational Topic Workspace.
_Avoid_: source inventory dump, private diff, source Git manifest, canonical workspace manifest

**Sanitized Publication History**:
The Git ancestry formed only from approved Topic Publication Copy commits for one matching Publication Binding and canonical publication topology. An initial sanitized publication commit has no Source Topic Workspace Git ancestor. A later compatible sanitized commit may use the freshly observed prior publication commit as its direct parent and publish through an ordinary fast-forward push. Incompatible history or a required privacy, credential, identity, license, or withdrawal purge may use an exact leased replacement under `exclusive_snapshot` fallback authority. Sanitized Publication History never includes source repository commits, source `.git` data, arbitrary remote edits, or an unresolved destination conflict.
_Avoid_: Source Topic Workspace history, operational backup, arbitrary remote history, automatic merge lineage
...
The topic-owned `topic-workspace.toml` file at the Topic Workspace root. It binds semantic workspace surface labels to concrete paths, Topic Actor bindings, actor-scoped workspace templates, or bounded Agent Workspace templates for one selected Topic Workspace. It can also carry topic-scope Toolbox registrations, Toolbox Runtime Param imports, and Toolbox Runtime Params for Research Topic, Topic Actor, and Topic Agent specialization. It is the topology authority for Topic Actor names and Topic Actor Workspace paths. It is topic-owned configuration, not Project Config Directory state, not a Project Manifest override, and not Workspace Runtime state.
_Avoid_: Project Manifest path registry, runtime database, hidden workspace config

**Semantic Workspace Surface Label**:
A stable dotted label for a workspace path surface, such as `topic.repos.main`, `topic.repos.main.projections.readonly`, `topic.repos.main.projections.writable`, `topic.repos.main.projections.manifest`, `topic.repos.<group...>.<repo-name>`, `topic.records.artifacts`, `topic.runtime.db`, `topic.actors_root`, `topic.actors.workspace`, `topic.actors.tmp`, `topic.agents_root`, `topic.tmp`, `topic.repos.main.tmp`, `agent.workspace`, `agent.private_artifacts`, `agent.public_share`, `agent.scratch`, or `agent.tmp`. Semantic labels are the user-facing path contract; compatibility surface ids such as `records_artifacts`, `topic_actor_workspace:<topic-actor-name>`, or `agent_workspace:<agent-name>` may remain for older path plans and diagnostics.
_Avoid_: hard-coded path, directory name as contract, arbitrary surface string

**Default Layout Profile**:
The built-in `isomer-default.v1` mapping from semantic workspace surface labels to the standard Topic Workspace, Topic Main Development Repository, external repository projection, Topic Actor Workspace, and Agent Workspace directories, such as `repos/topic-main`, `repos/topic-main/isomer-managed/topic-owned/{readonly,writable}/extern`, `repos/topic-main/isomer-managed/tracked/manifests/extern-projections.toml`, `records/artifacts`, `runtime`, `actors/<topic-actor-name>`, `agents/<agent-name>`, and the local ignored `tmp/` surfaces. Repository helper commands use `repos/extern/...` as the default physical namespace for canonical non-main topic repositories. It is a fallback and explicit materialization profile, not the only valid layout.
...
**Effective Topic Actor Context**:
The resolved process-local Topic Actor identity used by actor-scoped commands and semantic path queries. It includes Topic Actor Name, optional runtime kind, Topic Actor Workspace path, and source metadata from explicit selectors, supported environment variables, cwd-derived Topic Actor Workspace matching, lifecycle refs, or Topic Workspace Manifest bindings. It is a path-resolution convenience for human-orchestrated work and is not an Agent Instance id, an Agent Team Instance membership record, an access token, or filesystem-grade identity.
_Avoid_: Agent Instance context, Agent Workspace context, OS user, durable team identity

### Workspace Taxonomy

Use **workspace** only for the three filesystem work areas that Isomer manages directly:

- **Topic Workspace**: the topic-level work area declared by the Project Manifest.
- **Topic Actor Workspace**: the per-Topic Actor work area inside a Topic Workspace for human-orchestrated workers outside formal Agent Instance identity.
- **Agent Workspace**: the per-Agent Instance work area inside a Topic Workspace during team execution.
    ...
    Topic Workspace, declared by the Project Manifest
      ...
      Topic Main Development Repository, semantic label topic.repos.main
        Isomer-managed worker namespace, isomer-managed/
          External repository projections, semantic labels topic.repos.main.projections.readonly and topic.repos.main.projections.writable
          External projection manifest, semantic label topic.repos.main.projections.manifest
        Local Tmp Surface, semantic label topic.repos.main.tmp
      Canonical external repositories, semantic labels under topic.repos.<group...>.<repo-name>, helper-created under repos/extern/
      ...
      Topic Actor Workspace(s), semantic label topic.actors.workspace, created for Topic Actors during human-orchestrated work
        Local Tmp Surface, semantic label topic.actors.tmp
...
**Topic Actor**:
A topic-local, path-safe worker identity for human-orchestrated work inside one Topic Workspace. A Topic Actor can represent the reserved `operator` actor, a manually started coding agent, a shell session, a Houmao-backed helper, or another worker controlled by the Project Operator Session or Operator Agent without requiring a Topic Agent Team Profile, Agent Team Instance, Agent Instance, or Agent Workspace. Topic Actor bindings live in the Topic Workspace Manifest; Workspace Runtime may record registration or materialization provenance when available, but the manifest remains the topology authority.
_Avoid_: Agent Instance, Agent Role, Agent Name, Service Agent Instance, untracked worker label
...
An optional topic-scoped Service Agent posture or Agent Profile that serves as the Topic Workspace manager. A Topic Service Master starts with cwd at the resolved Topic Workspace directory, receives Project Operator Session or Operator Agent commands, and performs authorized topic-level operational work on their behalf. It can also receive bounded support requests from Agent Team Instance members when the requested operation requires the Topic Workspace cwd or topic-owned surfaces, such as Pixi environment configuration, Topic Main Development Repository or projection support, Workspace Runtime or runtime-support inspection, generated link or symlink introduction into Agent Workspaces, topic-owned diagnostics, and support Artifact writing. If no Topic Service Master is started, the Project Operator Session or Operator Agent remains responsible for these workspace-management duties. A Topic Service Master is still a Service Team member, not an Operator Agent, not a research team lead, and not a holder of research decision authority.
_Avoid_: Project operator, research master, team owner, owner of Gate decisions, required topic daemon
...
**Agent Workspace**:
A per-agent work area inside a Topic Workspace assigned to one Agent Instance for owned scratch files, local runtime state, logs, Agent Artifacts, and the worker agent's normal launch cwd. Its semantic label is `agent.workspace`; under `isomer-default.v1` that label resolves to `<topic-workspace>/agents/<agent-name>`, and the directory is a Git worktree of the prepared Topic Main Development Repository checked out to an agent-owned branch such as `per-agent/<agent-name>/main`. It is an advisory ownership boundary: Isomer records and documents expected access, but does not provide filesystem-grade access control.
_Avoid_: Private sandbox, secure workspace, role directory as a generic term

**Topic Main Development Repository**:
The topic-owned normal, non-bare Git repository resolved by semantic label `topic.repos.main`; under `isomer-default.v1` that label binds to `<topic-workspace>/repos/topic-main`. Topic Workspace environment setup creates, configures, and verifies this repository before Topic Actor Workspace or Agent Workspace setup materializes worktrees. It is the primary topic-owned source repository and integration surface for code-bearing topic work, and it acts as the Git anchor for Topic Actor Workspace worktrees and Agent Workspace worktrees. It is not the required shared cwd for every manually controlled worker. Its standard Isomer-specific worker namespace is `isomer-managed/`, split into Git-tracked Isomer material, untracked worker-owned material, topic-owned external repository projections, and generated links. Existing user-provided repositories should stay root-clean: Isomer-created material belongs under `isomer-managed/`, not top-level `extern/`, `shared/`, `tasks/`, `runs/`, or similar directories. It is not a Topic Workspace, not a Topic Actor Workspace, not an Agent Workspace, and not Workspace Runtime state.
_Also called_: Topic Main Repository
_Avoid_: Workspace Runtime, Topic Actor Workspace, Agent Workspace, teams directory, root shared directory

**Canonical External Repository**:
A non-main topic repository resolved through a semantic label under `topic.repos.<group...>.<repo-name>`, with `repos/extern/<repo-label-path>/` as the helper-created default physical namespace. Canonical external repositories store supporting third-party or topic-local code; they are not Agent Workspace worktree anchors. Topic environment setup may inspect or acquire them, and existing repositories are read-only evidence by default unless a target spec or user instruction explicitly authorizes mutation.
_Avoid_: Topic Main Development Repository, Agent Workspace, top-level topic-main extern directory

**External Repository Projection**:
An Isomer-managed exposure of a canonical external repository inside the Topic Main Development Repository. Read-only projections use `topic.repos.main.projections.readonly`, which resolves under `isomer-managed/topic-owned/readonly/extern/`; writable projections use `topic.repos.main.projections.writable`, which resolves under `isomer-managed/topic-owned/writable/extern/`. Projection metadata lives at `topic.repos.main.projections.manifest`, which resolves to `isomer-managed/tracked/manifests/extern-projections.toml`. Projection access intent names whether agents should treat the projected repo as read-only or writable, and writable projections must use an isolated copy, clone, worktree, or explicitly authorized source.
_Avoid_: canonical source repository, top-level `extern/` inside topic-main, unrecorded symlink
...
A Git worktree rooted at the resolved `agent.workspace` path and attached to the prepared Topic Main Development Repository. Under `isomer-default.v1`, that path is `<topic-workspace>/agents/<agent-name>`. The default branch is `per-agent/<agent-name>/main`; future branches for that agent stay under `per-agent/<agent-name>/`.
_Avoid_: Plain directory, task workspace, secure sandbox

**Topic Workspace Records Root**:
The owner-preserved records surface resolved by semantic label `topic.records`; under `isomer-default.v1` that label binds to root `records/` inside a Topic Workspace. It stores Artifacts, task records, Run records, View Manifests, logs, snapshots, and normalized outputs that are not normal worker input. Worker-visible copies or summaries belong in the resolved Topic Main Development Repository `isomer-managed/` namespace only after explicit publication or projection.
...
_Avoid_: Topic Main Development Repository, Agent Workspace, cache unless a later contract marks a file disposable

**Local Tmp Surface**:
A local, Git-ignored, disposable, non-shared workspace surface resolved by semantic labels such as `topic.tmp`, `topic.repos.main.tmp`, `topic.actors.tmp`, and `agent.tmp`. Under `isomer-default.v1`, these labels bind to `tmp/` directories under the Topic Workspace, Topic Main Development Repository, Topic Actor Workspace, and Agent Workspace respectively. A Local Tmp Surface is not Workspace Runtime truth, not an Artifact, and not worker-visible collaboration material until selected content is explicitly promoted, published, or recorded elsewhere.
_Avoid_: Artifact, record, public share, durable cache

**Worker Visibility Boundary**:
The advisory distinction between worker-facing surfaces and topic-owner, runtime, or disposable local surfaces. Topic Actors normally operate inside the resolved `topic.actors.workspace`, and formal team agents normally operate inside the resolved `agent.workspace`. Workers exchange information through Git branches, `isomer-managed/tracked/` material, owner-approved worker public shares, policy-controlled `isomer-managed/topic-owned/` projections, generated `isomer-managed/links/`, or topic-owned Pixi tasks. They should not treat resolved records, runtime support, runtime database, Local Tmp Surfaces, or Project Config material as ordinary input.
...
A checked immutable content or LaTeX `main` tree shipped inside the Kaoju write owner. Explicit Kaoju topic initialization may copy it into mutable topic-owned named stock. When role-local topic `main` is verified absent, paper production may instead snapshot the packaged tree directly and record its identity, resource version, role, name, and digest. That fallback creates no topic record or exchange file. It never replaces an invalid topic record or an explicitly missing selector.
_Avoid_: named template stock, writing-template exchange, Mindset Source, venue template

**Writing-Template Exchange**:
A non-canonical owner-editable working copy of selected Kaoju content or LaTeX template stock. Workspace Path Resolution exposes semantic root `topic.paper.template_exchange_root`, whose built-in default is plural `intent/derived/writing-templates`; Kaoju appends `<kind>/<name>/`. Export metadata records whether the copy came from topic stock or a packaged default. Editing the directory does not change canonical stock. Accepted edits require typed create or state-checked update, affect future paper work, and never rewrite historical snapshots.
_Avoid_: canonical template record, Artifact root, direct database state, generated TeX draft

**Mindset Source**:
A directly user-editable derived-intent JSON question catalog that tells an agent what to ask itself and answer while an applicable Workflow proceeds. In Kaoju, each Source is a deterministic direct child `<mindset_key>.json` beneath semantic label `topic.intent.kaoju_mindsets`, whose default path is `intent/derived/mindsets`. A v2 Source owns a stable key, purpose, applicability, ordered canonical questions, named question sets, one repeatable additional-question collector, prompts, bounded `additional_notes`, answer expectations, required postures, evidence expectations, and optional derivation provenance. Each question set owns one descriptive triggering condition and selects either `"all"` questions in catalog order or an ordered list of canonical question ids; ids may appear in many sets, and every Source has exactly one set named `default`. For one applicable Run, the agent selects one closest matching specialized set or falls back to `default`. A valid v1 Source remains a preserved implicit-default compatibility form. The Source is current topic intent rather than an Artifact: users may view, copy, or edit it directly, existing valid files survive create-missing and upgrades, and explicit replacement uses validation plus an observed-digest check. Source data remains low authority and cannot schedule a Workflow Stage, execute a command, authorize mutation, satisfy a Gate, accept evidence, or override instructions.
...
- Use `topic_actor` for a topic-local human-orchestrated worker identity, `topic_actor_workspace` for that actor's managed work area, and `effective_topic_actor_context` for actor-scoped path resolution.
...
- Avoid bare `workspace` when the scope could mean Project, Topic Workspace, Topic Actor Workspace, or Agent Workspace.
...
- Use `domain_agent_team_template`, `topic_team_specialization`, `topic_team_instantiation_packet`, `topic_agent_team_profile`, `agent_team_instance`, `project_operator_session`, `operator_agent`, `topic_actor`, `agent_role`, `agent_profile`, `agent_instance`, `capability_binding`, `coordination_policy`, `execution_adapter`, and `workflow_stage` for team and execution concepts.
- Use `service_team` for the built-in operational support team, `service_request` for one bounded support assignment, `service_agent_instance` for the runtime support actor, `topic_service_agent` for a topic-scoped service actor, and `topic_service_master` for the optional topic-scoped Service Agent posture or Agent Profile that manages Topic Workspace operations from the resolved Topic Workspace cwd.
...
- Use `topic_actor_workspace` for a managed work area assigned to a Topic Actor outside formal Agent Instance identity.
...
- A **Topic Workspace** may contain many **Topic Actor Workspaces** during human-orchestrated work.
- A **Topic Actor Workspace** is contained by exactly one **Topic Workspace**.
- A **Topic Actor Workspace** belongs to one active **Topic Actor** binding in the **Topic Workspace Manifest**.
- A **Topic Actor** may be controlled by a **Project Operator Session**, **Operator Agent**, human-started coding-agent session, shell session, or Houmao-backed worker without joining an **Agent Team Instance**.
- **Topic Actors** may coexist with **Agent Team Instance** material in the same **Topic Workspace**.
...
- Topic-local manually controlled workers that are not formal **Agent Instances** should be represented as **Topic Actors**, not as Agent Team Instance members.
...
- A **Topic Service Master** is an optional topic-workspace manager posture or **Agent Profile** for a **Topic Service Agent**. It performs bounded topic-owned operational work from the resolved **Topic Workspace** cwd; if it is not started, the **Project Operator Session** or **Operator Agent** performs those duties directly.
...
- Do not use bare "workspace" when **Project**, **Topic Workspace**, **Topic Actor Workspace**, and **Agent Workspace** are all plausible readings.
...
- Use **Research Inquiry Relationship** for graph links between inquiries. Do not use **Research Branch** unless quoting stale docs or referring to a separate compatibility note.
...
- Use **Topic Actor Workspace** for manually controlled topic-local worker cwd surfaces that are not formal Agent Instance workspaces. Do not call a Topic Actor Workspace an Agent Workspace, and do not imply Agent Team Instance membership from a Topic Actor binding.
...
- Use **Topic Main Development Repository** for semantic label `topic.repos.main`; under `isomer-default.v1` it binds to `<topic-workspace>/repos/topic-main`. Keep it as the Git anchor and integration surface for Topic Actor Workspace and Agent Workspace worktrees, not as the universal cwd for every manually controlled worker. Keep Isomer-specific worker-visible collaboration material under `isomer-managed/` inside that repository or inside its worktrees, and expose canonical external repositories only through the projection labels when workers need them from topic-main.
...
- Use **Local Tmp Surface** for local ignored `tmp/` surfaces such as `topic.tmp`, `topic.repos.main.tmp`, `topic.actors.tmp`, and `agent.tmp`. Do not call tmp output durable, shared, or recorded until it is explicitly promoted.
...
- Use **Topic Service Master** for the optional Topic Service Agent posture or Agent Profile that manages Topic Workspace operational work from the resolved Topic Workspace cwd. Do not make it required for topic operation, and do not make it a new holder of research authority.
